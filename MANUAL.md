...
Dashboard probes `bridge.info` before using newer binding fields. It replaces an empty legacy daemon with the bundled same-version Bridge. If a legacy daemon still has bindings, Dashboard keeps normal inspection/removal and controlled takeover available but rejects new links or returns to Feishu that cannot provide current completion tracking, and never sends the optional session summary to the older closed parser. A confirmed stale/pausing removal can gracefully migrate that daemon first only when it is the daemon's sole binding and no active turn would be disrupted; ambiguous remove acknowledgement is checked by snapshot and never blindly retried. As a one-time rolling-upgrade exception, removing an active/paused binding through the already-running legacy daemon cannot emit the newer lifecycle card: Dashboard preserves that daemon's canonical removal path instead of interrupting live authority, and a later current-capability operation upgrades it once empty.

If Claude or Codex is already running when a group binding acquires or regains Feishu ownership, the Bridge records a fenced result watch tied to the exact structured Agent session and turn boundary. The group is treated as busy while that inherited local task is running. After two stopped observations at least one second apart, terminal-control reads the Agent-owned JSONL transcript and returns only the final assistant message for that correlated activity; the bot sends it exactly once as a top-level green reply card. Terminal rendering, tool calls/results, the input composer, and footer text are never eligible card content. A result that is not yet durable is retried without sending, correlation uncertainty fails closed, and an unknown Feishu send acknowledgement is persisted as uncertain and is never retried blindly.
