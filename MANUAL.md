...
1. Add and test the Linux x86_64 devbox under `Settings → Connections`. In this lane the devbox is only the Relay Center, not a Host scope. The Host currently exposes only the real Mac's local TW-managed scope and sessions. This explicit self-hosted lane additionally requires Node.js 22.16 or newer and an ext-family filesystem; the general Dashboard/CLI requirement remains Node.js 20+.
...
The local Host must use the one canonical terminal-control authority. Before the first self-hosted migration, explicitly update and restart the Dashboard terminal-control owner so the bundled daemon serves both `terminal-control-v1.sock` and its `.relay-v2-exact-*` sibling. Do not create an alternate socket/state or a second daemon for the same sessions.
...
`Link group` loads every page of group chats visible to the selected application bot and offers them in a picker; Chat ID and group name remain editable as an explicit fallback. Chat loading and new links are disabled while the Bridge reports that its `lark-cli` event consumer is starting or reconnecting, so a process that cannot receive group messages is not shown as ready. The link dialog also shows an editable, non-sensitive session summary for lifecycle and inherited-task completion cards, plus a **Reply placement** choice. **Topic reply** is the default and keeps each answer in the source message topic; **Direct reply** still replies to the source message but places the card in the group's main message flow. This choice only changes reply placement: Card JSON 2.0 rendering and reactions stay the same, and a failure never falls back to the other placement. No administrator Open ID is required. A normally writable `FREE` Dashboard terminal does not need a preparatory keystroke: Link Group acquires the lazy interactive lease and submits the Feishu handoff under the same PTY lock. A real Feishu owner or `RECOVERY_REQUIRED` state still fails closed. Once the exact binding and lease are durable, the bot sends a top-level Card JSON 2.0 status card with the managed session type, tmux session name, exact lifecycle ID, and public summary. After linking, any human member of that group can @ the bot to start or steer the bound Agent. The first authorized message creates the one marker-correlated reply turn; later authorized messages while that turn is active are injected through the same fenced terminal writer as steering updates, get a best-effort idempotent acknowledgement, and do not create competing turns. The final answer remains a reply to the first source message using the binding's selected placement. Bot/self and non-user events are ignored, ordinary messages without the exact bot mention are not injected, and only the current turn's explicitly marked public reply is sent back as a non-streaming Card JSON 2.0 card. Reply cards use `tw agent on <session>` as their title; inherited-task completion cards use the persisted public summary when available, falling back to the session name. Titles are normalized to one line and shortened when necessary, while the full prompt and terminal output never enter the title. A turn times out only after ten consecutive minutes with no new fenced terminal output: every new byte from the same target, epoch, generation, and fence slides that inactivity deadline, so a long-running Agent is not cut off by a fixed ten-minute wall clock. An accepted steering update also restarts that inactivity window but never rewinds the output cursor or replays earlier input. Dashboard's `agent running` hint does not authorize delivery; the final answer card is still sent only after the exact closed turn marker is present in the correlated rendered snapshot. Before the first Lark call, the Bridge persists the exact bounded reply payload and deterministic idempotency key. A restart resumes a still-prepared payload with the same key; if authority or delivery cannot be established, Agent content is not guessed or blindly resent and the group receives a separate recovery card containing no Agent output. The bot adds a `Typing` reaction after terminal input is accepted, removes it when processing settles, and replaces it with `CrossMark` only for a certain failure. Reaction write failures are a visual degradation only and never change terminal ownership or the reply result; the bot profile needs `im:message.reactions:write_only` for those indicators. An empty picker means the selected application bot reported no group memberships, rather than silently hiding the lookup. `Settings → Integrations` shows the selected bot's current `session → group` bindings and their active, paused, pausing, or stale state, lets you change Reply placement when no turn is active, and provides a confirmed **Unlink** action even when the original terminal is no longer present. The Terminal view requires the same explicit confirmation. Existing bindings created before this setting default to Topic reply.

Manual unlink releases a live Feishu lease, or first proves that Feishu no longer owns the exact target, before durably removing the binding and queuing a top-level reason card. Current Dashboard and CLI callers identify their actual local entry point in that audit card; an older or unknown caller is described only as a local management client, not falsely as a user-confirmed action. A stale binding with no recoverable lease remains visible when authority is still Feishu-owned or `RECOVERY_REQUIRED`; recover it locally first, then unlink. The Bridge also reconciles every persisted binding, including paused and stale bindings, against its exact `controlTargetId` at startup and during maintenance. A certainly deleted target is removed and announced as deleted; a same-name replacement is removed only after the old exact target is proven ended, and is never selected automatically. Controller unavailability, `RECOVERY_REQUIRED`, and a new ID caused only by unknown controller continuity remain fail closed without a false group announcement. Lifecycle cards use an independent ordered, best-effort lane with deterministic idempotency, so slow delivery cannot block lease renewal and a send failure cannot roll back binding state.

Dashboard probes `bridge.info` before using newer binding fields. It replaces an empty legacy daemon with the bundled same-version Bridge. If a legacy daemon still has bindings, Dashboard keeps normal inspection/removal and controlled takeover available but rejects new links or returns to Feishu unless the daemon provides structured completion tracking, steering, event-consumer health, and durable reply payloads, and never sends optional fields to an older closed parser. A confirmed stale/pausing removal can gracefully migrate that daemon first only when it is the daemon's sole binding and no active turn would be disrupted; ambiguous remove acknowledgement is checked by snapshot and never blindly retried. As a one-time rolling-upgrade exception, removing an active/paused binding through the already-running legacy daemon cannot emit the newer lifecycle card: Dashboard preserves that daemon's canonical removal path instead of interrupting live authority, and a later current-capability operation upgrades it once empty.

If Claude or Codex is already running when a group binding acquires or regains Feishu ownership, the Bridge records a fenced result watch tied to the exact structured Agent session and turn boundary. With no group input, two stopped observations at least one second apart let terminal-control read the Agent-owned JSONL transcript and return only the final assistant message; the bot sends it exactly once as a top-level green reply card. If an authorized group message arrives while that watch is probing, armed, or stop-candidate, the Bridge submits it as a canonical start-or-steer operation and, only after accepted input, converts the inherited task into the normal marker-correlated Feishu reply turn instead of also sending an inherited completion card. A watch already sending its completion card briefly rejects new input until that outbound disposition settles. Kimi does not currently expose the structured transcript source required for an automatic inherited-task completion card, but the first authorized group message still uses the same start-or-steer input path, so an already-running Kimi receives it as steering rather than requiring an idle restart. Terminal rendering, tool calls/results, the input composer, and footer text are never eligible card content. A result that is not yet durable is retried without sending, correlation uncertainty fails closed, and an unknown Feishu send acknowledgement is persisted as uncertain and is never retried blindly; a separate content-free recovery card tells the group that local inspection is required.
...
- Self-hosted Host exits while the existing `terminal-control-v1.sock` still responds: the incumbent Dashboard daemon may predate the exact sibling ingress. Upgrade and explicitly restart that Dashboard terminal-control owner, then retry. The hidden child intentionally fails closed and will not replace, stop, or run a parallel authority beside the incumbent.
