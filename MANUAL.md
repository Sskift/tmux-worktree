...
`Link group` loads every page of group chats visible to the selected application bot and offers them in a picker; Chat ID and group name remain editable as an explicit fallback. Chat loading and new links are disabled while the Bridge reports that its `lark-cli` event consumer is starting or reconnecting. The dialog keeps the non-sensitive session summary used by lifecycle notices and offers **Reply placement**. **Topic reply** is the default and places both the CoT message and its separate final answer in the source-message topic. **Direct reply** places both in the group's main message flow. Delivery never falls back between the two modes. Existing bindings created before this setting default to Topic reply.

No administrator Open ID is required. A normally writable `FREE` Dashboard terminal does not need a preparatory keystroke: Link Group acquires the lazy interactive lease and submits the Feishu handoff under the same PTY lock. A real Feishu owner or `RECOVERY_REQUIRED` state still fails closed. Once the exact binding and lease are durable, the bot sends a top-level Card JSON 2.0 lifecycle notice with the managed session type, tmux session name, exact lifecycle ID, and public summary. After linking, any human member of that group can @ the bot to start or steer the bound Agent. Bot/self and non-user events are ignored, and ordinary messages without the exact bot mention are not injected.

The first authorized message creates one marker-correlated reply turn. Later authorized messages while that turn is active are submitted through the same fenced terminal writer as steering updates, receive a best-effort idempotent acknowledgement, and do not create competing turns. Only after the current turn's closed marker is proven in the correlated rendered snapshot does the Bridge create a Feishu CoT message, write bounded AG-UI run/reasoning status events, send the explicitly marked public answer as a separate Markdown reply, and finish the CoT with `RUN_FINISHED`. The CoT status never contains raw terminal output, tool logs, the prompt, or private model reasoning. Feishu's CoT UI requires PC client 7.70+ or mobile client 7.74+; older clients are outside the supported display path.

A turn times out only after ten consecutive minutes with no new fenced terminal output. Each new byte from the same target, epoch, generation, and fence, and each accepted steering operation, restarts that inactivity window without rewinding the cursor or replaying input. Dashboard's `agent running` hint does not authorize delivery. Before the first Lark call, the Bridge persists the bounded final-answer payload, placement, terminal result, and deterministic key. A still-`prepared` attempt may resume because it has not crossed the non-idempotent CoT-create boundary; once CoT creation begins, the attempt is persisted as `sending`, and a restart or unknown acknowledgement becomes `uncertain` and is never replayed. The separate final Markdown reply uses the deterministic key. If authority or delivery cannot be established, Agent content is not guessed or blindly resent and the group receives a separate Card JSON 2.0 recovery notice containing no Agent output.

The bot adds a `Typing` reaction after terminal input is accepted, removes it when processing settles, and replaces it with `CrossMark` only for a certain failure. Reaction write failures are a visual degradation only and never change terminal ownership or the reply result; the bot profile needs `im:message.reactions:write_only` for those indicators. An empty picker means the selected application bot reported no group memberships. `Settings → Integrations` shows the selected bot's current `session → group` bindings and their active, paused, pausing, or stale state, lets you change Reply placement when no turn is active, and provides a confirmed **Unlink** action even when the original terminal is no longer present. The Terminal view requires the same explicit confirmation.
...
Dashboard probes `bridge.info` before using newer binding fields. It replaces an empty legacy daemon with the bundled same-version Bridge. If a legacy daemon still has bindings, Dashboard keeps normal inspection/removal and controlled takeover available but rejects new links or returns to Feishu unless the daemon provides structured completion tracking, steering, event-consumer health, durable reply payloads, and CoT result delivery, and never sends optional fields to an older closed parser. A confirmed stale/pausing removal can gracefully migrate that daemon first only when it is the daemon's sole binding and no active turn would be disrupted; ambiguous remove acknowledgement is checked by snapshot and never blindly retried. As a one-time rolling-upgrade exception, removing an active/paused binding through the already-running legacy daemon cannot emit the newer lifecycle card: Dashboard preserves that daemon's canonical removal path instead of interrupting live authority, and a later current-capability operation upgrades it once empty.

If Claude or Codex is already running when a group binding acquires or regains Feishu ownership, the Bridge records a fenced result watch tied to the exact structured Agent session and turn boundary. With no group input, two stopped observations at least one second apart let terminal-control read only the final assistant message from the Agent-owned JSONL transcript; the bot then emits a top-level CoT status message and a separate Markdown final answer exactly once. If an authorized group message arrives while that watch is probing, armed, or stop-candidate, the Bridge submits it as a canonical start-or-steer operation and, only after accepted input, converts the inherited task into the normal marker-correlated Feishu reply turn instead of also sending an inherited completion result. A watch already sending its CoT result briefly rejects new input until that outbound disposition settles.

Kimi does not currently expose the structured transcript source required for automatic inherited-task completion, but the first authorized group message still uses the same start-or-steer input path, so an already-running Kimi receives it as steering rather than requiring an idle restart. Terminal rendering, tool calls/results, the input composer, footer text, and private reasoning are never eligible result content. A result that is not yet durable is retried without sending; correlation uncertainty fails closed; and an unknown Feishu acknowledgement is persisted as uncertain and is never retried blindly. A separate content-free recovery card tells the group that local inspection is required.
