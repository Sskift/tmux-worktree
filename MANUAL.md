...
`Link group` loads every page of group chats visible to the selected application bot and offers them in a picker. Choosing a group fills its name and, when Feishu returns one, the group owner Open ID used for the initial administrator policy; the fields remain editable for explicit fallback. An empty picker means the selected application bot reported no group memberships, rather than silently hiding the lookup. `Settings → Integrations` also shows the selected bot's current `session → group` bindings and their active, paused, pausing, or stale state.
