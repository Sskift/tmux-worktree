...
10. **重构阶段保持分支隔离，验收后再合并。** Dashboard replacement 与 Android 分别在对应工作分支完成、push 和验收后已 fast-forward 合入 `master`；Phase 7 在同步最新 `master` 的同名 Dashboard 分支上单独提交和复验，不直接在主分支开发。
...
14. **`tw` 是 headless control plane，不是 Dashboard 的 TUI 复制品。** 二进制负责 managed worktree/terminal 生命周期、Host/SSH、automation、relay、capability negotiation 和稳定 JSON RPC；Files、Editor、Git graph、theme、layout、Pinned 等只属于 Dashboard。
15. **Agent 和 Dashboard 共享 mutation contract。** 新建/恢复 worktree、创建 standalone terminal 都在目标主机执行 `tw rpc`；Dashboard/Relay 不再保留第二套创建器。`tw host` 提供 Host CRUD、独立 SSH/tmux/TW probe、ControlMaster 和远端 RPC/attach。
16. **默认路径以远端可移植性为准。** 新 worktree 默认 `~/.tmux-worktree/worktrees`，显式 `~/...` 在目标主机展开；发现和 orphan scan 继续兼容历史 `/private/tmp/tmux-worktree/projects`。
...
- Node 侧 `src/rpc.ts` 不是常驻 IPC/daemon，而是 `tw rpc` 的一次性 stdout CLI；当前稳定 subcommand 为 `list`、`capabilities`、`create-worktree`、`create-terminal`、`restore-worktree`、`kill-session`。`managed-state` 只是 capability 名，不是独立可调用 subcommand；RPC 负责 runtime lifecycle，不替代 Tauri 的 Editor、Git、File、PTY command 层。
...
- CLI 与 Dashboard 对 `~/.tmux-worktree.json` 的 mutation 使用同一个跨进程 lock-dir contract，避免同时编辑 Host/Relay/Project 时发生 lost update。

### 10.2 Headless control plane（Phase 7）

- `tw rpc capabilities/list/create-worktree/create-terminal/restore-worktree/kill-session` 是 Agent、Dashboard 和 Relay 共用的目标主机 runtime contract。
- `tw host ls/add/update/rm/probe/connect/connection-status/disconnect/rpc/attach` 是本机 SSH control surface；JSON 模式不弹 prompt。
- Host probe 分开报告 SSH reachable、tmux available 和 TW RPC compatible；兼容性看 protocol/capability，不要求远端 semver 与 Dashboard 完全相等。
- Dashboard 的 Host 安装动作优先复制同版本 bundled CLI；资源不可用时固定当前版本 tag，不拉取漂移的 `master`。
- `.tw-dashboard-terminals.json` 继续只保存 label、排序、Dashboard id 和恢复偏好；不把 UI state 混入 managed state。
- 人类使用的 `tw <ai> ...` 自动 attach 和 `tw` TTY 向导有意保留；非 TTY 的无参数调用只显示帮助，避免 Agent/SSH 卡住。

### 10.3 Agent adapter（后续阶段）
...
当前状态：Dashboard replacement 基线 `85bde13` 的 release Tauri bundle 已完成真实数据 smoke，能读取既有 worktree、terminal 和 2/2 Host，Files 常驻、CodeMirror 文件打开、Git status/diff panel 和 tmux 主视图均可达。该历史 bundle 不作为 Phase 7 新 RPC 的 packaged smoke 证据；Phase 7 正式发布前需重新构建。原生 smoke 发现 Git status/diff/fetch discovery 曾在同步 Tauri command 中阻塞 UI，现已统一隔离到 `spawn_blocking` 并加回归门禁。当前仓库仍没有 Electron target；本轮也不引入 Electron migration。
...
- Compose Android V2 UI、Room/Outbox/terminal hardening、安全 Relay v1 pairing 与 bounded FIFO/backpressure 已从 `tmux-worktree-apk-re-5f44d` fast-forward 合入；Relay v2 contract 文档随后进入 `master@c356a73`。
- 旧 Java UI/layout/assets 已删除；仅保留一个升级周期所需的 21 行 `MainActivity` compatibility shim。
- Phase 6 合并时 Dashboard/Android/CLI/Rust 全套门禁通过，已交付提交与当时远端 `master` 核对一致。
...
### Phase 7 — Binary / Dashboard 职责收口与 SSH control-plane 基线（已完成）

已实现：

- canonical worktree root 与目标主机 `~` 展开；旧 macOS 默认目录仍可发现和恢复。
- `tw rpc capabilities/list/create-worktree/create-terminal/restore-worktree/kill-session` 成为 Agent、Dashboard 和 Relay 共用的目标主机 runtime contract；create/state 失败 fail closed 并执行有证据的回滚。
- managed state 与 CLI/Dashboard config 使用 owner-token lock、严格 mutation read、随机 exclusive temp + atomic rename，避免并发 Agent 或 stale owner 丢记录；交互初始化也在锁内以 create-if-absent 发布，不覆盖期间新建的 Host/Relay 配置。
- `tw host` 提供 Host catalog、三层 probe、ControlMaster lifecycle、远端 RPC 和 attach；CLI、Rust、renderer 与 Relay 复用同一 ControlPath/keepalive 语义。
- Dashboard 本地/远端 terminal、orphan restore、managed close 和 Relay terminal 生命周期全部委托 TW RPC；关闭一律 canonical-first，只有明确 non-managed/旧 RPC 才允许 direct tmux compatibility，state 损坏和 transport/protocol 错误 fail closed。
- managed kill 使用旧 record 身份做条件删除；同名 session 在 kill 与 state cleanup 之间重建时不会被旧请求删掉新状态。
- `.tw-dashboard-terminals.json` 只作 UI metadata：Node/Rust 共用 owner-token lock 和 atomic write，远端 key 保留 scope，坏 item 隔离，只有 live tmux session 才会发布，kill 后 best-effort 清理。
- Host probe 独立显示 SSH、tmux 与完整 lifecycle RPC compatibility；`local` 是保留 scope，Host ID/target/user/identity schema 在 CLI、Rust、Relay 的加载与执行边界统一校验，SSH/SCP option boundary 已加固。
- 远端 TW 安装优先复制 bundled same-version CLI，资源不可用时才固定当前版本 tag；本地 installed fallback 同时校验 version 与完整 lifecycle capabilities。
- managed RPC discovery 与严格 legacy tmux/git shape discovery 合并去重；继续识别历史 `/private/tmp/tmux-worktree/projects`。

有意不统一：

- Dashboard 的 Files、Editor、Git graph、theme、layout、Pinned、terminal label/order 不进入 binary。
- Dashboard 的 legacy discovery 与 `tw ls` 的 live tmux 视图可以不同；它们是兼容层，不是 mutation contract。
- `tw <ai> ...` 为人类保留创建后 attach；Agent 使用不 attach 的 RPC；无参数非 TTY 只输出帮助。

完成门禁：root CLI 73/73、Dashboard renderer 334/334 + build/typecheck、Rust 81/81 + fmt、Android JVM 59/59、`git diff --check` 全绿；CLI help、README、MANUAL、ARCHITECTURE 与本计划同步；残余 `tmux new-session` 仅属于 canonical core、明确 legacy ensure、Relay broker/mirror transport 或测试。

### Phase 8 — SSH 长连接稳定性（下一阶段）

目标：在不切 Electron、不提前实现 Relay v2 的前提下，继续把本地 Dashboard 到 SSH Host 的发现、attach、文件、Git 和 terminal 长连接做成可诊断、可恢复、不会重复副作用的稳定链路。Phase 7 已完成安全 argv、ControlMaster、keepalive 和 capability 基线；本阶段聚焦统一状态机、取消、故障注入与 soak。
...
Phase 8 完成后再按 [Relay v2 contract](docs/relay-v2-contract.md) 拆 broker identity/carrier、host ledger/snapshot、Android persistence 和 terminal resume 四个可独立验收的实现 slice；任一 slice 未通过 contract §10 前不得宣告 V2。
...
- 合并动作需要用户明确授权；Dashboard replacement/Android 已完成合并，Phase 7 也已取得用户“合入 master”的明确授权，仍须在最终门禁与远端 HEAD 核对后执行。
