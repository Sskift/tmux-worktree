...
> 状态：Phase 2.1（可用编辑器 + Git 拓扑）已完成，Phase 3 本机 dogfood 继续
>
> 日期：2026-07-11
>
> 实施分支：`tmux-worktree-app-re-ddf7c`
>
> 当前实现基线：`8fd148e`（Files 常驻 + CodeMirror 编辑器 + Git Graph 收口）
>
> 当前安装态：本轮 Phase 2.1 release Tauri bundle 已 ad-hoc 签名、替换 `/Applications/tw-dashboard.app`，并通过 bundle、CLI resource、process 和 on-screen window smoke test
>
...
├── WorkspaceHeader.tsx
├── WorkspaceView.tsx
├── Inspector.tsx            # 当前是 Git-only panel shell，后续可重命名
├── CommandPalette.tsx
├── Settings/
│   ├── SettingsDialog.tsx
│   ├── GeneralSettings.tsx
...
- 当前依赖已安装并形成可重复验证基线；root CLI 29 项、app 316 项和 Rust 63 项测试通过。
...
- `app` 335/335 tests、Rust 68/68 tests、root CLI 29/29 tests 通过。
...
当前状态：已开始。本轮包含 CodeMirror 编辑器与 Git Graph 的 release Tauri bundle 已本地 ad-hoc 签名、覆盖 `/Applications/tw-dashboard.app` 并从正式路径启动；签名、7 个 bundled CLI resources、常驻 process 和 on-screen window 已通过 smoke test。旧安装 bundle 暂存于 `/tmp/tw-dashboard-backup-20260711-142127.app` 供短期回滚。这只是本机 dogfood，不是版本发布、DMG 发布或主分支合并。当前仓库仍没有 Electron target。
...
本机覆盖 `/Applications/tw-dashboard.app` 仅用于 dogfood，不视为完成 Phase 6。
...
- 当前 upstream 已验证为 `origin/tmux-worktree-app-re-ddf7c`；阶段提交继续显式 push 到同名远端分支
- 全程禁止 `HEAD:master`
- 禁止直接 push `master`
