...
### Phase 7 — Binary / Dashboard 职责收口与 SSH control-plane 基线（合并候选）
...
- managed state 与 CLI/Dashboard config 使用 owner-token lock、严格 mutation read、随机 exclusive temp + atomic rename，避免并发 Agent 或 stale owner 丢记录。
...
- Dashboard 本地/远端 terminal、orphan restore、managed close 和 Relay terminal 生命周期全部委托 TW RPC；新 metadata 带 canonical cwd / `managed=true`，只有历史记录允许 direct tmux compatibility path。
- Host probe 独立显示 SSH、tmux 与 TW RPC compatibility；Host 输入在保存和每次执行前校验，SSH/SCP option boundary 已加固。
...
完成门禁：root CLI、Dashboard renderer、Rust 全量测试；CLI help、README、MANUAL、ARCHITECTURE 与本计划同步；残余 `tmux new-session` 仅属于 canonical core、明确 legacy ensure、Relay broker/mirror transport 或测试。
