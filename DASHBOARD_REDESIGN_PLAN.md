...
```text
┌────────────────────────────────────────────────────────────────────────┐
│ merged titlebar: workspace / status / Git / Scratch                    │
├───────────────┬──────────────────────────────────┬─────────────────────┤
│ Sidebar       │ Workspace                        │ Inspector           │
│               │                                  │                     │
│ Pinned        │ Terminal / editor / secondary     │ Files               │
│ Worktrees     │ management page                   │ Git                 │
...
7. 单一 `Settings` footer：同时显示本地、SSH Host、Mobile Relay 汇总状态；不再并列两个都进入 Settings 的按钮。
...
窗口最上方 titlebar 承担唯一 workspace header 的职责；正常 session 不再额外渲染 WorkspaceHeader + Terminal pane bar。首版显式动作只保留：

- Git：直接打开右栏 Git tab
- Scratch：打开真实占位 split；与 Inspector 互斥，避免挤压文件/终端
...
- Automation 不再作为 Inspector tab；统一进入 Sidebar `Manage` 的二级管理页。
...
| Layout | Scratch resize、Inspector 内部关闭 | 恢复默认布局、启动恢复行为 |
...
```json
{
  ...
  "workspace": { "mode": "terminal", "scratchWidth": 380, "scratchCollapsed": true },
