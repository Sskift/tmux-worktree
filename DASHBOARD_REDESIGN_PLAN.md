...
> 状态：用户冻结的 Dashboard replacement scope 已完成，Phase 6 合并交付进行中；Phase 4/5 保留为后续能力
>
> 日期：2026-07-11
>
> 实施分支：`tmux-worktree-app-re-ddf7c`
>
> 当前实现基线：`tmux-worktree-app-re-ddf7c` 合并候选（Files 常驻 + CodeMirror 编辑器 + Git Graph + CLI/RPC/runtime parity + 旧 Dashboard 清理）
>
> 当前验收态：release Tauri bundle 已 ad-hoc 签名并从当前分支启动，能读取真实 worktree、terminal 和 Host；Files 常驻、编辑器、Git panel 与 terminal 主流程通过原生 smoke。合并门禁为 root CLI 33/33、app 332/332、Rust 70/70。
>
...
### Phase 6 — 主分支合并（进行中）与正式 Release（后续独立执行）

本轮进入条件：

- 用户已明确冻结以 Phase 1–2.1、CLI/RPC/runtime parity 和旧 Dashboard 清理为本轮 scope；Phase 4/5 不属于本次合并阻塞项。
...
2. 解决冲突并重跑验收。
3. 按本轮已获得的用户授权 fast-forward 合并到 `master`。
4. 合并后再做版本、DMG 和发布流程。

主分支合并不等于正式版本发布；版本号、tag、DMG、npm 和 GitHub Release 仍需单独执行 release 流程。
