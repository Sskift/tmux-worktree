...
Feishu Bridge 拥有 Lark event 消费、群 binding、群成员精确 @Bot 触发策略、event dedup、单个 awaiting turn、该 turn 的 steering input、`[[notify-group]]` 提取、CoT/最终答复位置和相关审计。生命周期与恢复通知继续使用 Card JSON 2.0。每个 binding 持久化 `replyMode=topic|direct`：默认 `topic` 让 CoT 和单独 Markdown 最终答复进入第一条源消息话题，`direct` 让两者留在群主消息流；两种位置不得互相 fallback，旧 binding 缺字段时归一化为 `topic`。该配置由 Bridge 的串行 mutation 修改，active turn 存在时拒绝修改。

一个 active turn 后续收到的授权群消息不得创建第二个 turn：Bridge 先持久化 event dedup，再以由 `turnId + eventId` 派生的 operationId 调用同一个 `input.agent-message` authority operation，复用原 marker、output generation 和 cursor，并向 steering 源消息尝试发送幂等的 best-effort 状态确认。确认接受会滑动无输出期限但不能跳过旧 output；写入 disposition 不确定会把原 turn 和 binding 置为 recovery，绝不自动重放。Dashboard 的正常绑定把空 `allowedSenderIds` 解释为同群任意真实用户，不要求管理员 Open ID；显式非空列表只用于兼容 CLI。Bot/self、非用户事件和未精确 @Bot 的消息仍被过滤。`Typing` 与确定失败时的 `CrossMark` 是 best-effort reaction，不得改变 lease、turn 或 outbound reply 的确定性。

回复第一次调用 Lark 前必须在同一个原子状态提交中持久化已经过 authority 证明、清洗和限长的完整最终答复载荷、目的位置、最终 turn 状态与确定性键。未跨越外部边界的 `prepared` attempt 可以续投；CoT 创建接口没有幂等键，因此创建前必须持久化为 `sending`，Bridge 重启或未知响应一律转为 uncertain，不能再次创建 CoT。Markdown 最终答复单独使用确定性幂等键。CoT 只写入有限的 AG-UI 处理状态和 `RUN_FINISHED`，不得承载 raw terminal output、工具日志、用户 prompt 或模型私有推理；官方要求的最终答案必须单独发送。支持的 CoT 展示客户端为 PC 7.70+、移动端 7.74+。旧记录缺少载荷或出站结果不确定时不盲发 Agent 内容，而是单独排队不含 Agent 输出的恢复状态卡。Bridge 必须通过 terminal-control plane 取得 lease 和发送输入，不能保存或裁决全局 lease，也不能调用 Relay transport。
...
- Feishu awaiting turn 和 outbound reply attempt 必须持久记录。新 prepared reply 必须携带 exact bounded final-answer payload、目的位置、最终 turn 状态和确定性键；只有尚未开始外部调用的 prepared attempt 可以在 daemon restart 后续投。CoT create 没有幂等键，调用前必须落盘为 `sending`，任何 restart/unknown response 都进入 uncertain/recovery 且不得重放；单独 Markdown 最终答复继续使用确定性幂等键。恢复状态卡不含 Agent 输出且使用独立幂等键。只做 inbound event dedup 不够。
...
| CoT/最终答复已发送但 ACK 未知 | 不误报完成、不自动 transfer、不重复创建 CoT、不盲发 Agent 内容；primary attempt 进入 uncertain，另发不含 Agent 输出的 recovery card |
| Bridge 在 prepared/sending reply 后重启 | `prepared` 的 exact bounded payload 可续投；`sending` 因已跨越无幂等 CoT create 边界而转 uncertain，绝不重放；旧 prepared 记录缺 payload 则 fail closed |
