...
- 在 target 消失、状态连续性不明或 owner 冲突时 fail closed。
...
| --- | --- | --- | --- | --- |
| `InputOwnershipLease` | local terminal-control plane | 一个 `controlTargetId` | 哪个产品实例当前可写 backend | Relay frame 已送达、群 turn 已回复 |
...
- relay-host 重启/SUPERSEDED 可以 reset Relay stream，但不得替换 Feishu/Dashboard owner。
...
Owner ID 必须标识具体 producer instance，不能使用会把多个进程合并的粗粒度字符串：
...
owner kind 可以用于非敏感 UI，但 lease authority 依赖完整 instance identity。Relay public wire不得暴露 Feishu chatId、Lark credential、群成员或 controller内部 leaseId/fence。

## 6. Input owner 状态机

```text
FREE
  `-- acquire(owner) ----------------------> HELD(owner, leaseId, fence)
...
- 任何旧 leaseId/fence 的 Dashboard PTY write、Relay input、command execution或 Feishu callback都在 backend write/群 post 前被拒绝。
- controller在 transfer commit 或 ownership release 时轮换 output generation；旧 cursor后来读到 `[[notify-group]]` 也只能得到 stale-cursor拒绝，不得生成群回复。
- 已排队但尚未进入 controller critical section 的旧 owner operation全部失败。
...
`send_agent_message` 的规范化正文和可选 Enter 必须作为一个不可被其他 producer插入的 controller operation。Relay v2 commandId/ledger负责网络重试和副作用收敛；controller lease/fence负责跨产品排他；二者都不能替代对方。
...
3. 只对缺失的新command检查target ownership。若被Feishu/其他owner持有，使用既有 `PERMISSION_DENIED`，`retryable=false`、`commandDisposition=not_accepted`，不创建ledger。
...
- ownership拒绝使用 `terminal.input_error` 携带既有 `PERMISSION_DENIED`、`retryable=false`、`commandDisposition=not_applicable`；不得写backend或发送该input的ACK。
...
- `terminal_input`/resize在stream write前被controller拒绝，relay-host发送现有带streamId的v1 `error`；不得缓存、重开stream后重放或写backend，output仍可继续。
...
| Relay v2 | open/output/replay可用；send/input/resize/kill拒绝 | v2 adapter取得lease后才写 | detached stream不自动保留input owner |
