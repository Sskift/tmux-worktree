...
当前源码包含未接 runtime 的 materialized foundation 与持久 pinned `state.snapshot` spool foundation：scope/Session mapping、capacity reservation、revision/eventSeq 与 welcome subscriber barrier 落在 H0；spool 从同一 critical-section cut 构建 immutable chunks/manifest，并独立处理跨进程 owner fence、quota、lease、release tombstone 与 fail-closed recovery。完整 rebuild/non-reuse runtime、真实 discovery/negative-evidence adapter、`relay-host`/wire/G2 composition 和 capability advertisement 仍未实现，因此不能宣告 H2 或任何 required capability；当前产品仍只运行 Relay v1。
