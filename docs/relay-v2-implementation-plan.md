...
- credential authority 的 enrollment create/redeem、client/host refresh、host reauthentication 与 host bootstrap API，以及严格的 B1 `POST /v2/hosts/bootstrap` ingress adapter，均已在隔离模块和专项测试中可调用。B1 只做 host-bootstrap admission、bounded read、closed decode/mapping并调用 T1 窄 port；它不构造 authority/native store/server，尚未接 production listener/router/composition/E0，因此这些流程没有外部生产 HTTP 可达路径，不产生 ready/capability，也不能宣告已经交付。
