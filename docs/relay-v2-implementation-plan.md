...
- canonical structured mutation process adapter 已实现 command executor 的 `process` port：每次 execute 经 config foundation queryPort 的 live target generation 重解析 local/SSH target authority，严格 `tw rpc-v2` argv、byte/timeout 上限与 exact outcome，retired generation fail closed；但仍无 production 构造/注入 callsite，H1 lifetime acceptance policy 与 create target 的 target-side catalog observation/revision/fence ABI 仍未交付。
