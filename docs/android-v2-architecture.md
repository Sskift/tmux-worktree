...
| 能力 | Relay v1 | Android UI V2 当前处理 | Relay v2 目标 |
|---|---:|---|---|
...
Android UI V2 不会把 Relay v1 没有的数据伪造成完整功能。Agent 状态和通知必须在 Relay v2 协议落地后再启用。
...
- Android JVM 测试、Debug/Release Lint、Debug APK 和 unsigned Release 构成无设备门禁；连接真实模拟器或设备的仪器测试是独立设备门禁。统一入口分别是根目录的 `npm run verify:android` 和 `npm run verify:device`，不在文档中固定测试数量。
- unsigned Release 只用于源码构建验证，不是已签名的生产发布物。Stage A 只表示 Relay v1 兼容客户端的功能边界，不代表生产签名、TLS 基础设施、后台通知或 Relay v2 已完成。

### 阶段 B：Relay v2（规划中，未实现）

[`relay-v2-contract.md`](relay-v2-contract.md) 已冻结 Android、broker 与 relay-host 的首个 v2 合同，包括权威边界、command ledger、snapshot cursor、terminal resume、错误语义和双栈迁移。该文档不是实现完成的声明；所有运行时代码仍是 Relay v1，v2 协议代码仍待实现。
