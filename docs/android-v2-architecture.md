...
以下代码不是无用 legacy，当前源码合入及首个正式 2.x 迁移版本必须保留：
...
- 已通过 54 项 Android JVM 测试、API 36 模拟器设备测试、Debug/Release Lint（0 error）、Debug APK 与 unsigned Release 构建；根项目、Dashboard 和 Rust 测试保持通过。unsigned Release 仅用于源码构建验证，不是已签名的生产发布物。
