...
Android 还包含未接线的纯 Relay v2 Outbox authority core foundation。它绑定 profile/principal/host lineage、dedupe window、command、operation、opaque Scope/Session 与 canonical request fingerprint，输出不可变 state、原子 mutation plan 和 typed effect。独立 Room repository 是这些事实的唯一持久写入口：同一 transaction 内 load、strict decode/restore、pure reduce、按 plan 写 meta/entries，commit 后调用者才可处理 effect；`AtomicReissue` 的原 entry replacement、新 entry 与 creation cursor 因而原子提交。未接线的 `command.statuses` recovery bridge 显式消费 actor-owned apply lease port，在 lease 内按完整 namespace/host epoch/command identity 将整批状态预归约，只有单事务提交成功后才同时返回 exact receipt 与 typed effects；stale generation、缺项、identity/dedupe 不匹配、reducer rejection 或存储失败都不产生 receipt/effect。损坏或未知 Outbox namespace fail closed，不会当作空队列自动重发。该路径仍未接入 actor effect consumer、`V2ViewModel`、UI、network 或 composition root，因此不改变 APK 当前只运行 Relay v1 的事实。
