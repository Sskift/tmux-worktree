...
Android 还包含一个未接线、Android-free 的 Relay v2 terminal checkpoint/reducer core。其版本化 timeline identity 包含 `profileId`/activation generation、`principalId`、`clientInstanceId`、完整 host lineage、Scope/Session、stream/generation、open/close ID 与 resume-token credential reference；socket/effect delivery token 单独持久并只允许显式单调 rebind。每个 network action、parser callback 与 effect 都带当前 delivery fence，parser callback 还绑定完整 timeline identity、`parserContinuityId` 和精确 byte start/end。网络连续接收与 parser 实际应用使用不同 watermark；只有后者推进累计 output ACK。pending parser queue 同时受 512 KiB raw-byte 和 128 item 上限约束，control/replay/closed/reset 不因 output queue 饱和而静默丢失。input/resize 从 1 编号并与累计 ACK 分离；generation 改变时未 ACK input 进入 `AMBIGUOUS`，旧 pending resize 直接丢弃并以新 open cols/rows 建立尺寸基线。该 core 只产生 immutable typed result/effect，不拥有 Room、`TerminalWebView`、xterm、Relay actor、socket、network 或 composition；因此不表示 APK terminal v2 已完成。
