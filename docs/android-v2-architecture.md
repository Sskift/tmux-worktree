...
Production root 现只把这个 port 接到一个受限的 Session reply producer。`RelayV2BaseRuntimeComposition` 从 actor 取得 exact current ONLINE context/lease，再以同一 hostEpoch、dedupe window、activation namespace 和 Room committed opaque Session cut 构造唯一 `send_agent_message`；arguments 固定为 `pane=0`、`submit=true`。Session cut 的 Room read、构造、reply read→enqueue 及所有 base-sync/recovery materialized-state mutation共用 composition 内唯一 product mutation owner；projection 写入前还必须在 actor `lifecycleLock` 内以原 ONLINE cut完成 final publication recheck，因此 ONLINE→RESYNCING、disconnect、close 或 terminal fence 后的旧 read不能复活 UI。`V2ViewModel` 不直接读取 v2 Room、不观察 ONLINE 来拼 authority，也不创建 sender；UI 使用不可碰撞的长度前缀本地 Session identity，wire Scope/Session identity仍只随 opaque cut进入 composition。Room commit 后只有当前 admission、captured composition 与 exact cut仍成立且 raw draft未变化时才清 draft；晚到旧成功/拒绝均不能覆盖新 draft或 `actionError`。随后只唤起既有 fresh producer/dispatcher。若 commit 后 generation/profile fence 已 stale，durable row留给 successor 的既有 fresh/query flow。not-online、profile barrier、foreign/stale cut、capacity、corrupt 与 store failure均 typed fail closed且保留 draft。
