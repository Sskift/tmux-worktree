...
Production root 现只把这个 port 接到三个受限 producer：existing-Session reply、`kill_session`，以及只面向已有至少一个 materialized Session 的 UI 可见 Scope 的 `create_worktree`。`RelayV2BaseRuntimeComposition` 是三条写路径的唯一 owner。reply/kill 消费 issuing composition 的 opaque exact Session cut；create 只接受 composition 从当前 materialized Session exact cut 派生、同 Scope 共享的 opaque `RelayV2ScopeCreateCut`，该 cut 精确表达 activation namespace、`scopesRevision` 与 Scope authority，UI raw scopeId 仅用于 `(hostId, scopeId)` cut lookup。三者都先取得 actor current ONLINE lease，再进入唯一 product mutation owner，重验 current projection exact cut，并重读 Room materialized Session cut：reply/kill 逐字段核对 exact Session，create 则再次确认该 Scope 仍有 Session 且 namespace、hostEpoch、`scopesRevision` 与 Scope resource 全部 exact。reply 固定构造 `send_agent_message`（`pane=0`、`submit=true`），kill 固定构造 `RelayV2OutboxArguments.KillSession`，create 固定构造 canonical `RelayV2OutboxArguments.CreateWorktree` 并 enqueue 到 `sessionId=null` 的 exact create lane。Room commit 返回 receipt 后才调用既有 fresh producer/dispatcher；若 commit 后 generation/profile fence 已 stale，durable row 留给 successor 的既有 fresh/query flow。create queued 不等于 created：UI 只退出 creating 并提示 queued，不本地插入 Session、不导航，等待权威 `sessions.changed` upsert。空 Scope 不支持该路径；未来必须由独立 durable Scope cut 建立 authority，不能伪造。
...
2. 表单在本地完成必填校验。
3. Relay v1 命令携带 requestId、hostId 和请求上下文；已 admission Relay v2 的 Worktree create 只用 UI 选择查找 composition-issued opaque Scope cut，并提交 durable commandId create lane。
4. Relay v1 保持既有创建完成后的缓存与导航行为。Relay v2 create 的 enqueue 成功只退出加载态并提示 queued，不写入或导航到伪 Session；只有后续权威 `sessions.changed` upsert 才展示新 Session。失败、断线和超时都会退出加载态并给出原因。
...
| 新建 Worktree/Terminal | 支持 | v1 保持 requestId + 超时；v2 仅已有 Session 的可见 Scope 支持 Worktree durable queued | commandId、ledger 与结果查询 | 不涉及 |
