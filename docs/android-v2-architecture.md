...
selected-Session live collector 现会 combine composition-scoped Outbox 与 Agent revision：任一 revision 变化都会在同一 exact cut 下重读，而 status admission 对每个 cut 仍只调用一次。reply read 仍只接收 Outbox revision；发布 fence 同时复核 exact composition identity、dialect、opaque Session identity及两个当前revision。`agentTimelineRevision` 只在 primary Agent durable consume 已提交并返回 eligible `Applied`/`ExtensionFault` 后、notification claim或`RequestSync`等 post-commit 动作前递增一次；`DUPLICATE`、`EXTENSION_NOT_ACTIVE`、Unavailable、未协商、未形成durable consume结果的异常、stale apply以及recovery/status prepare/load-init均不递增。两个revision都只是 replay-latest live invalidation，不是durable truth，不进入Room、DAO、repository或actor。live Agent revision refresh已接通，但capability advertisement仍unowned/default-off。

显式 v2 runtime phase 同步投影为 `Relay v2 transport` health 与 synthetic host status，使 Workspace、Session detail 和 diagnostics 不再显示 Relay v1/UNKNOWN。其中 ONLINE只表示 v2 transport在线且基础 state cut已同步，diagnostics明确标记 capability readiness未广告，不宣告完整 ready/capability。

Relay v2 Outbox 的 existing core + Room repository 提供 internal durable enqueue port：caller 必须提交已经稳定的 exact activation namespace 与 `RelayV2OutboxDraft`。repository 在单个 transaction 内 strict decode/restore current state、执行 `RelayV2OutboxAction.Enqueue`、验证 activation namespace 与新 row 的 exact host lineage并持久化唯一 mutation plan 的 meta/entry；authority namespace 不含 host selector，同一 activation 可持有分别绑定不同 `hostId/expectedHostEpoch` 的 rows。只有 commit 完成后才返回不含 arguments、Scope/Session 或 state 的非敏感 row correlation。duplicate、capacity、foreign activation、corrupt/unknown state 与 store failure均 typed fail closed且零 receipt，既有数据不会被清空或覆盖。

Production root 现把这个 port 接到四个受限 producer：existing-Session reply、`kill_session`，以及每个当前 materialized、UI 可见 Scope（包括空 Scope）的 `create_worktree` / `create_terminal`。`RelayV2BaseRuntimeComposition` 是四条写路径的唯一 owner。reply/kill 消费 issuing composition 的 opaque exact Session cut；两类 create 只接受 composition 从当前 committed Scope projection 签发的 opaque exact Scope cut，该 cut 精确表达 activation/state namespace、`scopesRevision` 与 Scope authority，UI raw scopeId 仅用于 `(hostId, scopeId)` cut lookup。四条路径都先取得 actor current ONLINE lease，再进入唯一 product mutation owner，重验 current projection exact cut，并重读 Room materialized authority：reply/kill 逐字段核对 exact Session，两类 create 则再次确认 namespace、hostEpoch、`scopesRevision` 与 Scope resource 全部 exact。reply 固定构造 `send_agent_message`（`pane=0`、`submit=true`），kill 固定构造 `RelayV2OutboxArguments.KillSession`，两类 create 分别构造 canonical `RelayV2OutboxArguments.CreateWorktree` / `CreateTerminal` 并 enqueue 到各自 `sessionId=null` 的 exact create lane。Room commit 返回 receipt 后才调用既有 fresh producer/dispatcher；若 commit 后 generation/profile fence 已 stale，durable row 留给 successor 的既有 fresh/query flow。create queued 不等于 created：UI 只退出 creating 并提示 queued，不本地插入 Session、不导航，等待权威 `sessions.changed` upsert。

`V2ViewModel` 只有在 startup admission 明确为 `RELAY_V2` 时才从 current composition 取得该 exact cut并提交 kill；Compose 不读取 v2 Room、不观察 ONLINE 来拼 authority，也不创建 sender。reply 的 late callback 只有在 admission、captured composition、exact cut 与 raw draft仍 current 时才可清 draft；kill 的 late callback同样复核 admission、composition 与 cut，queued 成功不乐观删除 Session。Session 只会在权威 `sessions.changed` delete（或包含该删除的权威 materialized state）提交并重新投影后消失。ONLINE→RESYNCING、disconnect、profile/generation 前移、foreign/stale cut、corrupt state、capacity 与 store failure均 fail closed；queued 只表示 durable command 已提交，不表示远端已经终止，也不新增 ready/capability。

同一个 composition 还形成彼此分域的 selected-Session projection/request seam。Agent evidence 继续使用 actor 的 current repository read authority、materialized Session exact-cut read、同一 Agent durable repository以及现有 selection/read/presentation controllers；Agent status admission复用同一个 selection controller、现有 durable load-or-initialize adapter和 dormant runtime composition 内唯一 `RequestSync` coordinator；本机 reply 则只通过既有 activation Outbox port按完整 activation namespace strict restore，再以当前 opaque Session cut 的 `hostId+hostEpoch+scopeId+sessionId` 和 `send_agent_message` 精确过滤。reply projection 只返回 `commandId/message/createdAtMillis/state`，隐藏 `REISSUED` parent，保留 exact replacement child，并按 durable creation order取最近最多 256 行。composition 的 replay-latest `outboxTimelineRevision` 只在真实 durable enqueue、query admission、status/result/statuses recovery或fresh `QUEUED→SENDING` transaction commit 后递增；读前后同时复核 exact Session cut与expected revision，durable read failure返回`Unavailable`，期间变化返回`Stale`。

`V2ViewModel.timeline(sessionId)` 是两个 presentation 到 typed UI state 的唯一 owner，返回 `SessionTimelineState(events, agentEvidenceAvailability)`：demo 固定为 `AVAILABLE`；Relay v1 保留本机出站 timeline 并标为 `RELAY_V1_UNSUPPORTED`；Relay v2 在当前 cut 的 `transformLatest` 内先对每个新 exact cut调用一次 status admission，再开始收集上述 revision；每次 revision 只读取一次最新 durable snapshot，不轮询且不重复请求status，cut/profile/composition替换会取消旧 collector，read后还要通过current fence才可发布。Agent `Content` 仍按原规则决定 `AVAILABLE`；Agent `Disabled`/`Unavailable`/`Stale` 时，本机 reply rows仍作为`USER` event保留，同时 availability保持`RELAY_V2_UNAVAILABLE`。本机状态只逐项映射`QUEUED/SENDING/ACCEPTED/CONFIRMING/SUCCEEDED/FAILED_FINAL/AMBIGUOUS`，event identity以完整本地Session identity与commandId做长度前缀typed编码；本slice不读取或处理transcript correlation，也不合成其他状态。两个source固定分组为本机reply rows在前、Agent events在后；每组保持各自authority发布的原顺序，不按`createdAt`或其他本地字段建立跨authority wall-clock顺序。Compose 只渲染这个合并状态：v1 沿用明确的 Relay v1 limitation，v2 unavailable 使用独立且不声称 v1 的 evidence notice，`AVAILABLE` 不显示 unsupported notice；reply composer输入不由 evidence availability控制，但发送仍走独立 command path并要求当前 Session online。transcript entry保持既有run/turn-scoped本地timeline投影；结构化 lifecycle event按原run/turn scope投影为`SYSTEM` evidence，delivery为空，状态、发生时间以及失败code/summary只来自该event，`isCurrentSource=false`会明确标为历史source evidence。lifecycle evidence不提升为 Session-wide `AgentState` 或 summary，`agentStateAvailable`仍只表示 Session-wide aggregate，也不从terminal、文本或本地timer推断。production actor 的 optional capabilities仍固定为空，因此当前 APK的 status admission会在 actor current repository read cut前返回`Unavailable`，零 Agent Room read/初始化/写入、零网络发送，也不产生notification platform call或replay/snapshot动作；只有显式 client/broker/host 三方协商后，才会为该 exact cut执行一次exact load-or-initialize并通过同一 `RequestSync` 最多发送一条`agent.timeline.status.get`。真实Android notification platform owner虽然已由production root注入，但同样被空capability挡住；这不影响已经durable committed的本机reply timeline，也不表示通知产品能力已经交付。这个 closed projection/request seam不表示真实 Agent reply/state/capability已交付；除四个冻结 operation 外的 extension command UI仍未接线；terminal已进入admitted base runtime，Agent runtime/notification owner已装配但capability advertisement固定为空，也没有 v1 fallback。

本文中的 **V2** 指第二代 Android 产品界面和客户端架构，即 Compose UI、`V2Activity` 与 `V2ViewModel`；它不代表 Relay v2。Dashboard、`relay-server` 和 `relay-host` 的默认 CLI/shipping 路径仍只实现 Relay v1；显式default-off v2 roots不因此成为shipping或capability。Android production root 另有一个严格受限的 Relay v2 base-sync 路径：只有 cold-start credential/admission 已验证并直接携带的显式 v2 profile 才能进入；这不表示完整 Relay v2、ready 或 capability 已交付。Android已有独立Relay v2 strict codec，以及使用v2 profile/credential、fakeable transport/clock、串行socket ownership、双代际fence、握手watchdog、`host.presence`/`hosts.snapshot` gap恢复和显式query/resync/reject effect的`RelayV2ConnectionActor` seam；其默认 action queue 为 64 个普通槽、8 个保留控制槽和 1 MiB raw UTF-8 byte budget，effect queue 为 32 项和 1 MiB raw UTF-8 byte budget。Android production root 对上述已验证 profile 复用同一个 `tw_mobile_relay_v2_state.db`/repository 实例执行基础 state-sync、exact activation Outbox query admission、recovered command status 的 post-commit Execute ordering，受限 Session reply、existing-Session kill 与每个 materialized 可见 Scope（包括空 Scope）的 durable `create_worktree` / `create_terminal` enqueue、bounded fresh `QUEUED` durable dispatch，以及只针对 exact retryable connection failure 的单一 bounded exponential reconnect owner；除上述closed selected-Session本机reply、transcript/lifecycle evidence只读projection、existing-Session kill与两类create外，额外extension command UI仍未接线。terminal的Room/journal、exact actor send、raw-byte parser ACK与Compose attachment已由admitted base runtime闭合；Agent runtime和真实notification platform owner也已装配，但空optional capability使其保持disabled。数据库 version 6 通过显式 `MIGRATION_1_2`、`MIGRATION_2_3`、`MIGRATION_3_4`、`MIGRATION_4_5`、`MIGRATION_5_6` 保留原有六张 state-sync 表、version 2 新增的 Outbox/terminal 表、version 4 新增的 Agent lifecycle state/notification claim表，以及version 5加入的独立 row-oriented Agent materialized entry、snapshot staging header/records与 snapshot/gap 期间的 durable LIVE buffer；4→5只创建表、index、constraint，不解析、回填或改写旧payload/row。version 6新增terminal post-commit batches/fences/meta journal；5→6只创建相应表与索引，不解析、回填或改写旧rows。version 3 在 authority row 上加入原子 snapshot release journal，持久保存旧 cut identity、durable cursor、release reason 与 ACK 后动作，partial/未知 journal 读取 fail closed。state-sync 表按 `profileId+principalId+clientInstanceId+hostId+hostEpoch` 隔离 authority、Scope、opaque Session、snapshot metadata/records 和 durable event buffer，支持 welcome required watermark、pinned cut 流式 staging、连续 event 合并及单事务 commit/discard/release-CAS。独立 recovery repository adapter 由 `RelayV2BaseRuntimeComposition` 的 effect pump 使用，只在 actor apply lease 下提交完整 Room mutation，事务提交后才把 exact receipt 交回 actor；UI 的 STOPPED/CONNECTING/RESYNCING/ONLINE/FAILED 只描述该显式 profile 的连接状态，不宣告 Relay v2 readiness 或 capability。Outbox meta/entry 表按 `profileId+profileActivationGeneration+principalId+clientInstanceId` 隔离 authority，entry 另以 `hostId+expectedHostEpoch+commandId` 定位并显式保存稳定 creation order；terminal checkpoint 表以 SendOpen 前已稳定的完整 target（包含客户端生成的 `streamId` 与 pane）为整行主键。三类 versioned canonical payload 都显式保存 UTF-8 byte count 与 SHA-256，读取时依次执行上限、实际 byte count、digest 和 strict roundtrip 校验。该数据库不复用或原地升级 v1 profile/session/Outbox/stream rows，也不保存 access、refresh、enrollment、resume token 值或其他敏感 credential material；terminal 涉及 resume credential 时只保存非敏感 Keystore reference/fingerprint。snapshot 每块最多 256 records/512 KiB canonical bytes，完整 cut 最多 100000 records/256 MiB canonical bytes，staged raw UTF-8 最多 512 MiB；RESYNC event buffer 最多 4096 条/16 MiB。这些是每个 state namespace 的硬上限。
...
private dispatcher 在任何 send callback 前先通过配对 consume port永久消费 capability，再把私有 wire defensive copy和完整 `profileId`、profile activation、principal、`clientInstanceId`、host lineage与 socket generation authority交给 actor-owned exact-generation send port。`RelayV2ConnectionActor` 已实现该 Execute send port：它只在完整 repository/socket authority 仍精确 current 时于 lifecycle fence 临界段内随 `ONLINE` 发布 exact execute-ready cut，任何离开 ONLINE 或 owner/authority 撤销都先在同一个锁内撤该 cut。send 在该锁内核对 cut、完整 authority 与当前 socket owner 后执行唯一一次同步 transport send；输入 bytes 先受公共 frame 上限约束并 defensive copy，actor 不 retain或记录。`QUERYING`、首次 snapshot 与 ONLINE gap 后的 `RESYNCING`、profile 切换、client rebind、disconnect/reconnect、closing、handshake未完成或 socket generation 前移都在调用 transport 前返回 stale；`command.query` 仍由独立 actor sender 执行，不走该 Execute port。stale、transport `send=false` 和抛错均已永久消费：前者返回 `Stale`，后两者返回绑定原 Execute correlation 的 `ConfirmingRequired`，adapter 内不重发；并发、重入或再次提交同一 capability只会有一个调用进入 transport，其余返回 `AlreadyDispatched`。`retryAfterMs` 的等待与 successor attempt仍属于上层 scheduling owner。dispatcher seam 不生成 attempt/request ID、不改变 Outbox state，不决定 retry、`AMBIGUOUS` 或 query redrive，不持有平行 network generation，也没有 v1/profile fallback；它不订阅 flow、不构造 actor/socket。query-send/attempt-registration、recovered status ordering 与 fresh dispatch 已由 production base composition 接通：composition 在 status durable commit 后按 exact generation/query-recovery lineage 有界累积 capability，gap/resync/disconnect/close/profile failure 或 stale handoff 都清空；actor 返回最终 `OnlineReady` 后先 flush recovered，再调用 fresh producer。fresh producer 不观察 `StateFlow.ONLINE`，而是在每个 actor lease 内由 Room transaction 调用 core 的 DispatchEligible 选择；每批 commit 后才签 capability，发送仍必须通过 actor-owned ONLINE exact-generation gate。commit 后发送前崩溃或 partial send failure 都保留 `SENDING`，下次 connection attempt 只 query、不盲发，也不会自动 resend/reissue command。这个受限接线仍不构成 G2/G3、capability advertisement、ready 或完整 production Relay v2。

repository 必须在 actor-owned apply lease 内完成代际 effect 的整个 Room transaction；state `matchesGeneration` 只是非原子的代际预筛，不拥有 phase authority，也不能构成 disconnect/profile-switch barrier。切换时 actor 会先原子禁止旧代际取得新 lease，再等待已经进入的旧事务完成；repository 的 profile cleanup API 只接受该 v2 profile 的 exact disconnect receipt，并在一个 transaction 中删除原有六张 state-sync 表的数据、该 profile 的所有 activation-scoped Outbox/terminal、Agent lifecycle/notification rows，以及所有 activation/timeline 下的 row-oriented Agent transcript/snapshot/LIVE-buffer rows。

Android production profile runtime 已注入 OkHttp HTTPS credential adapter：redeem/refresh 严格使用 HTTPS、精确 path、现有 closed codec schema 和单次 HTTP/1.1 exchange。enrollment QR/Intent 仅进入内存 review，第一次显式确认仅 redeem 并保存 credential，独立第二次 Connect 动作重新执行 startup admission 与 exact-profile 比较后才启动 base runtime，不会因 enrollment 或 confirm 自动建立 socket，也不回退 Relay v1。bounded RFC6455 WSS adapter 现是 base composition 的唯一 socket 入口，以系统 trust 和 RFC2818 hostname verification 建立单次 HTTP/1.1 Upgrade，在读取或分配 payload 前执行 1 MiB frame/fragment 累计限界，并拒绝 extension negotiation。actor/profile startup admission、Room 基础 state-sync repository、Outbox query/recovered-status ordering、受限 Session reply、existing-Session kill 与每个 materialized 可见 Scope（包括空 Scope）的 durable `create_worktree` / `create_terminal` producer、bounded fresh durable dispatch、bounded WSS 与 retryable-failure reconnect 已通过 base composition 接入 `AppContainer`/`V2ViewModel`。

reconnect 复用同一 actor，只由一个 capped exponential timer 驱动；socket open 不重置 attempt，只有 exact `OnlineReady` 重置。每次 successor 等待旧 transport/apply fence并重新读取 Room Outbox，snapshot 通过 opaque attempt identity 绑定 actor 选出的 exact generation；close、parent cancellation、disconnect/profile-switch barrier 与 terminal failure 均先同步 fence pending retry。exact failed-attempt admission detach 与 timer claim 在同一个 retry fence 临界段完成：并发 disconnect 要么随后取消已安装 timer，要么先取得 fence 使该 failure 只作为 stale 返回，不能把 barrier terminalize。同一个 tracked connection-attempt Job 覆盖 backoff、Room snapshot read 与 actor admission；disconnect 在返回 receipt 前 cancel/join exact Job，close cancel 它，read failure 只有在 Job 与 retry fence 仍 exact current 时才能映射为 RuntimeIncomplete。`actor.connect` accepted 后，wrapper Job ownership 会在同一个 connection fence 临界段内、任何 immediate transport failure effect 可 claim successor 之前 relinquish；旧 wrapper completion 不能清 successor，期间也不允许第二个 owned attempt。retry fence 与 connection admission retirement 保持分离：disconnect/close 先同步禁止 timer/successor，exact 当前 admission 保留到 actor 已 seal 且 apply drain 完成后才 retire，因此已取得 lease 的 effect 不会在 profile switch 中被误判为 missing admission。owned backoff 在 base state 与 `V2ViewModel` 中保持 `CONNECTING`，同一失败 generation 的迟到 actor `FAILED` 被 fence；non-retryable 与 RuntimeIncomplete 仍为 terminal `FAILED`。当前仍没有真实设备或公网Relay TLS互操作证据；`auth.expiring` exact rollover已接入。本地 admitted-v2 用户驱动的 self-revoke quarantine/confirmed cleanup saga 已闭合但仍default-off；没有真实TLS/device/qualification/signing/release证据，因而不构成production availability或GO。额外extension command UI与capability advertisement仍未接线；terminal已进入admitted base runtime，Agent/notification owners已装配但空capability使其disabled。它们与 Node codec 共同消费冻结 fixture 或按冻结契约建模，但当前连接状态不表示 runtime ready、capability advertisement 或完整生产可用。

显式 **Retry now** 复用上述唯一 connection owner：它在同一 `connectionLock` 内撤销当前 backoff fence、取消 timer 并认领 fresh attempt；若 composition 已进入 terminal failure，typed 结果要求 `V2ViewModel` 精确 drain/close 旧 owner、清空同一 UI slot 与 opaque cuts，再从当前 cold-start admission 返回的同一 profile 创建 fresh composition。任一 drain、re-admission、profile identity 或安装失败都 fail closed，且不进入 Relay v1。抽屉 **Refresh sessions** 与两个 create 页的 **Retry load targets** 则调用同一 `V2ViewModel.refresh()`：在 admitted v2 下，它只进入 actor 的单一 bounded manual-resync admission，绑定 exact profile、connection generation 与 current transport，同时发送冻结的 `hosts.snapshot.get` 和 full `state.snapshot.get`，后续继续复用 recovery adapter 与 Room snapshot staging/commit；UI 不直接写 Room。Relay v1 的既有 refresh 行为不变。
...
2. 表单在本地完成必填校验。
3. Relay v1 命令携带 requestId、hostId 和请求上下文；已 admission Relay v2 的 Worktree/Terminal create 只用 UI 选择查找 composition-issued opaque exact Scope cut，并分别提交 durable commandId create lane，包括当前为空的 materialized Scope。
4. Relay v1 保持既有创建完成后的缓存与导航行为。Relay v2 两类 create 的 enqueue 成功都只退出加载态并提示 queued，不写入或导航到伪 Session；只有后续权威 `sessions.changed` upsert 才展示新 Session。失败、断线和超时都会退出加载态并给出原因。
...
| 新建 Worktree/Terminal | 支持 | v1 保持 requestId + 超时；v2 对每个 materialized 可见 Scope（包括空 Scope）支持两类 durable queued create | commandId、ledger 与结果查询 | 不涉及 |
| 手机发送消息 | 仅支持发送与 ACK | 持久 Outbox；真实时间线只展示本机出站消息 | 服务端去重、结果查询与明确不确定状态 | 不涉及 |
| Agent 入站 transcript | 不支持 | 不伪造；只映射结构化selected-Session evidence，并以 typed availability 区分 v1 unsupported 与 v2 unavailable | 不属于冻结基础 slice | 结构化 entry、cursor、重放与去重 |
| 终端流 | 支持 | stream generation + 打开 watchdog + 同配置/网络恢复后自动重开 | generation、offset、ring 与 resume | 不涉及 |
| Waiting/Failed/Completed | 不支持 | 只映射结构化 lifecycle evidence；不可用时清空 events 并显示对应 availability notice | 不属于冻结基础 slice | 结构化 turn/run lifecycle |
...
- **明确限制**：Relay v1运行路径没有Relay v2的幂等执行、远端命令结果查询、Agent入站时间线、Agent状态事件、通知事件或terminal offset续传。显式admitted v2 client现已实现durable terminal offset/resume/reset，但仍缺端到端Host capability与真实设备互操作证据。这些能力不能由 Android 本地状态可靠推断。

## 当前交付与实施协调

Agent primary durable presentation commit现会推进`RelayV2BaseRuntimeComposition`内存revision并驱动selected-Session live reread；该revision不替代既有Agent Room durable truth。default optional capability advertisement仍为空，因此这条接线不启用Agent extension、不产生capability，也不改变真实Agent runtime/notification仍未交付的结论。

当前源码已实现 Compose 信息架构、Room/DataStore/Keystore 持久层、Relay v1 actor、每会话串行 Outbox、配对切换 barrier、分维快照、终端恢复、二维码 review、WSS 校验，以及 Relay v2 codec conformance、独立 actor/profile seam、profile owner 内的 closed startup admission、Room state-sync repository、纯 Outbox authority及其 Room 持久 owner、入站 command evidence seam、durable command-query 准入屏障、post-commit exact-generation 出站 adapter seam及 actor-owned send port、OkHttp HTTPS credential adapter、bounded RFC6455 WSS adapter、纯 terminal checkpoint/reducer及其 Room 持久 owner。其中只有已存在且 admission/credential 验证成功的显式 v2 profile 会进入 production `RelayV2BaseRuntimeComposition`，复用同一 credential store 和 state repository，以 bounded WSS 执行基础 hello/query-or-resync、snapshot/release/expiry、scopes/sessions state-sync 和 recovered command status dispatch ordering。可选 Agent extension 的独立 public codec artifact、lifecycle/notification reducer、typed durable operations/Room repository、row-oriented Room v5 materialization、durable notification claim与revision-pinned read projection已在隔离模块和现有 `RelayV2StateDatabase`/DAO owner 内部接通，并已有 JVM 与定向 emulator 证据。新增的 default-off runtime composition seam只接收上层显式交给它的单个effect：已协商Agent frame复用同一个durable repository实例进入现有consumer、notification dispatch coordinator与read projection，未协商frame在任何durable访问前closed；非Agent effect与Agent unavailable effect都以携带完整原effect的`NotOwned`交给upper-layer dispatcher，只有携带exact failed request/admission identity的unavailable effect才归RequestSync redrive owner。它不自行订阅`RelayV2ConnectionActor.effects`。`dormant(...)`与base composition的窄platform参数默认仍是`NOTIFICATIONS_DISABLED` suppressor；`AppContainer`以`applicationContext` lazy构造单例`AndroidAgentTranscriptLifecycleNotificationPlatform`并注入base composition。构造只取得system service并装配port，不请求权限、不调用`notify`或执行notification intent。`RelayV2ConnectionActor` 的可选 capability 仍只能显式注入且默认为空，只有 host welcome 完成并形成 client/broker/host 三方交集后才会产出extension frame。production `RelayV2BaseRuntimeComposition`由唯一effect pump以actor apply/handoff fence委托dormant runtime composition；只有exact协商的Agent effect在同一apply lease内完成durable consume与post-commit notification claim后，content-free ticket才最多一次交给注入platform；platform call不在Room transaction内，suppressed/failed也不建立retry。空optional capability使构造、非Agent effect、Agent unavailable与当前production hello保持零platform call。同一 composition 已把 selected-Session 本机reply projection、capability-fenced单页structured transcript/lifecycle evidence projection和exact-cut status admission接到`V2ViewModel`，后者只在每个新cut的revision collector启动前请求一次。default optional capability advertisement仍为空，所以production status admission与Agent projection都得到`Unavailable`且零Agent Room mutation/网络发送，但已durable committed的本机reply rows仍可显示；这不表示当前APK能够取得、展示或宣称真实Agent reply、Waiting/Failed/Completed已交付。真实Android platform owner已装配，但空capability挡住production调用，通知产品能力仍未交付。v2 Outbox 的 production query registration、recovered status ordering 与 bounded fresh durable dispatch 已接入唯一 base effect pump；最终 actor `OnlineReady` 后先消费本次 durable status commit 生成的 capability，再按 creation order生产 fresh batch。受限 Session reply producer及其本机Outbox timeline、existing-Session kill producer，以及每个 materialized 可见 Scope（包括空 Scope）的 durable `create_worktree` / `create_terminal` producer 已接线；额外extension command UI、Agent capability advertisement、离线push/background notification路径及真实公网Relay TLS集成或互操作证据仍未交付。terminal的Room/journal、actor、WebView parser与Compose接线已进入admitted base runtime；`auth.expiring` rollover已接入。本地 admitted-v2 用户驱动的 self-revoke quarantine/confirmed cleanup saga 已闭合但仍default-off；没有真实TLS/device/qualification/signing/release证据，因而不构成production availability或GO。连接前 strict读取 exact activation Outbox：当前 lineage queryable state进入 query、终态忽略，同 lineage bounded `QUEUED` 留给 fresh producer且不进入 query，foreign/corrupt/overlimit fail closed；status commit 只允许 zero-effect 或本 owner 的 recovered Execute，其他未拥有post-commit effect都会typed fail closed；terminal effect由同一base composition的terminal owner认领，未协商Agent effect仍保持unavailable，绝不 ACK、丢弃或 fallback 到 v1；`autoConnect=true` 会在 exact retryable connection failure 后由唯一 bounded backoff owner 重连；非 retryable 与 RuntimeIncomplete 保持 terminal，`autoConnect=false` 保持 STOPPED 且不开 socket或调度 timer。不得据此生成 v2 enrollment、宣告 capability、把连接状态描述为 runtime ready、把受限 fresh durable dispatch描述为完整 command runtime、宣告 APK terminal v2 完成或系统通知产品能力已交付。Android JVM、Room migration validation、Lint、APK build、migration device execution 与连接设备验证仍是不同证据；`assembleRelease` 只生成 unsigned 构建验证产物，不代表生产签名、TLS 基础设施、后台通知或渠道发布已经完成。
