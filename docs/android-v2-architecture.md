...
当前源码已实现 Compose 信息架构、Room/DataStore/Keystore 持久层、Relay v1 actor、每会话串行 Outbox、配对切换 barrier、分维快照、终端恢复、二维码 review、WSS 校验，以及尚未接入生产运行路径的 Relay v2 codec conformance、独立 actor/profile seam、Room state-sync repository、纯 Outbox authority core、OkHttp HTTPS credential adapter 和 bounded RFC6455 WSS adapter foundations。AppContainer、V2ViewModel、Android production repository 与 composition root 仍未交付且只装配 v1；v2 Outbox 的 Room/actor/runtime/UI/network 接线、terminal checkpoint 及真实 Room/device/公网 Relay TLS 集成或互操作证据也尚未交付。不得据此生成 v2 enrollment、宣告 capability、自动连接 v2 profile 或描述为 runtime ready。Android JVM、Lint、APK build 与连接设备验证仍是不同证据；`assembleRelease` 只生成 unsigned 构建验证产物，不代表生产签名、TLS 基础设施、后台通知或渠道发布已经完成。
