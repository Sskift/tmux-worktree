...
同一个 composition 还用 actor 的 current repository read authority、materialized Session exact-cut read 和已注入的同一 Agent durable repository，装配现有 selection/read/presentation controllers，形成只读 selected-Session seam。`V2ViewModel.timeline(sessionId)` 只对当前 composition 签发且仍在 cut map 中的 opaque exact cut读取一个固定上限的 structured page：transcript entry保持既有run/turn-scoped本地timeline投影；结构化 lifecycle event按原run/turn scope投影为`SYSTEM` evidence，delivery为空，状态、发生时间以及失败code/summary只来自该event，`isCurrentSource=false`会明确标为历史source evidence。lifecycle evidence不提升为 Session-wide `AgentState`，也不从terminal、文本或本地timer推断。cut 替换、disconnect、profile fence、`Disabled`/`Unavailable`/`Stale`或read后的current fence失败都会使页面投影为空。production actor 的 optional capabilities仍固定为空，因此当前 APK会在 actor capability cut处返回 `Unavailable`，不会进入 materialized Session exact read或Agent Room read，也不会产生网络、通知、namespace初始化、status/replay/snapshot动作。这个 closed projection seam不表示真实 Agent reply/state/capability已交付；除 Session reply 外的 v2 command UI、terminal、Agent capability/runtime与通知仍未接线，也没有 v1 fallback。
...
| Agent 入站 transcript | 不支持 | 不伪造；只映射结构化selected-Session evidence，未协商时为空 | 不属于冻结基础 slice | 结构化 entry、cursor、重放与去重 |
| 终端流 | 支持 | stream generation + 打开 watchdog + 同配置/网络恢复后自动重开 | generation、offset、ring 与 resume | 不涉及 |
| Waiting/Failed/Completed | 不支持 | 只映射结构化 lifecycle evidence；未协商时为空 | 不属于冻结基础 slice | 结构化 turn/run lifecycle |
