...
Production root 现只把这个 port 接到两个受限的 existing-Session producer：reply 与 `kill_session`。`RelayV2BaseRuntimeComposition` 是两条写路径的唯一 owner；它消费 issuing composition 的 opaque exact Session cut，在 actor 的 current ONLINE lease 内以同一 activation namespace、hostEpoch 和 dedupe window 重新读取并逐字段核对 materialized Session，然后才在 product mutation owner 下原子 enqueue。reply 固定构造 `send_agent_message`（`pane=0`、`submit=true`），kill 固定构造无额外 arguments 的 `RelayV2OutboxArguments.KillSession`。Room commit 返回 receipt 后才调用既有 fresh producer/dispatcher；若 commit 后 generation/profile fence 已 stale，durable row 留给 successor 的既有 fresh/query flow。
