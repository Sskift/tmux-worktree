...
当前仓库已有独立、未接线的Node HTTPS adapter foundation，只实现上述exact endpoint/system TLS/header gate/bounded strict outer decode与closed mapping，并直接服从`RelayV2ContinuityAnchor`传入的abort signal，不拥有第二个operation timer。另有独立、未接线的exact revoke/kid removal/expiry/ready-loss live-authorization fence foundation：credential authority只在durable commit后settle单调revision/fence并在首次withdraw同步通知窄port，BrokerCore在attach、auth-control、route和逐帧dispatch复核trusted-time expiry与exact identity fence并发出once-only typed symbolic close signal。仓库仍没有production backend、auth/config source、stable provisioning/ACL、真实 restart/旧备份/灾备 failover证据、production authority injection/composition、transport close policy的production adoption/listener/socket wiring或ready-loss真实证据。future composition只能按 native store open+self-check → external authority injection → credential authority open/reconcile → synchronous admission/connection fence → broker auth-control injection 的顺序建立 ready；独立未接线的 transport close coordinator 已在 broker owner 侧实现 4401/4403/1013 与 5000ms force-destroy policy；future production composition 仍须显式采用、接线和验收，external adapter 不得另行选择。`broker-credential.v1`与`agent-transcript-lifecycle.v1`是独立 external namespace，使用不同 anchorId/ACL/reset/decommission/tombstone history；broker owner binding保持 provisioning提供的不透明稳定 identity，不从尚未冻结的 issuer/tenant credential字段推断。前者fatal failure必须撤credential ready/admission并全局fence基础v2 Upgrade/route/active data；后者精确复用现有extension store的`AGENT_AUTHORITY_STORE_CONTINUITY_UNAVAILABLE|COMMIT_UNCERTAIN|CORRUPT` mapping，只隔离Agent extension。unavailable保留可证明timeline/cache lineage并repair/reopen authority，不能reset/new epoch；只有独立corrupt/明确continuity loss才允许extension reset/new epoch。基础credential/route/command/terminal继续且不得全局close。任一 production wiring或证据缺失时 adapter保持 NO-GO，v2不得 ready、enroll或advertise capability，也不得回退v1/BAU。
