...
Broker credential 本地持久化另有独立的 frozen [native state-store v1 contract](../contracts/relay/v2/broker-credential-state-store-v1/README.md)。它冻结 exact caller-owned-home open、raw N-API closed wrapper、deep port、single descriptor/fixed-offset binary fixture、same-fd lifetime kernel-lock 与 positional two-barrier publication，不修改本文 public wire。当前已有未接线的独立纯 Rust binary/publication core、未接 production composition 的显式 target/N-API/fixed-artifact optional loader，以及通过 opaque port 独占 credential state 与 external continuity 的未接线 authority source foundation；capability/open 仍只由 raw wrapper closed decode。Capability supported 不是 ready；open+self-check 后仍须该 authority 完成 external continuity，并由 future production composition 注入整体链路。Existing unknown/corrupt/unsafe/identity/durability failure preserving-invalid，不能伪装 missing/unsupported。当前仍没有 N-API binary、Darwin/Linux secure-open/lock/真实 durability adapter、host bootstrap/成功 credential 流、production authority injection/composition 和 packaging；Rust core/in-memory authority/loader conformance 也不是 native OS、lock、真实 durability、真实网络或 production continuity 证据。此前被拒绝的 BAU path/JSON 设计未纳入当前交付源码，不得重引入、迁移其 artifact 或作为 fallback。Production v2 仍 disabled，Relay v1 不依赖该 native contract。
