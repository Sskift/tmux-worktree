...
20. [2026-08-06] §10.20 relay-host 裸默认翻转决策记录：`tw relay-host` 无显式 flag 时，若 canonical v2 host profile（`~/.tmux-worktree/relay-v2-host/profile-v1.json`）已 provision 且通过严格校验，默认走 v2；否则回落 v1 并打印一行 provisioning 提示（说明如何 provision 后裸调用自动默认 v2）。依据：(a) 阶段 D 之前 G2（Node↔Node）与 G3（Android↔Node）端到端 interop 已通过，relay-host 作为 broker client 的 v2 路径具备可运行证据；(b) §10.19(a) 自托管单节点 lane 豁免 E0/external-continuity 生产证据，relay-host 本地 v2 激活不需要该 lane 尚未实现的证据即可默认启用；(c) 新增 `--v1` 逃生门，任何情况下可强制回到 v1（与显式 `--profile v2` 冲突时报错），默认翻转不产生不可逆行为变化；(d) 带 v1 专属 relay 参数（`--relay`/`--host-id`/`--secret` 等）、显式 `--profile` 或 `TW_RELAY_HOST_PROFILE` 时保持既有显式选路语义，v1 能力只增不减。relay-server（broker）裸默认仍保持 v1 的原因：broker 单节点自托管 lane 存在 Linux x64 + Node≥22.16 硬门槛（macOS fail closed），多租户 tier 的 E0 证据仍 descoped，故只翻转 host 侧（broker client，可在 Mac 上运行 v2）默认，不翻转 broker 默认。`--v2-local-dev` 及其它既有 relay-server/relay-host flag 行为保持不变。
