...
状态：**Frozen contract — v2.0.0-android-first；codec、受限 Android runtime 与显式 default-off Node compositions 已落地，但端到端互操作和生产可用仍为 NO-GO**

范围：Android、relay-server、relay-host 的首个可互操作 v2 slice。

当前 credential authority 的 enrollment create/redeem、client/host refresh、host reauthentication 与 host bootstrap API、严格的 B1 `POST /v2/hosts/bootstrap` ingress adapter、覆盖 enrollment redeem、client/host refresh 与 self-revoke 四个冻结端点的 B4 credential HTTPS ingress foundation，以及 E1 external continuity authority HTTPS adapter，均已在隔离模块和专项测试中可调用。B1/B4/E1现只由显式default-off activated Broker composition与同一credential authority采用；相邻的显式default-off public lifecycle root接管caller-supplied `node:https.Server`，激活该composition并拥有request/upgrade/listen与close-and-drain，但默认CLI不调用该root。`qualifiedRecords=[]`和缺失qualified E0部署使真实activation在外部可达前fail closed，因此这些流程不产生shipping ready/capability，也不能宣告已经交付。
...
默认 Dashboard shipping 与 `relay-server`/`relay-host` CLI 路径仍只运行 Relay v1；bundled hidden Dashboard management child 与 Tauri supervisor 固定使用 exact protocol v2。hidden child现通过唯一trusted deployment opener在自身进程内独占一个explicit-v2 Host root，并把同一stdio/runtimeVersion/signal经既有`dashboardManagement` seam交给same-lineage credential/controller/carrier composition；它不采用独立relay-host process lifecycle，也不创建第二owner或v1 fallback。但在没有qualified same-lineage Host inputs时仍对所有operation fail closed返回`UNAVAILABLE`。Android 另有只面向已 admission 显式 profile 的受限 v2 base runtime，Node 另有显式 default-off Broker/Host compositions。Android 本地 admitted-v2 用户驱动的 self-revoke quarantine/confirmed cleanup saga 已闭合但仍default-off；没有真实TLS/device/qualification/signing/release证据，因而不构成production availability或GO。Broker 与 Host native `qualifiedRecords=[]`、qualified E0/native/TLS/device/signing/release 证据缺失，且默认 CLI 不启用上述 compositions，因此端到端 v2 仍不可用。任何一端都不得仅凭本文、fixture、codec 或隔离 composition 存在而发布 v2 capability、生成可用 v2 配对二维码或把 v1 credential 当作 v2 credential。

### 非 production deployment policy：single-node co-located SQLite v1

本文增加一个不修改 public wire、closed schema、错误表或六项 required capabilities 的最小 deployment-policy 扩展。只有显式 `relay-server --v2-single-node-self-hosted` 可选择 literal policy `non-production-single-node-co-located-sqlite-v1`；production profile、default CLI、`qualifiedRecords` 与 E0 contract 不得选择或推断该 policy。

该 policy 将 canonical Broker credential authority 的 opaque state-store port、monotonic continuity port与 issuer keyring source 绑定到同一 owner-only SQLite deployment owner。它 MUST 只运行在 Linux x86_64、Node >=22.16、ext-family filesystem 和单 Broker 进程；MUST 要求显式 listen host/port、独立严格 HTTPS advertised root及匹配该 hostname 的 TLS identity；MUST 使用现存 canonical current-user-owned exact-0700 dedicated directory、exact-0600 single-link database、`synchronous=FULL` 与 exclusive locking；MUST 持久化并在重启后复用 issuer keyring/kid。continuity CAS MUST 只接受 sequence 0 genesis，或 `next.sequence=current.sequence+1` 且 `next.parentCommitId=current.commitId` 的后继；不匹配必须零写入 fail closed。

数据库 metadata MUST 绑定 machine-id digest和 directory device/inode；identity、schema、endpoint、mode、owner、link、filesystem 或 exclusive-lock mismatch 均须在 listener 发布前 fail closed。部署方 MUST NOT 复制 state directory、恢复旧数据库/快照、在同一目录替换数据库、导入状态、并行启动、failover 或把该目录用于 backup/DR；实现不得提供 override、migration、JSON/file fallback 或 stale-PID lock repair。SIGINT/SIGTERM/SIGHUP MUST 在 activation 前建立 startup/bootstrap fence，并在 handle 发布后通过同一 root 的 close-and-drain lifecycle 收敛。

该 continuity 与 credential state 位于同一 rollback domain，明确 **不是 E0**，也不满足本契约其余位置要求的 rollback-independent production continuity。它不得生成、伪造或修改 production qualification/E0 evidence，不得放宽 Host hello 的冻结 required capabilities，不得在失败时回退 v1/BAU。它只能证明受限单节点、禁止恢复策略下的进程重启 credential continuity，不构成 HA、灾备、production readiness/capability、G3 或 GO。

Broker credential 本地持久化另有独立的 frozen [native state-store contract revision 2](../contracts/relay/v2/broker-credential-state-store-v1/README.md)；N-API interface、capability storage、binary/header、private-location derivation与fixture仍各自为 v1，artifact、ABI和bytes未变化。Revision 2冻结 exact native account-home/mode/ACL secure-open、qualification-before-registry/mutation、final A/B/C descriptor proof、traditional process-owned `F_SETLK`、shared registry/PID/fork/final-close语义，以及 deny-by-default durability qualification；`qualifiedRecords=[]`且首条 record必须新增 contract revision，因此所有真实 open目前必须在 registry/mutation前 `DURABILITY_UNSUPPORTED`。它仍冻结 raw N-API closed wrapper、deep port、single descriptor/fixed-offset binary fixture与 positional two-barrier publication，不修改本文 public wire。当前已有未接线的独立纯 Rust binary/publication core、单点消费 manifest-derived container spec并拥有显式eager process token、pre-fd registry、PID/descriptor fence、panic-aware poison、common-owned exactly-once final close、private N1 bridge与opaque wrappers的platform-common lifecycle owner、Darwin/Linux platform adapter foundations、target-selected raw N-API binding foundation、显式 target/N-API/fixed-artifact optional loader，以及通过 opaque port 独占 credential state 与 external continuity 的authority source foundation。Platform-common本身不接`trustedHome`、path、syscall、filesystem/durability或N-API；adapter只通过其public lifecycle seam提供target open，binding在module init eager exactly-once捕获process token并只把`ProcessBound*`与target open seam转换为冻结raw ABI。空qualification allowlist使platform-adapter-qualified real open和actual opened JavaScript transaction均不可达。现有显式、未接线的本机target build/stage/npm-pack verification foundation共用loader的四项fixed descriptor，按exact Rust triple做locked release build，验证Mach-O/ELF header与digest/空layout后以final hard-link作为唯一不rollback的commit point，并用有界pure-ustar inspector拒绝PAX/GNU longname/link/special entry、只解出exact regular allowlist，再从自建`npm pack --ignore-scripts`临时目录导入packed fixed loader并运行packed JS/native closed binding test。Node path API没有`openat/linkat/unlinkat`，所以该foundation不宣称抵抗恶意同uid最终目录换名竞态。当前实际证据仍仅Darwin arm64；本轮没有Darwin x64/Linux、四target matrix或bit reproducibility，也没有Dashboard bundle、签名/notarization、minimum-OS/glibc/SDK、provenance、qualified real open或actual opened JavaScript transaction证据。Capability supported 不是 ready；open+self-check 后仍须 authority 完成 external continuity，并由 future production composition 注入整体链路。Existing unknown/corrupt/unsafe/identity/durability failure preserving-invalid，不能伪装 missing/unsupported。当前已有未接线的 T1 `role=host` bootstrap authority source foundation，fresh host grant/credential 仅在 authority 专项层可达；显式default-off broker production composition与HTTP adoption已存在，但默认CLI不注入；qualified E0 backend/deployment、Dashboard/production shipping、真实跨端交付与continuity readiness仍未交付；现有Rust/native/in-memory/loader evidence也不是qualified real open、真实filesystem/kernel lock/fork/power-loss、真实网络或production continuity证据。此前被拒绝的BAU path/JSON设计未纳入当前交付源码，不得重引入、迁移其artifact或作为fallback。Production v2仍disabled，Relay v1不依赖该native contract。
...
当前仓库已有由显式default-off Broker composition采用的Node HTTPS adapter，只实现上述exact endpoint/system TLS/header gate/bounded strict outer decode与closed mapping，并直接服从`RelayV2ContinuityAnchor`传入的abort signal，不拥有第二个operation timer。另有由同一activated composition采用的exact revoke/kid removal/expiry/ready-loss live-authorization fence：credential authority只在durable commit后settle单调revision/fence并在首次withdraw同步通知窄port，BrokerCore在attach、auth-control、route和逐帧dispatch复核trusted-time expiry与exact identity fence并发出once-only typed symbolic close signal。仓库仍没有qualified production backend与auth/config deployment、stable provisioning/ACL、真实restart/旧备份/灾备failover、真实public TLS/network/device/ready-loss证据；显式composition与authority injection代码存在，但`qualifiedRecords=[]`使真实activation不可达。显式default-off composition按native store open+self-check → external authority injection → credential authority open/reconcile → synchronous admission/connection fence → broker auth-control injection的顺序尝试建立owner，但不得据此宣告ready；独立未接线的 transport close coordinator 已在 broker owner 侧实现 4401/4403/1013 与 5000ms force-destroy policy；activated composition已显式采用该owner，public lifecycle root也已接管caller-supplied `node:https.Server`的listen/HTTP/WSS/drain；但qualified real public TLS/network/device/ready-loss仍须验收，external adapter不得另行选择。`broker-credential.v1`与`agent-transcript-lifecycle.v1`是独立 external namespace，使用不同 anchorId/ACL/reset/decommission/tombstone history；broker owner binding保持 provisioning提供的不透明稳定 identity，不从尚未冻结的 issuer/tenant credential字段推断。前者fatal failure必须撤credential ready/admission并全局fence基础v2 Upgrade/route/active data；后者精确复用现有extension store的`AGENT_AUTHORITY_STORE_CONTINUITY_UNAVAILABLE|COMMIT_UNCERTAIN|CORRUPT` mapping，只隔离Agent extension。unavailable保留可证明timeline/cache lineage并repair/reopen authority，不能reset/new epoch；只有独立corrupt/明确continuity loss才允许extension reset/new epoch。基础credential/route/command/terminal继续且不得全局close。任一 production wiring或证据缺失时 adapter保持 NO-GO，v2不得 ready、enroll或advertise capability，也不得回退v1/BAU。

Node 侧另有以下已落地、但仍 default-off 或未接生产路径的实现事实：Broker Host/client WSS ingress 已在 socket 层执行本文冻结的 4408 handshake deadline（host Upgrade 后 host.hello 5 秒、relay.welcome 后 client.hello 5 秒、client.hello 后 host.welcome 10 秒），一次性 token-scoped timer 只在对应注册/握手信号到达时取消，scheduler 失败 fail closed 为 1013；production Host welcome serializer 已实现本文 §2.2 冻结 resume matrix 与 EVENT_CURSOR_AHEAD 映射，但 canonical composition 只接受其为注入 port，production 路径尚无构造 callsite；canonical structured mutation process adapter 与 v2 config foundation 的 structuredProcess lane 已按 live target generation 重解析 local/SSH target authority，但仍无 production 构造/注入 callsite；create target 的 target-side catalog observation/revision/fence 现已存在为显式 default-off foundation：独立 versioned companion ABI `contracts/tw-rpc/create-target-observation-v1`（不扩展 frozen TW RPC v2、不允许 capability advertisement 或 fallback）定义 observe/admit 两阶段——target 侧 `tw create-target-observation-v1` entrypoint 原子观察闭集 catalog（config project、canonical realpath、canonical git-dir/common-dir identity digest 与 exact resolved base-ref OID）并签发覆盖全部观察输入的确定性 revision，admit 在同一进程内现场重新观察并对 revision/arguments/闭集 execution 做 canonical exact 比较，观察输入闭集内的漂移（含 OID/identity 漂移）即零副作用 `OBSERVATION_STALE` fail closed，匹配才执行 canonical resolved mutation（no retry/fallback/direct SSH parsing），已证明边界为 observe→admit 间 target 侧 reobserve exact 比较，残余风险仅为进程内 re-observe 与 mutation 之间不可约的本地 TOCTOU（与任何本地 create 同级）；Relay 侧 observation authority/store 保留在独立 authority module，由 admission 模块的 pair issuer 构造唯一实例并封入 record；claim wrapper、admission adapter 与 pair registry 共置 admission 模块，两个组件是模块私有 class，无公开 constructor 或 registry opener；唯一入口 `issueRelayV2CanonicalCreateTargetExecutionPairV1({ owner, runner, inner, ... })` 对所有 options 做一次 exact own-data 捕获（accessor/Proxy/symbol/多余 key 同步 fail closed）后打开 owner 的 same-owner bundle、构造唯一 observation store，并只返回 frozen null-prototype 空 ticket；私有 wrapper 与私有 adapter 延迟到一次性 H1 factory 内构造（base non-create process 在签发时封入），组件不可取得、hybrid 不可表达。Host canonical composition 以唯一 `createTargetExecutionPair` option 接收 ticket，在 hostState.read/H2/任何 lookup 前经 `captureRelayV2CanonicalCreateTargetH1FactoryV1` 同步 exactly-once capture（foreign/复制/replay 在该处 fail closed），所得一次性 lexical closure 被调用时对 input 做一次 exact own-data 捕获、装配 command target resolver/executor 并直接 openRecoveredAuthority，只返回 opaque readiness candidate；evidence 携带 catalogRevision，fence 同步一次性消费，admitted store 以不碰撞 opaque authority token 为唯一键（token 保留 per-实例唯一 digest 输入）、按逐-evidence exact identity（operation/arguments/闭集 execution 与携带 fenced command fingerprint 的 reservationCorrelation）恰一次消费，相同 execution 的并发 evidence 各自独立成行，create_terminal 的 admitted arguments 与 executor mutation 共享同一 exact canonical 形式（canonical cwd + derived label)；无 production shipping activation、无 capability advertisement、无 qualified 真实设备证据；H2 spool 已增加 one-shot fresh-install bootstrap port；Node terminal manager 已修正 durable mode=new open 重试在 stream_lost reset 中的 offset correlation。Android admitted explicit-v2 product composition 另已把 `create_worktree` 与 `create_terminal` 按 composition 签发的 exact online Scope create cut 排入 durable Outbox：离线、stale/foreign cut 与 profile barrier 均 fail closed，queued 不等于 created，必须等待权威 `sessions.changed`。这些事实不改变上述 NO-GO：qualified E0/native durability/public TLS/network/device/signing/release 与 shipping activation 证据仍缺失；HostCarrier 已把每个 accepted `host.welcome` 的 command window 期限绑定到 exact route，并在 accept 截止前 60 秒以 1013 退役 carrier，走既有 retryable reconnect 与新 hello；旧 window 不延长或复活，旧 ledger/query 保留。仍未交付的是 Broker `host.auth_expiring` 的 `refreshRecommendedAtMs` authority/policy。local terminal-control 侧已有 process-local exact read observation owner/ABI：claim 同步单消费后在 canonical lock 内重新 inspect live backend、重验 target record/incarnation proof，再 prepareOutput 并登记 observer，同一锁返回 controlEpoch/outputGeneration/outputCursor；prepare→consume 间同名重建拒绝为 TARGET_GONE。tail 严格沿用 output.tail 语义，不授予 input ownership；observer 活跃时最后 interactive release 回 FREE 但不 reset output generation，stale observer 被 retire 而不抑制后续 reset，observer close 先完成 deferred reset/持久化再注销、失败可重试；并经 remote exact compound 的 `observe`/`tail`/幂等 `close-observe` 帧在同一 canonical child 上贯穿观察与后续 lease acquire/renew/release。default-off canonical Host composition 现已在未注入 terminalBackend 时默认装配 exact-observation byte plane：open 只把 admitted claim 原子消费为只读 observation并从其 outputCursor tail，首个 input 经 lazy lease 在同一 compound channel 重 prepare→fence→consume，release 后回 observing 继续 tail，observation cleanup 经可重试 barrier、失败留存 fenced record 由 adapter.close() 重 drain并上抛；这只是 host-side internal composition 事实，本文 wire/ABI 不变。
...
- replay keyring 恰有一个 active key，最多保留 32 个仍被未到期 record 引用的 decrypt-only key；新 record 只使用 active key，并显式携带 exact replay-key id。新 ciphertext 的 AAD 还必须覆盖该 key id；旧单 key envelope 的 ciphertext 保持原 AAD 和 bytes，不在读取或迁移时重加密。
- rotation 使用持久 rotationId 收敛：同 rotationId 的 ACK-loss 重试返回原 replay-key id且不再轮换；每个新 rotation 只原子产生一个 active key。origin key id 与所有 rotation record 形成最多 1024 个 identity 的永久有界 non-reuse history；达到上限或 decrypt-only keyring 满且 active key仍有引用时必须在 rotation commit 前 fail closed。
- rotation 不修改任何既有 replay expiresAtMs。旧 key 只有在引用它的全部 record 按各自原始 600000 ms TTL 到期并被同一 authority mutation 清理后才能删除；不能通过 replay read 延长 TTL、提前删 key或偷偷 reencrypt。record 引用 missing/unknown key、AEAD authentication失败或 malformed keyring/state 都是 credential authority fatal failure，必须同步 withdraw admission/fail closed，绝不能降格为 replay miss、AUTH_INVALID 或另一协议 fallback。
...
上例中的三字段 resume 对象只适用于 mode=resume；mode=reset 的 resume 若存在，closed schema 仅允许 generation 和 resumeToken。
...
| reset | resume 可省略；若存在则必须且只能包含 generation、resumeToken，禁止 nextOffset | 显式替换 attachment，签发新 generation/token，offset 从 0；旧 offset 不参与 reset |
...
record fingerprint 包含 scopeId、sessionId、pane、cols、rows、mode，以及适用的旧 generation、resumeToken hash；nextOffset 只在 mode=resume 时纳入，mode=reset 不接受也不推导 requestedOffset；禁止持久或记录明文 resumeToken。
...
默认 Dashboard shipping 与 `relay-server`/`relay-host` CLI 路径仍只宣告 Relay v1；bundled hidden Dashboard management child固定使用exact protocol v2，并已把唯一trusted Host root的same-lineage management seam接到stdio，但当前没有qualified Host inputs，仍只fail closed返回`UNAVAILABLE`。Android 已有隔离的显式 v2 profile namespace、hostEpoch、opaque sessionId、durable Outbox/state 与 client terminal resume；Node/Tauri 也已有显式 default-off Broker、Host 与 management compositions。但 qualified authority、原子六项 capability、真实 TLS/device/signing/release 和跨端验收仍缺失，因此当前构建不得宣告 v2 ready/capability，且旧 v1 identity、Outbox 与 shared secret 仍不得原地提升。
...
18. 在以上测试全部通过前，Dashboard、`relay-server` 与 `relay-host` 的默认 shipping 路径继续只宣告和运行 Relay v1；Android admitted explicit-v2 lane 不能提升为 ready/capability，hidden Dashboard management child 也只能 fail closed `UNAVAILABLE`，任何端都不得生成可用 v2 enrollment 或 capability。
