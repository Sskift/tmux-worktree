...
Broker credential 本地持久化另有独立的 frozen [native state-store v1 contract](../contracts/relay/v2/broker-credential-state-store-v1/README.md)。它冻结 exact caller-owned-home open、raw N-API closed wrapper、deep port、single descriptor/fixed-offset binary fixture、same-fd lifetime kernel-lock 与 positional two-barrier publication，不修改本文 public wire。Capability supported 不是 ready；open+self-check 后仍须 future authority injection 完成 external continuity。Existing unknown/corrupt/unsafe/identity/durability failure preserving-invalid，不能伪装 missing/unsupported。当前 N-API、Rust/Darwin/Linux、loader、authority injection 和 packaging 均未实现，in-memory conformance 也不是 native/lock/durability/continuity 证据。保留的 unsafe BAU path/JSON prototype 的 native 安全验收失败且只保留原型；它的 state/lock/temp artifact 不迁移、不删除，不作为 fallback。Production v2 仍 disabled，Relay v1 不依赖该 native contract。
