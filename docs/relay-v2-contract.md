...
Broker credential 本地持久化另有独立的 frozen [native state-store v1 contract](../contracts/relay/v2/broker-credential-state-store-v1/README.md)。它冻结 exact caller-owned-home open、raw N-API closed wrapper、deep port、single descriptor/fixed-offset binary fixture、same-fd lifetime kernel-lock 与 positional two-barrier publication，不修改本文 public wire。当前已有未接线的独立纯 Rust binary/publication core，以及未接 production composition 的显式 target/N-API/fixed-artifact optional loader；capability/open 仍只由 raw wrapper closed decode。Capability supported 不是 ready；open+self-check 后仍须 future authority injection 完成 external continuity。Existing unknown/corrupt/unsafe/identity/durability failure preserving-invalid，不能伪装 missing/unsupported。当前仍没有 N-API binary、Darwin/Linux secure-open/lock/真实 durability adapter、authority injection 和 packaging；in-memory/loader conformance 也不是 native OS、lock、真实 durability 或 continuity 证据。此前被拒绝的 BAU path/JSON 设计未纳入当前交付源码，不得重引入、迁移其 artifact 或作为 fallback。Production v2 仍 disabled，Relay v1 不依赖该 native contract。
