...
Settings also contains a separate, explicit **Relay v2 · self-hosted** deployment surface for a Linux x86_64 devbox with Node.js 22.16 or newer and an ext-family filesystem. It persists the selected SSH devbox, user-entered root HTTPS Relay URL, explicitly entered private IPv4/port, and separate local 0600 TLS key, leaf certificate, and CA certificate paths. The bind address is never prefilled; `0.0.0.0` remains available only as an explicit all-interface opt-in. **Deploy / update bundle** publishes the complete bundled `tw-cli` directory, TLS/profile files, and a dedicated 0700 state directory; it rejects unsafe remote links/types/owners/modes, normalizes staged bundle directories/files to 0700/0600, and streams TLS bytes from an identity-checked `O_NOFOLLOW` descriptor instead of reopening a validated path. **Start/Stop v2 Relay Center** controls only that remote Center and preserves its SQLite credential/continuity/keyring state. The selected devbox is currently only the Broker Center: the self-hosted Host exposes the real Mac's local TW-managed scope and sessions, not any SSH Host scope from the account config. It never reads or promotes the Relay v1 shared token. The one-time Host bootstrap is requested only for the first uncredentialed Host setup and moves through Tauri-owned remote/local 0600 files without entering renderer state, URLs, logs, or secret-valued argv. Before a persisted self-hosted management child starts, Tauri also prepares the contract-fixed local native 0700 directory and private 0600 Host profile/CA/bootstrap inputs without passing the native directory path to Node. The remote preflight rejects an older Node before creating the self-hosted deployment layout or starting the Center. This surface requires a bundled CLI with the explicit `--v2-single-node-self-hosted` Broker seam and the matching Host-management handoff; it does not make Relay v2 production-ready or change the default Relay v1 path.
...
`--bootstrap-secret-mode replace-pending` is an exact private operator-repair selector, not a general bootstrap or capability flag. It is legal only with `--bootstrap-secret-input` on this self-hosted lane and requires the bridge-issued one-shot handoff bound to the exact native cell; production, local development, and direct privileged-intake calls without that handoff fail before reading the candidate or entering the Vault. Repair accepts only the existing version-zero bootstrap-pending envelope whose durable `oldSecretReference` matches this profile's bootstrap reference, preserves its attempt/correlation, and performs at most one token-replacement CAS. A same-source initial read is a zero-CAS no-op; bootstrap-only, ready/version>0, foreign/inconsistent, corrupt, uncertain, or concurrent-conflict state fails closed without deleting the cell, clearing Broker SQLite, extending the five-minute TTL, falling back to Relay v1, or creating readiness/capability/qualification evidence.
