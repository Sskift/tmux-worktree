...
Settings also contains a separate, explicit **Relay v2 · self-hosted** deployment surface for a Linux x86_64 devbox with Node.js 22.16 or newer and an ext-family filesystem. It persists the selected SSH devbox, user-entered root HTTPS Relay URL, explicitly entered private IPv4/port, separate local 0600 TLS key, leaf certificate, and CA certificate paths, plus an independent Host connector desired-running bit. The bind address is never prefilled; `0.0.0.0` remains available only as an explicit all-interface opt-in. **Deploy / update bundle** publishes the complete bundled `tw-cli` directory, TLS/profile files, and a dedicated 0700 state directory; it rejects unsafe remote links/types/owners/modes, normalizes staged bundle directories/files to 0700/0600, and streams TLS bytes from an identity-checked `O_NOFOLLOW` descriptor instead of reopening a validated path. **Start local v2 Host** and **Start v2 Relay Center** persist desired-running only after the exact management child accepts the connector start. Start Center first starts or reuses the remote Center, launches that child, and reuses an exact already-ready projection when possible; otherwise it accepts one controller-owned attempt and polls the exact projection for a bounded 60-second readiness window. Each local management exchange retains its separate 5-second corruption deadline. Start Center returns success only after `host.registered` and all six base capabilities are present, while a readiness timeout returns `NOT_READY` without poisoning the child or clearing the accepted desired state; the management composition remains the only retry owner.

After a Dashboard process restart, Tauri performs at most one restore only when the persisted desired-running bit is true, profile/Host credential state is complete, no recovery or bootstrap rotation is pending, and the exact bundle, TLS profile, and remote Center probe ready. Deployment enabled and a live Center tmux session are prerequisites, not connector intent. The restore is bound to the same prepared persisted-config identity and steady management launch identity, revalidated under the deployment operation owner that also serializes explicit Start/Stop, so an older restore cannot start after Stop returns. Config schema 5 and older migrate unconditionally to stopped even if an old file explicitly contains `connectorDesiredRunning: true`. When Save or Deploy changes the persisted configuration, published binding, or prepared CA identity, the same owner first writes the old configuration as stopped, waits for an exact stop/drain of the old launch identity, and only then commits the new configuration as stopped. A drain failure leaves the old stopped configuration and binding intact, synchronously disposes the same management child to fence its retry policy, and never publishes the replacement; restarting Dashboard is then required. **Stop local v2 Host** and **Stop v2 Relay Center** persist stopped before connector drain; Stop Center additionally verifies that the exact tmux session is absent and returns a redacted error without rolling the stopped intent back if that postcondition cannot be proved. A redacted startup-restore failure is retained in process memory for the existing status UI, without a Tauri retry timer. Both Center operations preserve the Center's SQLite credential/continuity/keyring state. The selected devbox is currently only the Broker Center: the self-hosted Host exposes the real Mac's local TW-managed scope and sessions, not any SSH Host scope from the account config. It never reads or promotes the Relay v1 shared token. The one-time Host bootstrap is requested only for the first uncredentialed Host setup and moves through Tauri-owned remote/local 0600 files without entering renderer state, URLs, logs, or secret-valued argv. Before a persisted self-hosted management child starts, Tauri also prepares the contract-fixed local native 0700 directory and private 0600 Host profile/CA/bootstrap inputs without passing the native directory path to Node. The remote preflight rejects an older Node before creating the self-hosted deployment layout or starting the Center. This surface requires a bundled CLI with the explicit `--v2-single-node-self-hosted` Broker seam and the matching Host-management handoff; it does not make Relay v2 production-ready or change the default Relay v1 path.
...
- [Relay v2 contract](docs/relay-v2-contract.md): frozen protocol contract with shared Node/Android codec conformance fixtures. Node includes H0–H3 authorities, an injected stateless adapter from H3 to the existing terminal-control protocol, and a bounded host route/runtime core now closed by one explicit default-off canonical Host composition; the default CLI does not call it. The composition accepts H1 only through the opaque one-shot candidate issued inside the command plane's final recovered HostState audit cut; exact hostId/hostEpoch/hostInstanceId matching synchronously publishes H1 and gives only a private gated command facade to runtime, while public H1 readiness exposes only `close()`. H2 is opened asynchronously from a different opaque one-shot candidate issued only by the canonical snapshot-spool entry after `open()` recovered an unexpired current-owner cut whose manifest exactly matches a fresh materialized candidate. The candidate binds the current host process, owner fence/spool generation, exact recovered snapshot/materialized identity, same-spool owner and an issuer/receiver pair whose receiver remains private to canonical host composition; composition does not return until the internal receipt verification, activation lease attachment and H2 readiness publication all succeed. Public H2 readiness exposes only frozen `close()`. Before promotion, recovered-cut release or observed expiry rejects the final same-cut/receipt/owner/time recheck. Successful promotion atomically transfers the same opaque activation token and source activation within that metadata commit to a spool-wide live H2 record and consumes the receipt; later release or observed expiry of the original client cut retires only that cut and does not withdraw active H2. Snapshot-owner takeover, source/sink close, spool/fatal close, outer H2 close, and composition disposal still synchronously withdraw the live activation; expiry has no composition timer. H1/H3 close or fatal failure synchronously withdraws readiness and applies the existing 4406 route fence; composition disposal waits for admitted H1 calls and H3 manager shutdown. The composition now also owns one fixed-generation readiness producer statically bound to the production strict codec; base and carrier composition expose that producer only as a frozen `close()` lifecycle. `src/relayHost.ts` now exposes one default-off canonical production composition opener. After the recovered H0–H3 and managed owner are constructed, it activates H0 and then H3 before publishing the facade; either activation failure follows the existing rollback without starting the connector or advertising a subset. Optional Dashboard management configuration uniquely claims the same-lineage managed port and owns one protocol-v2 session, while omission preserves the original facade. CLI `run()` and capability advertisement still do not call this opener, the carrier continues to advertise an empty capability set, and the isolated seam does not establish production availability. The adapter does not auto-start a daemon, retry terminal input/resize, retain a parallel lease/operation fact, or provide direct tmux fallback. A separate broker credential native state-store contract (descoped — native crates removed, see docs/relay-v2-status.md) was at revision 2 while its N-API interface, storage/binary format and fixtures remain v1; it freezes exact secure-open, traditional process-owned `F_SETLK`, shared registry/fork/final-close semantics and deny-by-default durability qualification without changing the artifact, ABI or bytes. Independent unwired Rust foundations provide the bounded binary selector and owned transaction/publication/close core, the shared platform-common lifecycle owner, Darwin/Linux platform open seams, and a target-selected raw N-API binding. The binding eagerly captures the process lifecycle token and exposes only the frozen capability/open ABI over platform-common `ProcessBound*` and the selected platform seam; it does not own readiness, credential schema, continuity, fallback, or capability advertisement. The qualification allowlist is empty, so no platform-adapter-qualified real open or actual opened JavaScript transaction is reachable. An explicit, unwired local-target build/stage/npm-pack verification foundation now shares the loader's four fixed artifact descriptors, checks the Cargo artifact's Mach-O/ELF target identity, uses a final hard-link as the sole no-rollback commit point, verifies the temporary `npm pack --ignore-scripts` bounded pure-ustar/unpacked layout, imports the unpacked fixed loader, and runs the existing closed native-binding test against unpacked JavaScript and native artifacts. Its Node path checks do not claim malicious same-uid final-directory replacement resistance. Current evidence is Darwin arm64 only; there is no Darwin x64 or Linux run, four-target matrix, bit reproducibility, Dashboard bundle, signing/notarization, minimum-OS/glibc/SDK qualification, artifact provenance, qualified real open, production injection/composition/wiring, continuity readiness, or ready capability. The Dashboard now carries the complete canonical `dist/` sibling tree plus the root `package.json` under `tw-cli/`; this resource layout preserves the hidden management child and module-private ESM/import identity, but does not by itself establish native qualification or Relay v2 readiness. An unwired optional loader performs explicit target/N-API/fixed-artifact selection, and an unwired credential authority source foundation owns the opaque credential state and external-continuity gate. The credential authority APIs for enrollment create/redeem, client/host refresh, host reauthentication, and host bootstrap, plus the strict B1 `POST /v2/hosts/bootstrap` ingress adapter, are callable only in isolated modules and focused tests. Node also has a broker credential live-authorization/fence foundation adopted only by an explicit default-off activated Broker server composition: durable revoke and kid removal publish exact authorization revision/fence receipts; BrokerCore rechecks expiry, role/host binding, stale attach/auth-control and each route/frame dispatch; first authority withdrawal synchronously latches all broker admission/active-data gates through the same narrow port. Once-only close signals expose only a typed symbolic reason, safe authorization snapshot and connection incarnation; an independent unwired transport-close coordinator consumes them through exact kind/ID/incarnation matching and implements the 4401/4403/1013 policy plus a separate 5000 ms force-destroy deadline. The activated composition can be injected into `startRelayBroker`; an explicit default-off public root adopts an otherwise-unowned caller-supplied `node:https.Server`, activates that composition, and owns request/upgrade/listen plus close-and-drain for HTTP and both client/Host WSS paths. CLI `run()` omits the root, `qualifiedRecords=[]` prevents qualified native activation, and qualified real public-TLS/network/device plus ready-loss evidence remains absent. Node also has an independent unwired E1 external-continuity HTTPS adapter foundation for the outer transport/decode boundary. B1/B4/E1 are connected only inside that explicit default-off activated composition; qualified E0 deployment and production ready-loss evidence remain unimplemented. Response replay-key rotation exists only as an unwired private credential-envelope v3 foundation with a bounded active/decrypt-only keyring, exact record key IDs, lossless v1/v2 single-key state/ciphertext/TTL migration, idempotent rotation IDs, and fatal unknown-key/AEAD/malformed handling; it has no production management entry point, changes no public wire, and creates no readiness, enrollment, credential, or capability production path. Binding, staging, or pack verification is not readiness and never authorizes v1/BAU fallback. The rejected BAU path/JSON design is not part of the delivered source and is not a fallback. Android production now has a strictly limited path for already-present, cold-start-admitted explicit v2 profiles: one actor, bounded RFC6455 WSS, the isolated Room state-sync repository, durable command-query/recovered dispatch, and bounded fresh `QUEUED` dispatch. Only the exact actor `OnlineReady` cut starts fresh production; recovered capabilities flush first in durable commit order, then the Outbox core and Room transaction select creation-ordered batches of at most 32, commit `QUEUED` to `SENDING`, and only afterward issue Execute capabilities through the actor-owned `sendIfCurrent` transport gate. A crash after commit or a partial send leaves `SENDING`, so restart queries and never blindly resends. The same state database contains Outbox and terminal checkpoint rows but no credentials. The same base composition now owns one bounded exponential retry timer for exact retryable connection failures. It resets only at the actor's exact `OnlineReady` handoff, waits the actor transport/apply fence before each successor, rereads Room Outbox state per attempt, and preserves query-only recovery for `SENDING`; close, cancellation, disconnect/profile switch, and terminal failures fence pending retries. Android now wires all four frozen v2 command UI/enqueue paths: scoped Session reply, already-admitted existing-Session kill, and durable `create_worktree` / `create_terminal` for every current materialized UI-visible Scope, including an empty Scope. Each create consumes a composition-issued opaque exact cut from the committed Scope projection, commits its own `sessionId=null` create lane, and treats queued as not yet created: it neither inserts nor navigates to a Session before an authoritative `sessions.changed` upsert. A queued kill is likewise not terminated and does not optimistically remove the Session; only an authoritative `sessions.changed` delete removes it. No additional extension command UI is implied; enrollment has an explicit, default-off review-and-confirm surface, while refresh has no product UI. The admitted base runtime now owns durable terminal Room/journal/parser/actor/Compose wiring, exact `host.presence` recovery, and a dormant Agent runtime with the real notification platform owner; Agent capabilities remain empty. The `auth.expiring` rollover path is wired. The local admitted-v2 user-driven self-revoke quarantine/confirmed cleanup saga is closed but remains default-off; without real TLS/device/qualification/signing/release evidence it does not establish production availability or GO. Default capability advertisement and real-device/public-Relay-TLS interoperability evidence remain absent. This limited Android path does not make Relay v2 ready or available end to end; default relay-server and relay-host CLI/shipping paths remain v1 and their v2 composition entries remain default-off, and no fallback is added.
  Fresh-install H2 keeps its one-shot composition pair but places the live source activation under the same snapshot-spool lifecycle. A transient materialized withdrawal immediately gates H2; the next exact-foundation authoritative reconcile may restore it only by capturing and activating the exact current same-lineage cut with a strictly larger H2 generation. The reconcile edge carries no authority, old late close cannot revoke the successor, and synchronous source-release failure permanently poisons future activation. Snapshot-owner takeover and spool/fatal/outer close remain permanent fences; this is neither tombstone recovery nor a process-restart fallback.
  Across different spool roots, that materialized foundation has exactly one current bound-spool H2 claim. It replaces the claim in the same HostState serialized section that invalidates prior candidates and activations; both fresh and recovered issuance, composition activation, receipt publication, successor recovery, and gated runtime/spool access require the exact claim. A stale spool close or late callback uses compare-exact release and cannot clear the successor.
  The two Host CLI statements above describe the default/no-profile lane. The explicit `relay-host --profile v2` lane now calls the same canonical opener only through the unique trusted deployment source described below. That source uses the existing terminal-control client/daemon lifecycle to make the canonical daemon and exact sibling ingress ready before runtime-bundle preflight. A dedicated explicit-v2 process lifecycle owner then retains the shipping handle, performs the initial connector start, retries only retryable offline outcomes with capped backoff on the same `hostInstanceId`, permanently stops on `SUPERSEDED`, and drains the connector/root on signal or failure. Current Host qualification still fails closed before socket/capability publication, so this does not change production availability, the empty capability advertisement, the default Relay v1 lane, or the no-fallback rule.
  For local loopback development only, `relay-host --profile v2 --local-development --trusted-home <absolute-path> --credential-https-ca-input <path> --carrier-wss-ca-input <path>` opens a separate explicit activation. The trusted home must already exist as a canonical, current-user-owned, exact-`0700` directory; local profile read/create, terminal control, and runtime state stay under it, while the production trusted source still uses the account home. This lane reuses the same fixed profile/bootstrap semantics, canonical runtime bundle, shipping root, privileged intake, Vault/credential authority, carrier/controller, and process lifecycle, while replacing only the qualified native-cell opener with one isolated process-local atomic credential cell. The cell starts empty on every process launch, rejects Promise and own/inherited/hostile thenable results without assimilation, and is discarded at close, so cold-start development still needs the existing bootstrap input. Both profile endpoints must be loopback, and each TLS lane requires its own current-user-owned, regular, single-link, exact-`0600`, bounded CA file; certificate-chain and hostname validation remain enabled. The same shipping root also retains its optional canonical Dashboard management-session seam for an explicit upper-layer owner; that seam drives the existing connector and enrollment authorities and does not adopt the auto-start process lifecycle. This development activation has no environment equivalent, never reads or changes production `qualifiedRecords`, and does not alter default v1 or production `--profile v2` fail-closed behavior.
  Android credential composition is now one narrow step beyond that product-availability summary: the production profile runtime injects the strict OkHttp HTTPS credential exchange, while cold-start admission remains network-free. The explicit default-off enrollment review surface can reach the confirmed-enrollment port only after a user tap; QR/Intent input otherwise only prefills an in-memory, redacted review. Confirmation saves the admitted credential but does not open a socket; a separate explicit Connect action re-runs closed startup admission, verifies the exact confirmed profile, and only then starts the existing base runtime without a v1 fallback. The current-active-profile refresh port is reachable only from the explicit Connect/Retry action: when the admitted profile's access credential is expired (or a durable pending refresh attempt exists) and the refresh credential is still locally valid, the same user action refreshes through the existing owner, re-runs closed startup admission, and reconnects; cold-start admission and background reconnect remain network-free, and refresh failures keep typed errors without a Relay v1 fallback. The separate local admitted-v2 user-driven self-revoke quarantine/confirmed cleanup saga is closed but remains default-off; none of these flows advertises capability/readiness or changes the NO-GO fact that relay-server and relay-host are not production-ready for Relay v2.
  Dashboard enrollment now keeps the one-time code and enrollment payload out of React: Tauri strictly decodes the secret-bearing child projection, builds one bounded in-memory native artifact containing the QR PNG and exact deep link, and exposes only a revocable `dqart1.*` handle plus expiry and non-sensitive review facts. The same current handle can open the native AppKit QR window or, after an explicit user action, copy the issuer URL, relay URL, or one-time enrollment link directly to the macOS clipboard; the main WebView receives no code, payload, clipboard value, data URL, or image bytes. Artifact-registry startup and projection failures fail closed and dispose the management child. The bundled Tauri management-child supervisor now pins its production expected protocol to v2; a protocol-v1 ready frame or any other mismatch fails closed without respawn, fallback, or version switching. The hidden Node child speaks exact protocol v2 and owns one trusted explicit-v2 Host root in its process: it supplies only its stdio/runtimeVersion/clock/signal to the unique deployment opener, which threads them through the root's existing `dashboardManagement` seam into the same credential/controller/carrier composition. The child does not accept raw deployment/runtime inputs or adopt the separate relay-host auto-start/retry lifecycle; any activation, qualification, owner, or cleanup gap converges to typed `UNAVAILABLE` or ordinary failure, never to a second session or v1 fallback. Host qualification and E0/real TLS/device evidence remain unavailable, so the current child still fail-closes before a usable management session and this boundary creates no production enrollment, readiness, or capability.

  Node also has a default-off, listener-free client WSS ingress foundation. An external HTTP(S) server hands it only an Upgrade request/socket/head; Core selects the current Host carrier for the verified client identity and the sole producer registry resolves the exact target, so callers cannot inject one. The ingress consumes a client-only one-shot claim from the canonical runtime facade, pins this entry's exact `WebSocket.prototype` and private callback-socket brand, and does not contend with the separate Host claim. It enforces exact `/client`, an empty query, the sole `tw-relay.v2` protocol, no negotiated extensions/per-message deflate, one pre-101 admission, and private noServer/handler-child drain after a post-101 failure; only the shared owner owns total canonical-runtime close. It never listens, binds, or creates an HTTP(S) server, is not wired to `relayServer` or another production root, and adds no enrollment, readiness/capability advertisement, retry, or v1 fallback.

  Node also includes B4 credential HTTPS ingress for the frozen enrollment redeem, client/host refresh, and self-revoke endpoints. It reuses the B1 strict HTTP boundary, calls only credential-authority ports, and is adopted only by the explicit default-off activated Broker composition; no default production listener, qualified E0 deployment, readiness, capability advertisement, or v1 fallback is connected.

  A separate default-off Host connector attempt adapter binds one injected factory result—an exact canonical HostCarrier actor, its exact transport, and an opaque drain proof—to the existing controller `startAttempt`/`disposeAndDrain` port. The controller remains the sole owner of controller generations and start/stop fences; HostCarrier remains the sole owner of carrier generations, status, queues, and routes. This isolated adapter creates no WSS, process, credential, retry timer, readiness, capability advertisement, or v1 fallback, and has no `relay-host` or other production callsite.

  A closed default-off managed Host connector/runtime composition now constructs that controller and adapter together with every canonical per-attempt HostCarrier actor. Its injected lifecycle factory supplies only the transport, connection binding, and exact drain evidence; every actor is closed over the existing single route/readiness runtime owner, the same host lineage, the `tw-relay.v2` dialect, and an empty advertised capability set. The public handle exposes only inspection, explicit start/stop-and-drain, exact-current reauthentication through the canonical actor and fixed credential reference, readiness, terminal-frame send, and close-and-drain. Close fences new starts before controller drain and disposes runtime last. It is reachable from `relayHost.ts` only through the explicit default-off canonical production opener and still creates no socket/process/credential/timer retry, projects registration as `registered_incomplete`, and adds no production capability or protocol fallback.

  An independent default-off Host WSS lifecycle factory can fill that injected port. It binds HostCarrier metadata and WebSocket Authorization to one opaque owner-bound credential cut: the carrier receives only the exact reference/version/grant/JTI metadata, while the authority spends the same cut once by writing the Bearer value directly to the lifecycle-captured handshake request and ending it. Constructor options contain no token-bearing headers object, and credential authority never constructs the socket. The lifecycle accepts only a root `wss://` URL transformed to `/host`, offers only `tw-relay.host.v2`, disables per-message deflate, and owns bounded binary/FIFO buffering, write-callback ACK ordering, and close/drain callback fencing. Its behavior evidence uses an injected fake WebSocket only. It is selected only by that explicit canonical opener, never by CLI `run()`, and has no real TLS/process/network evidence, preserves empty capabilities/readiness, and adds no retry or v1 fallback.

  The existing Host shipping/bridge/intake options now also thread two independently optional, injected-only CA trust records: one for credential-issuer HTTPS and one for carrier WSS. Each lane captures and copies only a bounded non-empty CA list; omission independently selects system trust. Both transports force certificate validation and Node hostname verification, and the port cannot carry a client certificate/key, validation override, custom verifier, or arbitrary TLS option. This adds no deployment source, CLI/profile field, qualification, capability, fallback, or production TLS evidence.

  The default-off canonical Host production composition constructs that managed WSS seam around the same Host credential authority/reference used by the managed carrier and maps one already-parsed explicit v2 profile plus already-recovered H0–H3/welcome/credential authorities into a single owner. It activates H0 and then H3 before publishing the facade, and optional Dashboard management uniquely owns a same-lineage protocol-v2 session; omission leaves the facade unchanged. CLI `run()` does not call the opener, so it creates no production scan, state, socket, process, retry, fallback, capability advertisement, or usable Dashboard management path.
...
admitted H1 calls drain and the H3 manager shuts down. The default-off canonical
Host production composition now activates the recovered H0 proof and then H3
before publishing its facade; either failure rolls back without starting the
connector or advertising a partial capability set. The `relay-host` CLI and
capability advertisement still do not call this opener, the carrier advertises
...
default CLI, profile, TLS/native intake, capability advertisement, fallback,
or daemon startup consumes this foundation. The explicit Host v2 trusted
deployment activation below is its only shipping consumer.
...
(`src/relay/v2/hostShippingDeploymentSource.ts`) now close that chain into one
owner. The source reads and freezes the existing reference-only profile once,
selects the revision-7 fixed trusted native loader/source, consumes the
canonical runtime bundle, and issues two independent process-local TLS trust
cuts for credential HTTPS and carrier WSS. One opaque one-shot activation
binds those authorities and the same profile/Host lineage to recovered H0,
...
prevent a qualified open and any capability/readiness claim; default Relay v1
shipping and the overall NO-GO are unchanged.
...
for strict HTTP plus client/Host WSS. The default CLI omits it, Broker
...
chmod 600 .local/relay-v2/relay-dev.key.pem .local/relay-v2/relay-dev.cert.pem
tw relay-server \
  --v2-local-dev \
  --port 8787 \
  --v2-dev-tls-key "$PWD/.local/relay-v2/relay-dev.key.pem" \
  --v2-dev-tls-cert "$PWD/.local/relay-v2/relay-dev.cert.pem" \
  --v2-dev-advertised-origin "https://relay-dev.example.duckdns.org:9443/" \
...
`--host` is rejected. By default it preserves the existing single endpoint
identity: `https://localhost:<port>/` for the issuer and
`wss://localhost:<port>/client` for Android. The optional
`--v2-dev-advertised-origin` accepts only a strict HTTPS root origin and changes
only the issued endpoint identity: its root is the issuer, and the same
host/port becomes `wss://.../client`. The listener remains loopback even when
the advertised origin names a devbox/DuckDNS entry; the smoke deployment owns
the external TLS/TCP proxy and routing. For an emulator using the default
identity, run `adb reverse tcp:<port> tcp:<port>`; the local Host uses the
corresponding `wss://localhost:<port>/` root and continues to derive its
separate `/host` carrier endpoint. The TLS files must be absolute-path,
current-user-owned, regular, single-link files with exact mode `0600`; the
...
only its stdio IO, runtime version, clock, and one signal to the unique Host
trusted deployment opener. The opener owns one explicit-v2 shipping root in
the child process and threads that channel through the existing
`dashboardManagement` seam, so `start_connector` and `enrollment.create`
reach the same credential/controller/carrier composition. It does not invoke
the separate relay-host auto-start/retry process lifecycle, accept raw
deployment inputs, create a competing Host owner, or fall back to v1.
The app packages `cli.cjs`, the ESM package boundary, and the canonical dist
sibling tree together under `tw-cli/`, so the child loads the same
process-local Host owner entries without rebundling their private identity
registries into CJS.
Session shutdown drains management/controller before the root closes its
remaining owners. `relay-server` now selects
its trusted deployment source but cannot activate while native/E0
qualification stays empty. The separate `relay-host --profile v2` entry now
selects its own unique trusted source, but Host `qualifiedRecords=[]` stops
the Dashboard child open before a socket or capability advertisement. Thus
the trusted wiring is present but the current child still returns typed
`UNAVAILABLE`; neither path has a protocol fallback, and Dashboard management
plus Relay v2 overall remain unavailable.
...
gate. The composition now reconnects the same actor only for exact retryable
connection failures, using one bounded exponential timer. It resets backoff
only after exact `OnlineReady`, waits the actor's old transport/apply fence, and
rereads the Room Outbox for every successor; `SENDING` remains query-only.
...
Once `actor.connect` accepts the attempt, wrapper ownership is relinquished
under the same connection fence before an immediate transport failure can claim
the sole successor job.
Disconnect/profile switch and close fence retry scheduling first, retain the
exact current connection admission through the actor apply drain, and retire it
...
and durable `create_worktree` / `create_terminal` for every current materialized
UI-visible Scope, including an empty Scope, are wired to that authority. Reply
and kill consume Room's exact opaque Session cut. Both creates consume an opaque,
composition-issued exact cut from the committed Scope projection; the raw UI
scope ID is only a lookup key. All four hold the actor's exact current ONLINE
lease, revalidate Room authority, commit first, and only then use the existing
fresh dispatcher. Each create uses its canonical arguments in its own
`sessionId=null` create lane. Queued create does not mean created: the app neither
inserts nor navigates to a Session until an authoritative `sessions.changed`
upsert arrives. A queued kill likewise is not terminated and does not remove the
Session; only an authoritative delete does. The admitted base runtime now also owns the
durable terminal checkpoint/journal, exact actor transport, raw-byte WebView
parser ACK, Compose attachment, `host.presence` recovery, and a dormant Agent
runtime whose optional capabilities remain empty. Its `auth.expiring` rollover
path is connected. The local admitted-v2 user-driven self-revoke quarantine and
confirmed cleanup saga is closed but remains default-off; without real
TLS/device/qualification/signing/release evidence it does not establish
production availability or GO.
There is still no qualified/default-shipping end-to-end server/host v2 path,
default capability advertisement, qualified/default-shipping usable enrollment,
Agent delivery, or additional extension command producer,
so Relay v2 remains unavailable as a complete production feature.
