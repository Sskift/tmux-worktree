...
that shares one credential authority across its client/Host WSS and strict HTTP
owners when injected into `startRelayBroker`. The default CLI omits that
composition, Broker `qualifiedRecords=[]` prevents qualified native activation,
and no qualified E0 deployment, real TLS listener, or shipping evidence exists.
...
facade. The Tauri supervisor now expects v2 in production, but the hidden Node
CLI remains v1 and default-off, while `relay-host`, `relay-server`, and E0 still
have no production owner wiring. The mismatch therefore fails closed without a
fallback, and Dashboard management plus Relay v2 overall remain unavailable.
...
Only the explicit-v2 Session detail reply, already-admitted existing-Session kill,
and `create_worktree` for a UI-visible Scope that already contains a materialized
Session are wired to that authority. Reply and kill consume Room's exact opaque
Session cut. Create consumes an opaque, composition-issued Scope cut derived from
that materialized projection; the raw UI scope ID is only a lookup key. All three
hold the actor's exact current ONLINE lease, revalidate Room authority, commit first,
and only then use the existing fresh dispatcher. Create uses canonical
`CreateWorktree` arguments in the `sessionId=null` create lane. Queued create does
not mean created: the app neither inserts nor navigates to a Session until an
authoritative `sessions.changed` upsert arrives. Empty Scopes are not supported by
this path. A queued kill likewise is not terminated and does not remove the Session;
only an authoritative delete does. The admitted base runtime now also owns the
...
Agent delivery, or other v2 command producer,
