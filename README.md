...
Only the explicit-v2 Session detail reply and already-admitted existing-Session
kill paths are wired to that authority. Both consume the actor's exact current ONLINE
cut and Room's exact opaque Session projection, revalidate the materialized Session,
and commit before using the existing fresh dispatcher: reply uses fixed
`send_agent_message` arguments while kill uses `RelayV2OutboxArguments.KillSession`.
A queued kill is not terminated and does not remove the Session; only an authoritative
`sessions.changed` delete does. There is still no end-to-end server/host v2 runtime,
default capability advertisement, enrollment, terminal, Agent delivery, or other v2
command producer, so Relay v2 remains unavailable as a complete production feature.
