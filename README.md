...
- [Relay v2 contract](docs/relay-v2-contract.md): frozen protocol contract with shared Node/Android codec conformance fixtures. Node includes H0–H3 authorities, an injected stateless adapter from H3 to the existing terminal-control protocol, and a bounded host route/runtime core now closed by one explicit default-off canonical Host composition; the default CLI does not call it. The composition accepts H1 only through the opaque one-shot candidate issued inside the command plane's final recovered HostState audit cut; exact hostId/hostEpoch/hostInstanceId matching synchronously publishes H1 and gives only a private gated command facade to runtime, while public H1 readiness exposes only `close()`. H2 is opened asynchronously from a different opaque one-shot candidate issued only by the canonical snapshot-spool entry after `open()` recovered an unexpired current-owner cut whose manifest exactly matches a fresh materialized candidate. The candidate binds the current host process, owner fence/spool generation, exact recovered snapshot/materialized identity, same-spool owner and an issuer/receiver pair whose receiver remains private to canonical host composition; composition does not return until the internal receipt verification, activation lease attachment and H2 readiness publication all succeed. Public H2 readiness exposes only frozen `close()`. Cut release, owner takeover, spool/fatal close and later observed expiry synchronously withdraw active H2; expiry has no composition timer. H1/H3 close or fatal failure synchronously withdraws readiness and applies the existing 4406 route fence; composition disposal waits for admitted H1 calls and H3 manager shutdown. The composition now also owns one fixed-generation readiness producer statically bound to the production strict codec; base and carrier composition expose that producer only as a frozen `close()` lifecycle. `src/relayHost.ts` now exposes one default-off canonical production composition opener. After the recovered H0–H3 and managed owner are constructed, it activates H0 and then H3 before publishing the facade; either activation failure follows the existing rollback without starting the connector or advertising a subset. Optional Dashboard management configuration uniquely claims the same-lineage managed port and owns one protocol-v2 session, while omission preserves the original facade. CLI `run()` and capability advertisement still do not call this opener, the carrier continues to advertise an empty capability set, and the isolated seam does not establish production availability. The adapter does not auto-start a daemon, retry terminal input/resize, retain a parallel lease/operation fact, or provide direct tmux fallback. A separate [broker credential native state-store contract](contracts/relay/v2/broker-credential-state-store-v1/README.md) is at revision 2 while its N-API interface, storage/binary format and fixtures remain v1; it freezes exact secure-open, traditional process-owned `F_SETLK`, shared registry/fork/final-close semantics and deny-by-default durability qualification without changing the artifact, ABI or bytes. Independent unwired Rust foundations provide the bounded binary selector and owned transaction/publication/close core, the shared platform-common lifecycle owner, Darwin/Linux platform open seams, and a target-selected raw N-API binding. The binding eagerly captures the process lifecycle token and exposes only the frozen capability/open ABI over platform-common `ProcessBound*` and the selected platform seam; it does not own readiness, credential schema, continuity, fallback, or capability advertisement. The qualification allowlist is empty, so no platform-adapter-qualified real open or actual opened JavaScript transaction is reachable. An explicit, unwired local-target build/stage/npm-pack verification foundation now shares the loader's four fixed artifact descriptors, checks the Cargo artifact's Mach-O/ELF target identity, uses a final hard-link as the sole no-rollback commit point, verifies the temporary `npm pack --ignore-scripts` bounded pure-ustar/unpacked layout, imports the unpacked fixed loader, and runs the existing closed native-binding test against unpacked JavaScript and native artifacts. Its Node path checks do not claim malicious same-uid final-directory replacement resistance. Current evidence is Darwin arm64 only; there is no Darwin x64 or Linux run, four-target matrix, bit reproducibility, Dashboard bundle, signing/notarization, minimum-OS/glibc/SDK qualification, artifact provenance, qualified real open, production injection/composition/wiring, continuity readiness, or ready capability. The Dashboard still carries only `cli.cjs`; the loader/CJS resource layout is not a Dashboard shipping contract. An unwired optional loader performs explicit target/N-API/fixed-artifact selection, and an unwired credential authority source foundation owns the opaque credential state and external-continuity gate. The credential authority APIs for enrollment create/redeem, client/host refresh, host reauthentication, and host bootstrap, plus the strict B1 `POST /v2/hosts/bootstrap` ingress adapter, are callable only in isolated modules and focused tests. Node also has a broker credential live-authorization/fence foundation adopted only by an explicit default-off activated Broker server composition: durable revoke and kid removal publish exact authorization revision/fence receipts; BrokerCore rechecks expiry, role/host binding, stale attach/auth-control and each route/frame dispatch; first authority withdrawal synchronously latches all broker admission/active-data gates through the same narrow port. Once-only close signals expose only a typed symbolic reason, safe authorization snapshot and connection incarnation; an independent unwired transport-close coordinator consumes them through exact kind/ID/incarnation matching and implements the 4401/4403/1013 policy plus a separate 5000 ms force-destroy deadline. The activated composition can be injected into `startRelayBroker`; an explicit default-off public root adopts an otherwise-unowned caller-supplied `node:https.Server`, activates that composition, and owns request/upgrade/listen plus close-and-drain for HTTP and both client/Host WSS paths. CLI `run()` omits the root, `qualifiedRecords=[]` prevents qualified native activation, and qualified real public-TLS/network/device plus ready-loss evidence remains absent. Node also has an independent unwired E1 external-continuity HTTPS adapter foundation for the outer transport/decode boundary. B1/B4/E1 are connected only inside that explicit default-off activated composition; qualified E0 deployment and production ready-loss evidence remain unimplemented. Response replay-key rotation exists only as an unwired private credential-envelope v3 foundation with a bounded active/decrypt-only keyring, exact record key IDs, lossless v1/v2 single-key state/ciphertext/TTL migration, idempotent rotation IDs, and fatal unknown-key/AEAD/malformed handling; it has no production management entry point, changes no public wire, and creates no readiness, enrollment, credential, or capability production path. Binding, staging, or pack verification is not readiness and never authorizes v1/BAU fallback. The rejected BAU path/JSON design is not part of the delivered source and is not a fallback. Android production now has a strictly limited path for already-present, cold-start-admitted explicit v2 profiles: one actor, bounded RFC6455 WSS, the isolated Room state-sync repository, durable command-query/recovered dispatch, and bounded fresh `QUEUED` dispatch. Only the exact actor `OnlineReady` cut starts fresh production; recovered capabilities flush first in durable commit order, then the Outbox core and Room transaction select creation-ordered batches of at most 32, commit `QUEUED` to `SENDING`, and only afterward issue Execute capabilities through the actor-owned `sendIfCurrent` transport gate. A crash after commit or a partial send leaves `SENDING`, so restart queries and never blindly resends. The same state database contains Outbox and terminal checkpoint rows but no credentials. The same base composition now owns one bounded exponential retry timer for exact retryable connection failures. It resets only at the actor's exact `OnlineReady` handoff, waits the actor transport/apply fence before each successor, rereads Room Outbox state per attempt, and preserves query-only recovery for `SENDING`; close, cancellation, disconnect/profile switch, and terminal failures fence pending retries. Android now wires all four frozen v2 command UI/enqueue paths: scoped Session reply, already-admitted existing-Session kill, and durable `create_worktree` / `create_terminal` for every current materialized UI-visible Scope, including an empty Scope. Each create consumes a composition-issued opaque exact cut from the committed Scope projection, commits its own `sessionId=null` create lane, and treats queued as not yet created: it neither inserts nor navigates to a Session before an authoritative `sessions.changed` upsert. A queued kill is likewise not terminated and does not optimistically remove the Session; only an authoritative `sessions.changed` delete removes it. No additional extension command UI is implied; enrollment has an explicit, default-off review-and-confirm surface, while refresh has no product UI. The admitted base runtime now owns durable terminal Room/journal/parser/actor/Compose wiring, exact `host.presence` recovery, and a dormant Agent runtime with the real notification platform owner; Agent capabilities remain empty. The `auth.expiring` rollover path is wired. The local admitted-v2 user-driven self-revoke quarantine/confirmed cleanup saga is closed but remains default-off; without real TLS/device/qualification/signing/release evidence it does not establish production availability or GO. Default capability advertisement and real-device/public-Relay-TLS interoperability evidence remain absent. This limited Android path does not make Relay v2 ready or available end to end; default relay-server and relay-host CLI/shipping paths remain v1 and their v2 composition entries remain default-off, and no fallback is added.
  ...
  Dashboard enrollment now keeps the one-time code and QR payload out of React: Tauri strictly decodes the secret-bearing child projection, renders a bounded in-memory PNG, and exposes only a revocable `dqart1.*` handle plus expiry and non-sensitive review facts. Showing the handle opens a native AppKit image window; the main WebView receives no code, payload, data URL, or image bytes. Artifact-registry startup and projection failures fail closed and dispose the management child. The bundled Tauri management-child supervisor now pins its production expected protocol to v2; a protocol-v1 ready frame or any other mismatch fails closed without respawn, fallback, or version switching. The hidden Node child speaks exact protocol v2 and now has one default-off selection entry (`relayV2DashboardManagementChildRuntime.ts`): only an injected narrow Host factory — the production wrapper binds the canonical Host shipping root, whose default-off `dashboardManagement` seam threads the caller's channel through the bridge/intake to the canonical composition's same-lineage session and exposes it as `runDashboardManagement` — can run a real management session; any qualification, owner, or cleanup gap converges to typed `UNAVAILABLE` or ordinary failure, never to a second session. The hidden-child production CLI does not obtain the trusted Host activation and the bundled .app ships cli.cjs only (the child entry stays a standalone external), so the child still fail-closes with `UNAVAILABLE` for every operation. The separate explicit `relay-host --profile v2` path now owns a trusted source, but Host qualification and E0/real TLS/device evidence remain unavailable; Dashboard management therefore remains unavailable and this boundary creates no production enrollment, readiness, or capability.
...
facade. The Tauri supervisor and hidden Node child both use exact protocol
v2. The child's default-off selection entry now consumes an injected narrow
Host factory — the production wrapper binds the canonical Host shipping
root's new default-off `dashboardManagement` seam — but the production CLI
injects no trusted deployment inputs and the .app ships cli.cjs only (the
child entry stays a standalone external), so the child still fail-closes
every operation with `UNAVAILABLE`. `relay-server` now selects
...
the open before a socket or capability advertisement; the hidden Dashboard
child does not obtain that activation. Neither path has a protocol fallback,
so Dashboard management plus Relay v2 overall remain unavailable.
