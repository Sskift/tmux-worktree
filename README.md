...
Only the explicit-v2 Session detail reply path is wired to that authority, through
the actor's exact current ONLINE cut and Room's exact opaque Session projection;
it durably commits fixed `send_agent_message` arguments before using the existing
fresh dispatcher. There is still no end-to-end server/host v2 runtime, capability
advertisement, enrollment, terminal, Agent delivery, or other v2 command producer,
