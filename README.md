...
- [Relay v2 contract](docs/relay-v2-contract.md): frozen protocol contract with shared Node/Android codec conformance fixtures. Node includes unwired H0–H3 and a bounded host route/runtime pure composition core; neither `src/relayHost.ts` nor any production composition constructs it. Android includes unwired actor/profile, isolated Room state-sync repository, HTTPS credential and bounded RFC6455 WSS adapter foundations, plus pure Outbox authority and Android-free terminal checkpoint/reducer cores; the state database itself omits Outbox, terminal checkpoint, and credential. The WSS foundation exists, while production WSS/composition and real-device/public-Relay-TLS interoperability evidence do not. No v2 capability, enrollment, or credential production path is connected; current products still run Relay v1.
- [Relay Agent extension v1](docs/relay-agent-transcript-lifecycle-extension-v1.md): frozen optional-extension fixtures plus unwired Node host authority/public codec/durable store/replay runtime and Android lifecycle/notification reducer foundations. The Node store has strict final-byte preflight, byte-aware pinned pages, retention, and a local crash-cut witness, but the required shared monotonic/CAS continuity anchor is not integrated, so paired rollback remains a security-review blocker. Production relay-host/Android wiring, G4, and capability advertisement remain pending.
