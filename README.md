...
capabilities, and no protocol fallback is added. The isolated Host connector
attempt adapter and its closed managed connector/runtime composition now accept
the credential-exact WSS lifecycle factory described above, but still have no
production composition callsite. No real TLS socket or process path has been
exercised.
...
only an authoritative delete does. There is still no end-to-end server/host v2
runtime, default capability advertisement, enrollment, terminal, Agent delivery,
or other v2 command producer, so Relay v2 remains unavailable as a complete
production feature.
