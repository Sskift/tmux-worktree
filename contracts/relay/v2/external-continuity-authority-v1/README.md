...
outer HTTPS trust boundary同样冻结。每个HTTP attempt都只能向config中的exact endpoint发送`POST`且不得redirect；同一logical read/CAS仅可按下文request identity规则在原deadline内发起新的attempt。每次attempt发送auth前必须按`tlsTrustReference`验证TLS peer及exact endpoint identity，不得派生path/query。任意redirect在follow及转发workload credential前即fail closed，credential只可用于最初的exact HTTPS endpoint。request固定`Content-Type: application/json`、`Accept: application/json`、`Cache-Control: no-store`与`Accept-Encoding: identity`；response要求`Cache-Control: no-store`，`Content-Encoding`只允许缺失或`identity`，禁止压缩与解压。只有HTTP status exact `200`、`Content-Type` exact `application/json`并满足上述response headers时才进入bounded `responseEnvelope` decoder；external error也只存在于该200 closed union中。任何非200或必需response header mismatch都在读取body前作为boundary failure，adapter不得读取、解析、记录、反射或回显proxy body，因而4xx/5xx不会产生另一条external error decoder路径。body与JSON limits直接复用`contracts/relay/v2/manifest.json`的`httpsBodyBytes=16384`、`httpsJsonMaxDepth=8`、`httpsJsonMaxTotalKeys=32`，不是本contract另造数值：发送前必须先得到不超限的完整body；接收端对超限Content-Length在读取前拒绝，对缺失长度/chunked使用counting reader并在limit+1停止，禁止先无界聚合。只接受strict UTF-8、恰好一个JSON object，duplicate key、trailing JSON、深度/key超限和closed schema violation都拒绝。上述boundary failure对read闭合为`ANCHOR_UNAVAILABLE`，对CAS闭合为`ANCHOR_COMMIT_UNCERTAIN`。
