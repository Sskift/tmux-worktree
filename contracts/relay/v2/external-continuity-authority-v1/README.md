...
outer HTTPS trust boundary同样冻结。每个read/CAS只向config中的exact endpoint发一次`POST`；发送auth前必须按`tlsTrustReference`验证TLS peer及exact endpoint identity，不得派生path/query或跟随redirect。任意redirect在follow及转发workload credential前即fail closed，credential只可用于最初的exact HTTPS endpoint。request/response的`Content-Type`固定`application/json`，request声明`Accept-Encoding: identity`，`Content-Encoding`只允许缺失或`identity`，禁止压缩与解压。body与JSON limits直接复用`contracts/relay/v2/manifest.json`的`httpsBodyBytes=16384`、`httpsJsonMaxDepth=8`、`httpsJsonMaxTotalKeys=32`，不是本contract另造数值：发送前必须先得到不超限的完整body；接收端对超限Content-Length在读取前拒绝，对缺失长度/chunked使用counting reader并在limit+1停止，禁止先无界聚合。只接受strict UTF-8、恰好一个JSON object，duplicate key、trailing JSON、深度/key超限和closed schema violation都拒绝。上述boundary failure对read闭合为`ANCHOR_UNAVAILABLE`，对CAS闭合为`ANCHOR_COMMIT_UNCERTAIN`。
...
每个 logical read/CAS 使用 collision-resistant CSPRNG opaque `operationId`；其具体编码属于 adapter实现，但不得被解析为业务 identity。fingerprint不是encoded JSON bytes或另复制的一套canonical hash：backend先完成strict bounded closed decode，再对manifest列出的read/CAS tuple逐字段比较exact JSON type/value；object key顺序不参与比较，禁止coercion或Unicode normalization。同一 operationId + exact decoded tuple必须重放同一 committed decision；相同 ID不同tuple返回 `IDEMPOTENCY_CONFLICT`。adapter 可以在原 deadline内重放同一 request identity，但 deadline/uncertain之后禁止用新旧 identity盲重试 CAS，必须 read-reconcile。
...
`agent-transcript-lifecycle.v1`复用现有extension store mapping，不新增error：`ANCHOR_UNAVAILABLE|BUSY` → `AGENT_AUTHORITY_STORE_CONTINUITY_UNAVAILABLE`；`STATE_COMMIT_UNCERTAIN|ANCHOR_COMMIT_UNCERTAIN|RECONCILIATION_REQUIRED` → `AGENT_AUTHORITY_STORE_COMMIT_UNCERTAIN`；`INVALID_CHECKPOINT|INVALID_AUTHORITY_RESPONSE|LOCAL_STATE_CONFLICT|CAS_CONFLICT|ROLLBACK_DETECTED` → `AGENT_AUTHORITY_STORE_CORRUPT`。三类结果都只允许extension authority/store fail closed并进入对应restore/reconcile/reset流程；它们不得撤回broker credential readiness，不得阻止基础v2 Upgrade/route/command/terminal，不得全局fence或关闭基础v2 connection。未来extension capability或extension frame admission只能在该extension authority完成相应恢复后重新开放。machine fixture中的`ready/admission/connectionFence`字段在agent namespace case表达基础broker路径仍为ready/open/unfenced，而不是声称extension仍可用。
