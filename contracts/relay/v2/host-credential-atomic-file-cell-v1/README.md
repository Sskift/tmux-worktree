...
状态：**Frozen contract revision 6；native ABI、native fixture、platform resource contract、claim journal format 与 credential mutation contract 各为 v1，native artifact identity contract 与 fixture 各为 v1。default-off、injected-only。Platform-common 已在唯一 `AdmissionOwner` 内实现 mutation v1，Darwin arm64 与 Linux `x86_64-unknown-linux-gnu` descriptor-relative syscall adapter 也已分别实现 mutation v1。Node 侧另有独立、default-off 的 injected one-shot native module source/holder foundation（`src/relay/v2/hostCredentialNativeModuleSource.ts`）：它只拥有 module handle 的一次性移交与 ABI admission，并声明 exact target/platform/contractRevision/ABI capability；加载完全来自注入 loader，one-shot 移交 module ownership 后不可重取，失败/close 有界回收并 fail closed。Revision 6 新增冻结的 native artifact identity（见下文 "Native artifact v1"）：四项 fixed target descriptor、binary/digest/layout/loader 输入规则已冻结，Host 专属 raw N-API crate、JS fixed target descriptor 模块、fixed loader 与本机 target build/stage/npm-pack verification foundation 已作为 default-off、未接线 foundation 存在；fixed loader 是可被 trusted deployment 显式选择的窄 source。它们都不是 trusted factory，也不产生 qualification；trusted production factory、durability qualification、recovery 与 production wiring 仍未实现，当前只有本机单 target（`darwin-arm64`）证据。**
...
Revision 6 保持 credential mutation/native ABI/fixture/platform resource/claim journal 各自为 v1，并新增 native artifact identity v1（artifact contract 与 fixture 各为 v1）。Platform-common 通过 additive `CredentialMutationPlatform` 子 trait，在同一个 `AdmissionOwner` 内拥有 read、opaque revision、双 current-check CAS、tracked temp、rename publication、cleanup 与 terminal fence；Darwin/Linux adapter 分别实现该 mutation trait 的 descriptor-relative syscall seam，既有 `DescriptorRelativePlatform` admission trait 与 owner 边界不变。当前仍不实现 trusted production factory、HOME/path/env 输入、global lookup、完整 admission、durability qualification、orphan recovery/enumeration、continuity、Vault/Authority injection、`relay-host` composition、readiness 或 capability advertisement。现有 H4a path-based cell 不是本 ABI 或 admission owner 的 production fallback。Node 侧的 injected one-shot native module source/holder foundation 只拥有 module handle 的一次性移交与 ABI admission，并只声明 exact target/platform/contractRevision/ABI capability；它不是上述 trusted factory，也不产生 qualification。revision 6 冻结的 N-API crate、fixed loader 与 build/stage/pack foundation 同样是 default-off、未接线 foundation，只证明本机单 target 的 closed artifact 链，不构成 real open、qualification、readiness 或 capability。
