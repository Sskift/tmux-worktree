...
状态：**Frozen contract revision 7；native ABI、native fixture、platform resource contract、claim journal format 与 credential mutation contract 各为 v1，native artifact identity contract 与 fixture 各为 v1，trusted factory contract 为 v1。default-off、injected-only。Platform-common 已在唯一 `AdmissionOwner` 内实现 mutation v1，Darwin arm64 与 Linux `x86_64-unknown-linux-gnu` descriptor-relative syscall adapter 也已分别实现 mutation v1。Node 侧另有独立、default-off 的 injected one-shot native module source/holder foundation（`src/relay/v2/hostCredentialNativeModuleSource.ts`）：它只拥有 module handle 的一次性移交与 ABI admission，并声明 exact target/platform/contractRevision/ABI capability；加载完全来自注入 loader，one-shot 移交 module ownership 后不可重取，失败/close 有界回收并 fail closed。Revision 6 冻结的 native artifact identity（四项 fixed target descriptor、binary/digest/layout/loader 输入规则）与 Host 专属 raw N-API crate、JS fixed target descriptor 模块、fixed loader、本机 target build/stage/npm-pack verification foundation 保持 default-off、未接线。Revision 7 以 additive 方式冻结并实现 trusted factory v1（见下文 "Trusted factory v1"）：raw N-API artifact 保持 exact v1 单 `open` 表面，raw factory 只作为 additive own-data entry 存在于 raw open function 上（不声称可见性隔离），其唯一 production driver 是 JS fixed trusted loader；native producer 已在 Darwin 与 Linux adapter 中实现 account-database home proof 与 contract 固定 private-location secure open；factory mint 不可伪造 capability，one-shot binder 只返回 exact own-data `{open}` 的最终 v1 module，最终 module 不携带 factory entry。该 final open 的 qualified 分支现已把 sole prebound descriptor 一次性移交 platform-common `AdmissionOwner`，并发布 frozen v1 的同步 read/CAS/close handle；它不新增 credential owner。它们都不产生 qualification；trusted factory 的 production driver selection/deployment、durability qualification、recovery 与 production wiring 仍未实现，当前只有本机单 target（`darwin-arm64`）的 closed-gate 证据。**
...
相邻的 `native/relay-v2-host-credential-atomic-file-cell-platform-darwin` 与 `native/relay-v2-host-credential-atomic-file-cell-platform-linux` 已分别实现 macOS 与 Linux 的 `DescriptorRelativePlatform` admission seam 和 additive `CredentialMutationPlatform` seam。当前真实验证覆盖 `aarch64-apple-darwin` 与 `x86_64-unknown-linux-gnu`：descriptor-relative filesystem 与 credential read/temp/write/fsync/unlink/rename 操作、独立 subprocess 间 `F_SETLK` busy 与 raw-close release，以及 exec 边界的 `FD_CLOEXEC`。这些是两个 target adapter 各自的实现证据，不是完整 admission 或 durability qualification；Darwin x86_64 与 Linux arm64 尚无证据。
...
Revision 6 保持 credential mutation/native ABI/fixture/platform resource/claim journal 各自为 v1，并新增 native artifact identity v1（artifact contract 与 fixture 各为 v1）。Revision 7 以 additive 方式冻结并实现 trusted factory v1（见下文），nested raw N-API/module ABI、fixture、platform、mutation、claim journal、read/CAS/close 全部保持 v1 不变。Platform-common 通过 additive `CredentialMutationPlatform` 子 trait，在同一个 `AdmissionOwner` 内拥有 read、opaque revision、双 current-check CAS、tracked temp、rename publication、cleanup 与 terminal fence；Darwin/Linux adapter 分别实现该 mutation trait 的 descriptor-relative syscall seam，既有 `DescriptorRelativePlatform` admission trait 与 owner 边界不变。N-API final module 只在 production qualification 成功后 adopt exact capability，并把同一个 owner 投影为既有同步 handle。当前仍不实现 HOME/path/env 输入、global lookup、durability qualification、orphan recovery/enumeration、continuity、Vault/Authority injection、`relay-host` composition、readiness 或 capability advertisement。现有 H4a path-based cell 不是本 ABI 或 admission owner 的 production fallback。Node 侧的 injected one-shot native module source/holder foundation 只拥有 module handle 的一次性移交与 ABI admission，并只声明 exact target/platform/contractRevision/ABI capability；它不是 trusted factory 的 production driver，也不产生 qualification。revision 6 冻结的 N-API crate、fixed loader 与 build/stage/pack foundation 同样是 default-off、未接线 foundation；当前证据仍只证明本机单 target 的 closed artifact 链，不构成 qualified real open、readiness 或 capability。

## Trusted factory v1

Revision 7 additive 冻结并实现 trusted native capability factory v1。最终 v1 module 的唯一 source 是本 factory；factory 的唯一 production driver 是 fixed trusted loader（`src/relay/v2/hostCredentialNativeLoader.ts` 的 trusted loader）。raw N-API artifact 保持 exact v1 单 `open` 表面（frozen `nativeInterface` v1 不变）；raw factory `createRelayV2HostCredentialAtomicFileCellTrustedFactoryV1` 只作为 additive own-data entry 存在于 raw open function 上——任何拿到 open function 的 JS 都能发现它，因此本 contract 不声称可见性隔离，只冻结其唯一 production driver/consumer；frozen v1 consumer 不依赖该 entry，既有 fixed loader、module source、wrapper 与 pack 流程的可观察行为全部不变。既有 raw v1 module 契约保持独立，不做 v1 fallback 或双选路；最终 module 的 open function 不携带该 entry。

Loader 导入 raw factory 后在初始化时 exactly-once 无参调用它。每次 callback 都先做 process-origin PID fence，再消费 exactly-once claim，最后才观察和校验 JS 参数；因此首次带任何参数返回 `INVALID_ARGUMENT` 但已经消费 claim，第二次无论参数为何都固定返回 `CELL_CLOSED`，fork child 也在参数观察前固定 `CELL_CLOSED`。调用时 native producer（Darwin/Linux adapter 各自实现）在任何 path observation 前先证明 credential snapshot（real==effective uid/gid 且非 root），再只经 native account database（getpwuid 族）取得 effective uid 的 account home——绝不读 HOME、env、argv、JSON 或全局 lookup——然后只拼接 platform-common build-time 单点消费的 contract 固定 private-location 组件 `.tmux-worktree/relay-v2-host-credential-atomic-file-cell-v1`，经 no-follow 逐层 open 取得 exact owner（euid+egid）、no-symlink、exact `0700`、`FD_CLOEXEC` 的 directory descriptor。home 必须是 euid 所有且无 group/other write；每个 derived component 必须是 directory、euid+egid 所有、exact `0700`。不创建、不 chmod、不修复、不选 alternate candidate；任何缺失/不安全/竞态对象都在 mint 前 fail closed。factory 从 JS 接收 path/fd/HOME/env/credential 一律不可能：ABI 根本没有参数。

成功后 factory mint 不可伪造 native capability：它只以 Rust 值存在，从不作为 value 暴露给 JS，也不暴露 raw fd number；descriptor 一次性接管，不 dup、不 reopen。one-shot binder（factory ready 结果上的 `bind`）消费该 capability 并返回最终 module：exact own-data 仅 `{ openRelayV2HostCredentialAtomicFileCellV1 }` 的 v1 module——factory 与 open 绝不在同一最终对象并列。binder replay 固定 `CELL_CLOSED`；构造失败的 rollback、未 bind 的丢弃、未 adopt capability 的 final close 都把 descriptor exactly-once raw close（不重试）；bind callback 与最终 module open callback 都把 capability origin PID fence 作为首个可观察操作，早于 argument observation/decode、mutex/take 或 descriptor handling。PID-fork 后 missing/malformed 参数也固定 `CELL_CLOSED`，fork child 绝不 take、read、adopt 或 close inherited descriptor。最终 module 的 `open` 保持 frozen v1 request/result ABI：空 `qualifiedRecords=[]` 下仍固定 `CELL_DURABILITY_UNSUPPORTED`，在 registry reservation、descriptor adopt 或任何 credential mutation 前失败；若未来冻结的 production qualification 成功，它才 one-shot take capability，将 platform+descriptor 移交 platform-common 唯一 `AdmissionOwner`，并发布 exact own-data `read`/`compareAndSwap`/`close` handle。opaque revision 由 native owner-bound one-shot value承载，read/conflict签发、CAS消费；close 只调用同一 common owner 的稳定 exactly-once close。adopt 失败后 descriptor cleanup 也只属于 common open attempt；本 factory 不新增 mutation owner 或 registry。Trusted factory 的 production driver selection/deployment、qualification、recovery、Vault/Authority 与 `relay-host` wiring 仍未实现；本 foundation default-off、未接线，不产生 qualification、readiness 或 capability。
...
## Credential mutation v1（platform-common + Darwin/Linux adapters）
...
上述完整机器语义与 failure cuts 见 [`credential-mutation-cases-v1.json`](credential-mutation-cases-v1.json)。实现声明为 platform-common、Darwin adapter 与 Linux adapter 均已实现；全局 `credential-cell-read-cas-temp-or-rename` 因此不再列入 `notImplemented`。这只移除已经完成的纯 credential-cell read/CAS/temp/rename 实现缺项：`qualifiedRecords=[]`、`productionProofConstructible=false`、`fullAdmissionValidated=false`、`durabilityQualified=false`、`productionWired=false` 与 `productionCapabilityEffect=none` 全部保持不变。Trusted factory v1 已由 revision 7 实现（见上文 "Trusted factory v1"），但其 production driver selection、durability qualification、recovery/enumeration、Vault/Authority 或 `relay-host` wiring 仍未实现；revision 6 冻结的 artifact identity 与 revision 7 的 factory foundation 都只是 default-off、未接线 foundation，不构成 real open 或 qualification，所以该链路仍 default-off、NO-GO，不产生 readiness 或 capability。
...
Revision 6 冻结 Host 专属 native artifact 的 build/stage/pack/loader identity（contract 与 fixture 各为 v1）。四项 fixed target descriptor（`darwin-arm64`、`darwin-x64`、`linux-arm64`、`linux-x64`）各自精确冻结 platform/architecture、cargo target triple、cargo dynamic library 文件名、runtime `.node` 文件名、loader-relative module specifier、staged 与 packed 相对路径；crate identity 固定为独立 cdylib `native/relay-v2-host-credential-atomic-file-cell-napi`（N-API、abiVersion 1、最低 napi 9、同步-only）。Binary identity 只接受冻结的 Mach-O/ELF header 规则；digest 输入规则固定为 SHA-256 覆盖整个 cargo artifact 字节，source 与 staged digest 必须在 hard-link 唯一 commit point 前一致，commit 后 final name 永不 rollback，只 identity-check 清理私有随机 staging 目录，packed artifact 必须是同一字节的恰好一个 native artifact。Layout 规则按 owner 固定：staged 目录与 packed tar 的 native subset 中，本 owner 的 runtime artifact 必须恰好一个且为选中 target；同一 target 的冻结 Broker owner artifact（由其 fixed target descriptor 模块枚举，本 owner 不校验其内容、不复制其命名）允许共存；其余任何未知条目（包括空 directory）、重复、错 target 或非 regular file 均拒绝，并禁止 npm lifecycle script 接入。final hard-link 只提交本 owner 文件；pre-existing 目录只在只含允许 sibling 时被 adopt，绝不替换或清理。Loader 规则固定 fixed mapping only、exact current target、最低 N-API 9；只有 fixed artifact 在 resolve 阶段确实缺失才映射 `native_artifact_missing`，其余一切加载/binding 失败都是 redacted invalid；无动态扫描、无 alternate candidate、无 env/HOME/JSON/BAU/v1 fallback。

机器可消费的 canonical descriptor 与 case 见 [`native-artifact-cases-v1.json`](native-artifact-cases-v1.json)；fixture 只含非敏感声明数据，不含真实 digest 值、路径或 credential。Raw N-API module 继续只提供 default-off refusal 路径；trusted-bound final module 另已实现 qualification 成功后的 descriptor adoption 与 frozen read/CAS/close handle。当前 production durability qualification 仍 deny-by-default，所以可观察 open 在 process registry 和任何 credential mutation 前固定返回 `CELL_DURABILITY_UNSUPPORTED`。Revision 7 的 trusted factory v1 已在同一 crate 中实现（见上文 "Trusted factory v1"）：raw artifact 保持 exact v1 单 `open` 表面，factory 只作为 additive own-data entry 存在于 raw open function 上；pack verifier 证明 packed artifact 可加载、raw open 在 gate 前失败、factory 的两条 contract-closed 结果分支（无部署目录时的 error union，或目录合法存在时 ready→one-shot bind→final exact-open→gate）均闭合，且 factory replay 固定 `CELL_CLOSED`。Artifact 的存在或可加载不产生 qualification、readiness 或 capability：`platformResources.durabilityQualification.qualifiedRecords` 仍为 `[]`。Trusted factory 的 production driver selection、durability qualification、recovery 与 production wiring 仍未实现；当前只有本机单 target（`darwin-arm64`）的 build/stage/pack/closed-binding 证据，没有 qualified opened-handle runtime evidence；其余三个 target、签名、bit-reproducibility 与 production shipping 证据仍缺失，整体仍为 NO-GO。

机器可读 union 和每个 conformance case 以 [`manifest.json`](manifest.json)、[`native-interface-cases.json`](native-interface-cases.json)、[`credential-mutation-cases-v1.json`](credential-mutation-cases-v1.json) 与 [`native-artifact-cases-v1.json`](native-artifact-cases-v1.json) 为准。Fixture 只含非敏感占位 bytes，不含 credential、secret、真实路径或 production readiness 声明。
