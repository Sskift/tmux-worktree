...
成功后 factory mint 不可伪造 native capability：它只以 Rust 值存在，从不作为 value 暴露给 JS，也不暴露 raw fd number；descriptor 一次性接管，不 dup、不 reopen。one-shot binder（factory ready 结果上的 `bind`）消费该 capability 并返回最终 module：exact own-data 仅 `{ openRelayV2HostCredentialAtomicFileCellV1 }` 的 v1 module——factory 与 open 绝不在同一最终对象并列。binder replay 固定 `CELL_CLOSED`；构造失败的 rollback、未 bind 的丢弃、最终 close 都把 descriptor exactly-once raw close（不重试）；bind callback 与最终 module open callback 都把 capability origin PID fence 作为首个可观察操作，早于 argument observation/decode、mutex/take 或 descriptor handling。PID-fork 后 missing/malformed 参数也固定 `CELL_CLOSED`，fork child 绝不 take、read、adopt 或 close inherited descriptor。最终 module 的 `open` 与 raw v1 module 走完全相同的 frozen gate 链：空 `qualifiedRecords=[]` 下仍固定 `CELL_DURABILITY_UNSUPPORTED`，在 registry reservation、descriptor adopt 或任何 credential mutation 前失败。未来真正的 adopt 只会进入 platform-common 唯一 `AdmissionOwner`/registry；本 factory 不新增 mutation owner 或 registry。Trusted factory 的 production driver selection/deployment、qualification、recovery、Vault/Authority 与 `relay-host` wiring 仍未实现；本 foundation default-off、未接线，不产生 qualification、readiness 或 capability。
