...
- Native `runExclusive` 必须 exactly once 调 callback，并在 callback settle 后才 settle；业务 callback 的 result/error 由 wrapper 本地保存，不要求 native identity-echo result，也不要求业务异常穿过 native 再返回。Duplicate callback、callback 尚 pending 时 raw run 提前 resolve/reject/throw，或完全未调用 callback 都是 protocol violation：wrapper 在向外返回 `NATIVE_INTERFACE_INVALID` 前立即 terminal-fence 同一 store，已 admission callback 后续 read/publish 与新 admission 都只能得到 `STORE_CLOSED`。
