...
test("bad fields fall back safely without throwing or contaminating valid fields", () => {
  ...
  assert.doesNotThrow(() => {
    migrated = compatibleLayout({
      ...
      worktreeGroupOrder: ["valid-group", 42, null],
      terminalOrder: ["valid-terminal", 42, null],
  ...
  assert.deepEqual(plainLayout(migrated as DashboardLayoutV2), {
    ...
    worktreeGroupOrder: ["valid-group"],
    terminalOrder: ["valid-terminal"],
...
test("invalid markers, conflicts, objects, and current layouts fail closed", () => {
  ...
  for (const value of [
    ...
    { schemaVersion: 2, columnOrder: [...DEFAULT_COLUMN_ORDER], worktreeGroupOrder: ["valid", 42] },
    { schemaVersion: 2, columnOrder: [...DEFAULT_COLUMN_ORDER], terminalOrder: ["valid", 42] },
  ]) {
    assert.deepEqual(decodeDashboardLayout(value), {
      kind: "invalid",
...
test("extensions cannot revive known fields or reserved version markers", () => {
  ...
  const extensions = {
    ...
    automationHeight: 1,
    sessionOrder: ["poison"],
    worktreeGroupOrder: ["poison"],
    terminalOrder: ["poison"],
