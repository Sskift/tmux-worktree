...
const COT_REPLY_BRIDGE_CAPABILITY: &str = "reply.cot-message.v1";
...
fn should_upgrade_empty_bridge(probe: &BridgeProbe) -> Result<bool, String> {
    Ok(bridge_snapshot_is_empty(&probe.snapshot)?
        && (probe.disposition == BridgeProbeDisposition::LegacyEmpty
            ...
            || !bridge_has_capability(probe, COT_REPLY_BRIDGE_CAPABILITY)))
}
...
mod tests {
    use super::{
        ...
        COT_REPLY_BRIDGE_CAPABILITY, DURABLE_REPLY_BRIDGE_CAPABILITY, REPLY_MODE_BRIDGE_CAPABILITY,
        STEERING_BRIDGE_CAPABILITY, UNKNOWN_BRIDGE_INFO_ERROR,
    };
    use serde_json::json;
    use std::fs;
