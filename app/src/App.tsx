...
import { DEFAULT_SCRATCH_PANEL_WIDTH } from "./dashboard/layout/scratchGeometry";
import { ScratchPanel, type ScratchContext } from "./ScratchPanel";
...
function App() {
  ...
  const resolveScratchContext = useCallback((key: string): ScratchContext | null => {
    if (key.startsWith("s:")) {
    ...
    return null;
  }, [sessions, cwdsBySession, allTerminals, hosts]);
  ...
  const centralWorkspace = (
    ...
    >
      ...
      <ScratchPanel
        ...
        onCollapsedChange={setScratchCollapsed}
        width={scratchWidth}
        onWidthChange={setScratchWidth}
        resolveContext={resolveScratchContext}
        ownerEpochKey={terminalDeckOwnerEpochKey}
        interactionBlocked={workspaceInteractionBlocked}
        metadataPending={workspacePresentation.metadataPending}
        onOpenFile={handleOpenFile}
        workspaceRef={dashboardWorkspaceRef}
      />
    </div>
  );
