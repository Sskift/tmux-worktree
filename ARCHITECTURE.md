...
- Relay v1 是当前唯一实现的移动协议。`docs/relay-v2-contract.md` 与 `contracts/relay/v2/external-continuity-authority-v1` 是未来契约，不代表任何一端已经支持 v2、已经有 external backend/adapter，或可以进入 ready/capability advertisement。
...
`contracts/relay/v2/external-continuity-authority-v1` 现已冻结future external authority的closed interop/security语义，但production adapter仍为NO-GO。external backend只拥有按`anchorId`的rollback-independent durable monotonic linearizable record和terminal lifecycle；continuity anchor继续拥有local-state-before-anchor ordering、唯一一代crash reconcile与bounded operation timeout，credential authority继续拥有credential state、ready withdrawal与closed error mapping，BrokerCore仍只消费auth-control authority。future composition唯一顺序是native store open+self-check → external authority injection → credential authority open/reconcile → synchronous admission/active-data fence → broker injection。outer HTTPS固定exact endpoint POST、no redirect、identity/no compression、双向`Cache-Control: no-store`与request JSON Accept；只有status 200和exact JSON Content-Type进入closed response decoder，非200或required header mismatch不读取、解析或回显proxy body。该boundary复用Relay v2 HTTPS JSON limits；workload credential不得redirect转发，request fingerprint按decoded closed tuple字段级比较而非key order/canonical hash。external auth/permission/capacity/rate/timeout code是新冻结的internal namespace，不是Relay wire error；现有5000ms是continuity operation timeout，不是ready-loss close deadline，具体public close code/deadline仍是owning broker contract的required production choice。broker credential与Agent extension使用独立namespace/anchorId/ACL/reset/decommission/tombstone history；broker owner binding由future provisioning提供opaque stable identity，不从issuer/tenant credential字段推断。`broker-credential.v1` fatal failure要求credential authority撤ready/admission并由owning composition全局fence v2 Upgrade/route/active data；`agent-transcript-lifecycle.v1` failure复用现有extension store的`AGENT_AUTHORITY_STORE_CONTINUITY_UNAVAILABLE|COMMIT_UNCERTAIN|CORRUPT` mapping并只隔离extension。unavailable保留可证明timeline/cache lineage并repair/reopen authority，不能reset/new epoch；只有corrupt/明确continuity loss允许extension reset/new epoch。基础credential readiness、connection和route/command/terminal保持不变。当前没有production backend、outer HTTPS transport/auth/config adapter、provisioning/ACL、真实restart/旧备份/网络/灾备证据、ready后connection fence、extension isolation/reset wiring、authority injection或broker v2 composition，因此不能ready、enroll或advertise capability。
