...
Android 另有未接线的纯 Kotlin Relay v2 terminal checkpoint/reducer foundation。版本化 stable timeline identity 完整绑定 profile activation generation、principal/client、host authority/process、Scope/Session、stream/generation 与非敏感 resume-token credential reference；`openId`/`closeId` 则分别属于可持久的单次 logical control attempt，不会被提升为 stable timeline identity。首次 network send 前，独立 pre-open checkpoint 先持久化本地可信 target、mode、open attempt/fingerprint、request 和 delivery authority；已应用的 logical open 另保留精确 result lineage，以便同 attempt 的 response-loss retry/deduplicated replay 幂等收敛而不重新清 parser 或 control。delivery fence 复用 actor-owned `RelayV2EffectGeneration(profileId, profileGeneration, connectionGeneration)`，再加 Android 本地单调 callback-dispatch token；较新的 actor generation 可从新的本地 token 起步，同 generation 才要求本地 token 单调递增。它不接收、伪造或持久化 broker carrier `routeId`/`routeFence`，旧 actor generation 或旧本地 token 的 callback 永久无权。raw-byte network receipt 只推进独立的 received watermark，只有绑定完整 identity、当前 open attempt、parser continuity 和精确 byte range 的 parser callback 确认连续应用后，才推进持久 parser-applied offset并产生累计 ACK。restore 会先按 O(1) queue count 与有界 raw-byte/shape preflight 拒绝不可信状态，再做深 snapshot；若持久 checkpoint 含 parser write/reset in-flight operation，adapter 还必须提供匹配 continuity、operation/range、applied offset 与状态的精确 proof，core 才会完成 callback 或安全重驱，否则进入显式 reset。该 core 独立持有有界 parser pending queue、input/resize pending 与累计 ACK、generation/closed/finalize 时未 ACK input 的 `AMBIGUOUS` 记录、replay、不可变 closed tombstone、可单调退化的 retained-ring metadata、可跨重连精确查询的 close intent 和 reset 原因；closed 到达即禁止新 input/resize/close backend effect，旧 generation 或 closed 时 pending resize 会直接丢弃。单帧上限为 64 KiB，terminal→parser pending/credit 上限为 512 KiB，并对 checkpoint、input、resize 和 frame count 设硬上限；损坏、过期或超限 restore 返回绑定当前本地 authority 的最小 reset 结果，不复制或保留不可信 queue。它只输出 typed parser/network/storage adapter effect，不引用 Room、WebView、xterm、actor、socket 或 Compose；parser continuity、完整 identity 或 schema 无法证明时 fail closed 为 `RESET_REQUIRED`，不会从 v1 name/stream 推导，也不表示 APK terminal v2 已完成。
