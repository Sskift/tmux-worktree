...
Relay v2 的 Broker/Host WSS ingress、carrier、H0–H3 authority、credential authority 与 canonical composition 均位于 `src/relay/v2/`，默认关闭，不接入默认 CLI/shipping 路径。实现状态与 descope 记录见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。
...
| `src/relay/v2/` | Relay v2 strict codec、Broker/Host carrier 与 ingress、H0–H3 authority、credential authority、materialized-state/snapshot、terminal manager，以及 default-off Broker/Host composition 与 shipping owners。默认 CLI/shipping 仍为 Relay v1；显式 v2 在 capability advertisement 前 fail closed，无 v1 fallback。实现状态见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。 |
| `src/relay/extensions/agentTranscriptLifecycle/v1/` | Agent transcript/lifecycle extension：frozen public `codec.ts` 与 broker capability gating。Node 实现模块已于 2026-08 删除；contract 保留为未来独立 proposal。不属于 base v2 交付。 |
| `contracts/relay/v2/external-continuity-authority-v1/` | frozen future external authority 互操作/安全 contract；定义 rollback-independent linearizable read/CAS、provisioning/ACL、closed internal error mapping、namespace lifecycle 与 ready-loss fence。无 production backend；E0 对单节点自托管 lane descoped。 |
| `native/relay-v2-broker-credential-state-store-*/` | **已 descoped/移除**：broker credential native state-store Rust crates（core/napi/platform-common/platform-darwin/platform-linux）。JS loader 在 artifact 缺失时返回 `"missing"` 并回退非原生路径。详见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。 |
| `native/relay-v2-host-credential-atomic-file-cell-*/` | **已 descoped/移除**：host credential atomic file cell Rust crates（napi/platform-common/platform-darwin/platform-linux）。同上回退非原生路径。详见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。 |
...
Dashboard 通过 `DashboardBackend` 驱动 Relay v2 enrollment/process 领域切片：区分 Relay v1 shared-secret profile 与 v2 host credential reference，建模 host bootstrap/refresh、`host.registered`、六项基础 capability、一次性 enrollment、client grant revoke、过期与 SUPERSEDED。Tauri 管理子进程以 protocol v2 与 Node Host composition 通信，敏感值（bootstrap/access/refresh token、v1 shared secret）不进入 IPC；一次性 enrollment code 只到 Tauri decoder 与有界内存 artifact registry，renderer 只收到可撤销 opaque handle。

Relay v2 Host composition（`src/relay/v2/`）拥有：managed connector/runtime、credential authority/Vault、HTTPS coordinator、canonical Host owner、shipping root 与 trusted deployment source。`relay-host --profile v2` 经唯一 trusted source 打开；`--provision-profile-input` 与 `--bootstrap-secret-input` 是 v2-only 的非 secret 路径输入，fd-bound 读取、closed-schema 校验。默认 CLI 仍为 Relay v1，显式 v2 在 capability advertisement 前 fail closed，无 v1 fallback。

实现状态（default-off、NO-GO、native/E0/TLS/device 证据缺失）见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。
...
Relay v2 的实现按 owner 分层：

- **Node codec / carrier / H0–H3**（`src/relay/v2/`）：strict codec、Broker/Host carrier、H0 事务状态、H1 command plane、H2 resource/snapshot、H3 terminal manager。默认关闭，不接入默认 CLI/shipping。
- **Node credential authority**：enrollment create/redeem、client/host refresh、host reauthentication、host bootstrap、response replay-key rotation。仅在隔离模块与专项测试中可调用。
- **Node Broker/Host shipping roots**：`brokerShippingRoot.ts` / `hostShippingRoot.ts` 把 reference-only profile、TLS material、native state-store loader、external continuity provider 装配为 activated composition；`relay-server --v2-profile` 与 `relay-host --profile v2` 经唯一 trusted deployment source 打开。
- **Node single-node self-hosted**（`--v2-single-node-self-hosted`）：credential、continuity row、issuer keyring 共置同一 SQLite（`synchronous=FULL`、exclusive locking、machine-id + device/inode 绑定）。非 production，不满足 E0。
- **Android v2 base runtime**：`RelayV2ConnectionActor`、Room state-sync repository、Outbox、terminal checkpoint/journal、command UI（reply/kill/create_worktree/create_terminal）、credential exchange（OkHttp HTTPS）。默认 capability advertisement 为空。
- **Agent extension**（`src/relay/extensions/agentTranscriptLifecycle/v1/`）：`codec.ts` 与 broker capability gating 保留；Node 实现模块已删除。

实现状态（default-off、NO-GO、native/E0/TLS/device 证据缺失、G4 off mainline）见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。
...
原生 credential state-store crates（broker credential state-store 与 host credential atomic file cell）已 descope 并从仓库移除；JS loader 在原生 artifact 缺失时返回 `"missing"` 并回退非原生路径。详见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。
...
Relay v2 Host composition（`src/relay/v2/`）装配 H0 事务状态、H1 command plane、H2 resource/snapshot、H3 terminal manager 与 codec readiness producer。H1/H2 通过 canonical one-shot candidate 激活，公开 lifecycle 只有 `close()`；H3 由 durable lineage candidate 激活。composition 不在 `relay-host` CLI 默认路径中；capability advertisement 由 readiness → pre-carrier claim 链控制，静态空 advertised capabilities 是 fail-closed 输入。

## Relay v2 Dashboard-owned Host desired state

Dashboard management 通过 hidden Node child（protocol v2）与 Host composition 通信。`connectorDesiredRunning` 是唯一持久 connector 意图；Start/Stop 在 management child 接受后才持久化，Stop 先持久化 stopped 再 drain。Save/Deploy 配置变更由同一 operation owner 串行：旧 config 先 stopped、drain，再提交新 config（stopped）。Dashboard startup 只在 exact binding 匹配且 desired=true 时重放一次 `start_connector`；retry 由 controller 内部 capped backoff 处理，Tauri 不创建 retry timer。

实现状态见 [`docs/relay-v2-status.md`](docs/relay-v2-status.md)。
