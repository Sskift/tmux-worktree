...
此前的 BAU path/JSON authority/continuity 设计直接处理 state/lock path、fd/inode、rename/unlink 与 cleanup，未通过 native 安全验收，因此没有纳入当前交付源码，也不得作为 fallback 重新引入。`contracts/relay/v2/broker-credential-state-store-v1` 另行冻结唯一 `RelayV2BrokerCredentialStateStore` deep port、exact caller-owned `trustedHome` open、transaction-scoped opaque revision、closed native capability/open/error union，以及 single descriptor-backed binary v1。该 container 固定 fileLength/absolute header+payload offsets，只允许 positional payload barrier → positional header barrier publication；同 descriptor 的 nonblocking process-wide exclusive kernel lock 从 self-check 前持有到 close 最后一步，不使用 lock file、shared cursor、named replace/rename 或 cleanup。`src/relay/v2/brokerCredentialStateStore.ts` 只提供 interface 与 raw N-API store/transaction/revision/bytes closed decoder/wrapper；它复制 snapshot/input 并在 uncertain 或 native protocol violation 时 terminal-close wrapper instance。`src/relay/v2/brokerCredentialStateStoreLoader.ts` 只在 pre-open 边界按显式 target、最低 N-API 和固定 artifact 映射 optional-load 一次 binding，并把 capability/open 原样交给上述 closed wrapper；只有固定目标 artifact 自身在 resolve 阶段确实缺失才映射 `native_artifact_missing`，其他加载、binding、capability 或 open 异常都保持 invalid。它精确传 caller-owned absolute `trustedHome` 与冻结上限，不产生 overall ready。`native/relay-v2-broker-credential-state-store-core` 独立消费 frozen binary fixture，纯核心只负责 bounded range selection、owned FIFO admission/transaction lease、opaque revision、same-bytes precedence、generation exhaustion、四阶段 publication、uncertain terminal poison 与 close barrier；fixture JSON/Base64 materializer 仅存在于 tests。未接线的 `RelayV2BrokerCredentialAuthority` source foundation 已通过 opaque port 独占 versioned issuer/enrollment/grant/replay/rate-limit、ready withdrawal 与 external continuity 业务语义，但 broker composition 与 HTTP router 尚未注入。该 foundation 尚无 host bootstrap，因而 fresh host grant、enrollment create/redeem、client/host refresh和 host reauthentication success 均不可达；replay-key rotation lifecycle及 revoke/kid-removal 的 live authorization fence 也未实现。port 不接收或暴露 storage/lock/temp path、fd/inode、`*at` 或 cleanup。Capability supported 和 opened+self-check 都不等于 ready；existing unknown/corrupt/unsafe/identity/durability failure 是 preserving invalid，不是 missing/unsupported。当前仍没有 N-API binary、Darwin/Linux secure-open/lock/真实 durability adapter、production authority injection 或 packaging；Rust core/in-memory/loader focused evidence 不是 native OS、lock、真实 durability、真实网络或 production continuity 证据，native adapter 缺失只表示 v2 unavailable，不能进入 BAU 设计或 v1 fallback。因此 production v2 继续 disabled，Relay v1 build/runtime 不依赖 native module。
