...
repository 消费代际 effect 时必须在 actor-owned apply lease 内完成整个持久化事务；公开 state 的 `matchesGeneration` 只用于非原子的代际预筛，不拥有 phase authority，也不能充当 profile-switch barrier。disconnect/profile switch 会先原子禁止旧代际取得新 lease，再等待已进入的旧事务完成，receipt 之后才允许通过显式 API 清理旧 profile 的 v2 state。Android 另有尚未接 production composition 的 OkHttp HTTPS credential adapter foundation：redeem/refresh 只使用 strict codec 的精确 endpoint/body，并将每次调用封闭为单次 HTTP/1.1 exchange。Relay v2 WSS production adapter 尚未交付；后续 owner 必须在 frame reader 分配或读取 payload 前执行 1 MiB frame/fragment 累计上限，并且 Upgrade 不得 offer WebSocket extension。该 actor/profile、Room repository 与 HTTPS adapter 仍未接入 AppContainer、V2ViewModel 或 production composition root；真实 target resolver、canonical executor adapter、H2 runtime adapter、production carrier pump、v2 Outbox、terminal checkpoint、enrollment、自撤销和 capability advertisement 也仍未交付。当前发布和运行数据流仍全部是 Relay v1。
...
| `docs/relay-v2-contract.md` | 冻结契约 | codec conformance 与未接线的 H0/H1/H2 spool/H3、Android actor/Room/HTTPS foundations 已落地；Relay v2 WSS production adapter、跨端 composition/runtime/capability 均未交付，不能据此宣告 capability 或生成 v2 credential |
