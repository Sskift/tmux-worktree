...
最重要的所有权规则是：`tmux` 保存活进程事实，`tw` 保存 managed lifecycle 事实，Dashboard 保存桌面展示状态，Android 保存手机侧缓存与待发送状态。展示层缓存不能决定一次破坏性操作应走哪条生命周期路径。
...
relay-host on the Mac admin machine
          |-- local scope -> tw serve / local tw RPC / tmux
          `-- SSH scopes  -> remote tw RPC / tmux over Mac SSH credentials
```
