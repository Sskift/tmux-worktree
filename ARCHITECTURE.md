...
可选 X extension 目前另有独立、未接线的 Node host authority reducer foundation：它直接消费 `agent-transcript-lifecycle/v1` authority machine fixture，以显式 restore/corruption boundary、不可变持久索引、active-turn index和全状态 count/canonical-byte预算处理受控 source event。该纯 reducer不是 authority store；结构正确的旧 snapshot 回滚仍必须由未来 store 的原子 continuity watermark 拒绝。X 的 durable store/retention、public codec、snapshot/replay/event log、relay-host adapter、Android consumer和 capability advertisement均未实现，`hostAuthorityMachineConformance`、G4 与 production capability仍为 false/pending。
...
repository 消费代际 effect 时必须在 actor-owned apply lease 内完成整个持久化事务；公开 state 的 `matchesGeneration` 只用于非原子的代际预筛，不拥有 phase authority，也不能充当 profile-switch barrier。disconnect/profile switch 会先原子禁止旧代际取得新 lease，再等待已进入的旧事务完成，receipt 之后才允许通过显式 API 清理旧 profile 的 v2 state。Android 另有尚未接 production composition 的 OkHttp adapter foundation：公共 socket 只使用受信 `wss://`、Bearer `twcap2` header 和 `tw-relay.v2` subprotocol，credential redeem/refresh 只使用 strict codec 的精确 HTTPS endpoint/body；adapter 不拥有 phase、重连或 generation 状态。该 actor/profile、Room repository 与 adapter 仍未接入 AppContainer、V2ViewModel 或 production composition root；真实 target resolver、canonical executor adapter、H2 runtime adapter、production carrier pump、v2 Outbox、terminal checkpoint、enrollment、自撤销和 capability advertisement 也仍未交付。当前发布和运行数据流仍全部是 Relay v1。
