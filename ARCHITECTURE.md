...
repository 消费代际 effect 时必须在 actor-owned apply lease 内完成整个持久化事务；公开 state 的 `matchesGeneration` 只用于非原子的代际预筛，不拥有 phase authority，也不能充当 profile-switch barrier。disconnect/profile switch 会先原子禁止旧代际取得新 lease，再等待已进入的旧事务完成，receipt 之后才允许通过显式 API 清理旧 profile 的 v2 state。Android 另有尚未接 production composition 的 OkHttp HTTPS credential adapter foundation：redeem/refresh 只使用 strict codec 的精确 endpoint/body，并将每次调用封闭为单次 HTTP/1.1 exchange；另有未接线的 bounded RFC6455 WSS adapter foundation，使用系统 trust、RFC2818 hostname verification 和单次 HTTP/1.1 Upgrade，在 frame reader 分配或读取 payload 前执行 1 MiB frame/fragment 累计上限，且不 offer 或接受 WebSocket extension。该 actor/profile、Room repository 与两类 adapter foundation 仍未接入 AppContainer、V2ViewModel 或 production composition root，也没有真实设备或公网 Relay TLS 互操作证据；真实 target resolver、canonical executor adapter、H2 runtime adapter、production carrier pump、v2 Outbox、terminal checkpoint、enrollment、自撤销和 capability advertisement 也仍未交付。当前发布和运行数据流仍全部是 Relay v1。
