...
可选 X extension 目前另有独立、未接线的 Node host authority reducer、public codec、durable store 与 replay runtime foundations。reducer 完整直接消费 `agent-transcript-lifecycle/v1` accepted authority machine fixture，以显式 restore/corruption boundary、不可变持久索引、active-turn index和全状态 count/canonical-byte预算处理受控 source event。store 在单一锁事务内组合 reducer snapshot、source cursor/dedupe、公开 event log、pinned snapshot/replay cut 与 host commit-time retention；最终落盘 bytes 会用读取侧相同的 strict key/node/byte inspection 和完整 restore 校验，pinned page 则在 durable cut 前逐页通过真实 public codec 的 256-item、1 MiB 和 JSON limits。state 与本地 witness 可拒绝损坏、ownership 错误和精确 one-ahead crash cut，但二者位于同一存储域，不能阻止成对回滚；共享 Relay v2 monotonic/CAS continuity anchor 尚未接入，durability security review 因此仍未通过。以上模块都未接 `HostRuntime`、`relayHost`、broker 或 production adapter；Android production consumer、G4 与 capability advertisement仍为 false，当前产品没有远端 Agent reply/lifecycle authority。
