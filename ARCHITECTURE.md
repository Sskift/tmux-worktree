...
| `src/relay/v2/` | 未接入生产 runtime 的 Relay v2 strict codec、Broker/Host carrier foundations、H0 事务状态库、H1 command-plane core、H2 materialized-state 与 persistent pinned snapshot spool、process-scoped H3 terminal core、bounded host route/runtime composition core，以及独立 broker credential native state-store port/raw N-API closed decoder/wrapper、显式 target/N-API/fixed-artifact optional loader和未接线 credential authority/external-continuity source foundation；尚无 N-API/platform implementation、完整 production composition wiring或 capability |
...
| `docs/relay-v2-contract.md` | 冻结契约 | codec conformance 与未接线的 H0/H1/H2 spool/H3、Android actor/Room/HTTPS/bounded RFC6455 WSS foundations 已落地；另有 revision 2 native broker credential store contract（interface/storage/binary/fixture仍v1）、未接线纯 Rust core/platform-common lifecycle owner/optional loader/credential authority source，但无 N-API/platform adapter、host bootstrap 或 production injection；platform-common仍无真实OS fork/syscall/lock/device证据，空 durability allowlist不允许真实 open；WSS foundation 尚未接 production composition，且无真实 TLS/device 互操作证据；跨端 runtime/capability 均未交付，不能据此宣告 capability 或生成 v2 credential |
