...
Android 还提供独立的 `RelayV2ConnectionActor` seam：它复用 v2 profile/credential 与 production codec，以串行 socket ownership、有界 action/effect queue、双代际 fence、握手 watchdog 和 hello query/resync/reject effect 固化客户端 runtime 边界。独立的 `RelayV2StateDatabase`/repository foundation 使用另一数据库和六张 v2 专用表，按 `profileId+principalId+clientInstanceId+hostId+hostEpoch` 持有 authority cursor/required watermark、Scope、opaque Session、pinned snapshot staging records及有界 durable state-event buffer；完整 cut 通过 count/canonical digest后才在单一 Room transaction替换 cache并顺序合并连续 event。它不迁移 v1 `hostId+name` 行，也不包含 Outbox、terminal checkpoint或 credential。
