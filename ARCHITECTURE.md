...
| `native/relay-v2-broker-credential-state-store-napi/` | 独立未接线的 raw N-API binding foundation；module init eager exactly-once 捕获 process lifecycle token，只把 platform-common `ProcessBound*` 与 target platform open seam 转成冻结 ABI。当前仅以本机 Darwin arm64 临时产物验证 focused require/export/prototype/capability/closed-open；没有 qualified real open、actual opened JS transaction、固定 artifact packaging 或 production wiring |
