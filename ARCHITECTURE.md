...
此前的 BAU path/JSON authority/continuity 设计直接处理 state/lock path、fd/inode、rename/unlink 与 cleanup，未通过 native 安全验收，因此没有纳入当前交付源码，也不得作为 fallback 重新引入。`contracts/relay/v2/broker-credential-state-store-v1` 另行冻结唯一 `RelayV2BrokerCredentialStateStore` deep port、exact caller-owned `trustedHome` open、transaction-scoped opaque revision、closed native capability/open/error union，以及 single descriptor-backed binary v1。该 container 固定 fileLength/absolute header+payload offsets，只允许 positional payload barrier → positional header barrier publication；同 descriptor 的 nonblocking process-wide exclusive kernel lock 从 self-check 前持有到 close 最后一步，不使用 lock file、shared cursor、named replace/rename 或 cleanup。`src/relay/v2/brokerCredentialStateStore.ts` 只提供 interface 与 raw N-API store/transaction/revision/bytes closed decoder/wrapper；它复制 snapshot/input 并在 uncertain 或 native protocol violation 时 terminal-close wrapper instance。`src/relay/v2/brokerCredentialStateStoreLoader.ts` 只在 pre-open 边界按显式 target、最低 N-API 和固定 artifact 映射 optional-load 一次 binding，并把 capability/open 原样交给上述 closed wrapper；只有固定目标 artifact 自身在 resolve 阶段确实缺失才映射 `native_artifact_missing`，其他加载、binding、capability 或 open 异常都保持 invalid。它精确传 caller-owned absolute `trustedHome` 与冻结上限，不产生 overall ready。Credential 业务状态、ready withdrawal 与 external continuity 属于后续 T1 的唯一 authority owner，当前尚未注入或接线；port 不接收或暴露 storage/lock/temp path、fd/inode、`*at` 或 cleanup。Capability supported 和 opened+self-check 都不等于 ready；existing unknown/corrupt/unsafe/identity/durability failure 是 preserving invalid，不是 missing/unsupported。当前没有 Rust core、N-API binding、Darwin/Linux adapter、authority injection 或 packaging；in-memory/loader conformance 不是 native、lock、durability 或 continuity 证据，native 缺失只表示 v2 unavailable，不能进入 BAU 设计或 v1 fallback。因此 production v2 继续 disabled，Relay v1 build/runtime 不依赖 native module。
