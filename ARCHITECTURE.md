...
Broker credential 当前还保留一个未接 production composition 的 unsafe BAU path/JSON authority/continuity prototype。它直接处理 state/lock path、fd/inode、rename/unlink 与 cleanup，native 安全验收失败；该结论不是授权继续修补其 path algorithm，它也不是 production adapter。`contracts/relay/v2/broker-credential-state-store-v1` 另行冻结唯一 `RelayV2BrokerCredentialStateStore` deep port、exact caller-owned `trustedHome` open、transaction-scoped opaque revision、closed native capability/open/error union，以及 single descriptor-backed binary v1。该 container 固定 fileLength/absolute header+payload offsets，只允许 positional payload barrier → positional header barrier publication；同 descriptor 的 nonblocking process-wide exclusive kernel lock 从 self-check 前持有到 close 最后一步，不使用 lock file、shared cursor、named replace/rename 或 cleanup。`src/relay/v2/brokerCredentialStateStore.ts` 只提供 interface 与 raw N-API store/transaction/revision/bytes closed decoder/wrapper；它复制 snapshot/input 并在 uncertain 时 terminal-close wrapper instance。业务语义、ready withdrawal 与 continuity 仍由 `RelayV2BrokerCredentialAuthority` 持有，port 不接收或暴露 storage/lock/temp path、fd/inode、`*at` 或 cleanup。Capability supported 不等于 ready；existing unknown/corrupt/unsafe/identity/durability failure 是 preserving invalid，不是 missing/unsupported。当前没有 Rust core、N-API binding、Darwin/Linux adapter、optional loader、authority injection 或 packaging；in-memory conformance 不是 native/lock/durability/continuity 证据，prototype artifacts 不迁移、不删除，native 缺失只表示 v2 unavailable，不能进入 prototype 或 v1 fallback。因此 production v2 继续 disabled，Relay v1 build/runtime 不依赖 native module。
