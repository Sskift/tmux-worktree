...
可选 X extension 目前另有独立、未接线的 Node host authority reducer foundation：它完整直接消费 `agent-transcript-lifecycle/v1` accepted authority machine fixture，以显式 restore/corruption boundary、不可变持久索引、active-turn index和全状态 count/canonical-byte预算处理受控 source event，因此 pure reducer fixture/machine conformance 为 true。该纯 reducer不是 authority store；结构正确的旧 snapshot 回滚仍必须由未来 store 的原子 continuity watermark 拒绝。X 的 durable store continuity/retention、public codec、snapshot/replay/event log、relay-host/runtime adapter、Android production consumer和 capability advertisement均未实现，G4 与 production capability仍为 false。
