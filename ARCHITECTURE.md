...
| `src/relay/v2/` | 未接入生产 runtime 的 Relay v2 strict codec、Broker/Host carrier foundations、H0 事务状态库、H1 command-plane core、H2 materialized-state 与 persistent pinned snapshot spool、process-scoped H3 terminal core、bounded host route/runtime composition core，以及独立 broker credential native state-store port/raw N-API closed decoder/wrapper；尚无 credential authority injection、native implementation、loader、完整 production composition wiring或 capability |
...
此前的 BAU path/JSON authority/continuity 设计直接处理 state/lock path、fd/inode、rename/unlink 与 cleanup，未通过 native 安全验收，因此没有纳入当前交付源码，也不得作为 fallback 重新引入。`contracts/relay/v2/broker-credential-state-store-v1` 另行冻结唯一 `RelayV2BrokerCredentialStateStore` deep port、exact caller-owned `trustedHome` open、transaction-scoped opaque revision、closed native capability/open/error union，以及 single descriptor-backed binary v1。该 container 固定 fileLength/absolute header+payload offsets，只允许 positional payload barrier → positional header barrier publication；同 descriptor 的 nonblocking process-wide exclusive kernel lock 从 self-check 前持有到 close 最后一步，不使用 lock file、shared cursor、named replace/rename 或 cleanup。`src/relay/v2/brokerCredentialStateStore.ts` 只提供 interface 与 raw N-API store/transaction/revision/bytes closed decoder/wrapper；它复制 snapshot/input 并在 uncertain 或 native protocol violation 时 terminal-close wrapper instance。Credential 业务状态、ready withdrawal 与 external continuity 属于后续 T1 的唯一 authority owner，当前尚未注入或接线；port 不接收或暴露 storage/lock/temp path、fd/inode、`*at` 或 cleanup。Capability supported 不等于 ready；existing unknown/corrupt/unsafe/identity/durability failure 是 preserving invalid，不是 missing/unsupported。当前没有 Rust core、N-API binding、Darwin/Linux adapter、optional loader、authority injection 或 packaging；in-memory conformance 不是 native/lock/durability/continuity 证据，native 缺失只表示 v2 unavailable，不能进入 BAU 设计或 v1 fallback。因此 production v2 继续 disabled，Relay v1 build/runtime 不依赖 native module。
