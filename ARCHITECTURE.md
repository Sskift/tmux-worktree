...
Android 另有未接线的纯 Kotlin Relay v2 terminal checkpoint/reducer foundation。版本化 stable timeline identity 完整绑定 profile activation generation、principal/client、host authority/process、Scope/Session、stream/generation 与非敏感 resume-token credential reference；`openId`/`closeId` 则分别属于可持久的单次 logical control attempt，不会被提升为 stable timeline identity。delivery fence 复用 actor-owned `RelayV2EffectGeneration(profileId, profileGeneration, connectionGeneration)`，再加 Android 本地单调 callback-dispatch token；它不接收、伪造或持久化 broker carrier `routeId`/`routeFence`，旧 actor generation 或旧本地 token 的 callback 永久无权。raw-byte network receipt 只推进独立的 received watermark，只有绑定完整 identity、当前 open attempt、parser continuity 和精确 byte range 的 parser callback 确认连续应用后，才推进持久 parser-applied offset并产生累计 ACK。该 core 独立持有有界 parser pending queue、input/resize pending 与累计 ACK、generation/closed/finalize 时未 ACK input 的 `AMBIGUOUS` 记录、replay、不可变 closed tombstone、可单调退化的 retained-ring metadata 和 reset 原因；closed 到达即禁止新 input/resize/close backend effect，旧 generation 或 closed 时 pending resize 会直接丢弃。单帧上限为 64 KiB，terminal→parser pending/credit 上限为 512 KiB，并对 checkpoint、input、resize 和 frame count 设硬上限；损坏或超限 restore 不复制不可信 queue。它只输出 typed parser/network/storage adapter effect，不引用 Room、WebView、xterm、actor、socket 或 Compose；parser continuity、完整 identity 或 schema 无法证明时 fail closed 为 `RESET_REQUIRED`，不会从 v1 name/stream 推导，也不表示 APK terminal v2 已完成。
