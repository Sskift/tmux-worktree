...
Android 另有未接线的纯 Kotlin Relay v2 terminal checkpoint/reducer foundation。它以版本化 identity 完整绑定 profile activation generation、principal/client、host authority/process、Scope/Session、stream/generation、open/close control ID 与非敏感 resume-token credential reference，并把 terminal timeline identity 与可显式重绑的 socket/effect delivery fence 分开；旧 delivery callback 永久无权修改新 route。raw-byte network receipt 只推进独立的 received watermark，只有绑定完整 identity、parser continuity 和精确 byte range 的 parser callback 确认连续应用后，才推进持久 parser-applied offset并产生累计 ACK。该 core 独立持有有界 parser pending queue、input/resize pending 与累计 ACK、generation 变化后的未 ACK input `AMBIGUOUS` 记录、replay/closed/final watermark 和 reset 原因；旧 generation pending resize 会丢弃，由新 open 的 cols/rows 建立基线。单帧上限为 64 KiB，terminal→parser pending/credit 上限为 512 KiB，并对 checkpoint、input、resize 和 frame count 设硬上限。它只输出 typed parser/network/storage adapter effect，不引用 Room、WebView、xterm、actor、socket 或 Compose；parser continuity、完整 identity 或 schema 无法证明时 fail closed 为 `RESET_REQUIRED`，不会从 v1 name/stream 推导。
