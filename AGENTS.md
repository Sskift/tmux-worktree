...
- `docs/relay-v2-contract.md` 是冻结的 v2 最小互操作契约；`contracts/relay/v2` 及 Node/Android 的独立 v2 codec 已形成 conformance 基础。Node 另有未接 production runtime 的 H0/H1、H2 materialized-state 与 pinned snapshot spool、H3 foundations，以及只连接这些 typed authority port、以同步 readiness withdrawal 和 exact route token 关闭竞态的 bounded host route/runtime composition core；`src/relayHost.ts` 和其他 production composition 均未构造该 core。Android 另有未接 production composition 的 v2 actor/profile、隔离 Room state-sync repository、HTTPS credential 与 bounded RFC6455 WSS adapter foundations，以及纯 Outbox authority 和 Android-free terminal checkpoint/reducer cores；state DB 本身不包含 Outbox、terminal checkpoint 或 credential。WSS foundation 已存在，但 production WSS/composition 与真实设备/公网 Relay TLS 互操作证据仍未交付。可选 `agent.transcript-lifecycle.v1` 也只有未接 runtime 的 Node host authority reducer 与 Android lifecycle/notification reducer foundations；没有 durable/public codec/Room/actor/UI/系统通知接线。broker、relay-host、Android production composition、credential/enrollment 和 capability 接线仍未交付。任何一端都不得因为 codec、契约或 foundation 存在就宣告 v2/extension capability、生成 v2 enrollment、把 v1 shared secret 提升为 v2 credential，或在失败时静默降级/重试到另一协议。
...
| 路径 | 所有权与职责 |
| --- | --- |
| `src/` | Node.js/TypeScript `tw` CLI：配置、managed state、git worktree/tmux 生命周期、JSON RPC、SSH Hosts、automations、`tw serve`、Relay v1 broker/host，以及尚未接入生产 runtime 的独立 Relay v2 codec、H0–H3 与 bounded host route composition foundations。 |
