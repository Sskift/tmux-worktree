...
- Node 的 default-off `HostConnectorController` 与 canonical `HostCarrier` 之间已有独立 attempt adapter foundation：只把注入 factory 创建的 exact actor、transport 与 drain proof 绑定为 controller 的 `startAttempt`/`disposeAndDrain` port；controller 继续独占 controller generation/start/stop/fence，actor 继续独占 carrier generation/status/queue/route。adapter 不创建 WSS、process、credential、retry timer、readiness 或 capability，不暴露 raw actor/transport/callback authority，也未接 `relay-host` 或其他 production composition；失败不允许 fallback v1。
