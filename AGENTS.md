...
- Android Relay v2 Outbox 的现有 core + Room repository 另提供独立 internal durable enqueue authority：调用方必须提交已稳定的 exact activation namespace、host lineage、command/Scope/Session identity 与 arguments draft；repository 在单个事务 strict restore、执行唯一 `Enqueue` plan并提交 meta/entry 后，只返回非敏感 row correlation。production root 现仅通过 `RelayV2BaseRuntimeComposition` 的窄 product port把该 authority 接给 Session reply：actor-issued exact current ONLINE lease 与 Room exact opaque Session cut 同时成立后，固定提交 `send_agent_message`（`pane=0`、`submit=true`），commit 后才按 exact composition/cut callback fence与未变化 raw draft清理并调用既有 fresh producer/dispatcher。Session projection 的 Room read/构造与所有 recovery materialized-state mutation共用唯一 product mutation owner，发布还需在 actor lifecycle lock 内 final recheck原 ONLINE cut，旧 ONLINE→RESYNCING/disconnect callback不能复活。UI stable identity为不可碰撞的本地编码，v2 phase只投影为明确的 transport health/host status，不宣告 ready/capability。其他 command producer仍未接；该路径不表示完整 UI command runtime 已交付。
