...
- Android Relay v2 Outbox 的现有 core + Room repository 另提供独立 internal durable enqueue authority：调用方必须提交已稳定的 exact activation namespace、host lineage、command/Scope/Session identity 与 arguments draft；repository 在单个事务 strict restore、执行唯一 `Enqueue` plan并提交 meta/entry 后，只返回非敏感 row correlation。production root 现仅通过 `RelayV2BaseRuntimeComposition` 的窄 product port把该 authority 接给 Session reply 与已 admission 的 existing-Session kill：两者都必须同时持有 actor-issued exact current ONLINE lease 与 issuing composition 的 Room exact opaque Session cut，并在 materialized Session revalidation 后提交；reply 固定使用 `send_agent_message`（`pane=0`、`submit=true`），kill 固定使用 `RelayV2OutboxArguments.KillSession`。Room commit 后才调用既有 fresh producer/dispatcher；kill 成功只表示 queued，不等于 terminated，也不能乐观删除 Session，Session 只能等待权威 `sessions.changed` delete 后消失。Session projection 的 Room read/构造与所有 recovery materialized-state mutation共用唯一 product mutation owner，发布还需在 actor lifecycle lock 内 final recheck原 ONLINE cut，旧 ONLINE→RESYNCING/disconnect callback不能复活。UI stable identity为不可碰撞的本地编码，v2 phase只投影为明确的 transport health/host status，不宣告 ready/capability。server/host production v2、default capability、enrollment、terminal 与其他 command producer仍未交付；该路径不表示完整 UI command runtime 已交付。
