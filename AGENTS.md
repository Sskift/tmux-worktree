...
- Android Relay v2 Outbox 的现有 core + Room repository 另提供独立 internal durable enqueue authority：调用方必须提交已稳定的 exact activation namespace、host lineage、command/Scope/Session identity 与 arguments draft；repository 在单个事务 strict restore、执行唯一 `Enqueue` plan并提交 meta/entry 后，只返回非敏感 row correlation。production root 现仅通过 `RelayV2BaseRuntimeComposition` 的窄 product port把该 authority 接给 Session reply、已 admission 的 existing-Session kill，以及已有至少一个 materialized Session 的 UI 可见 Scope 上的 `create_worktree`。reply/kill 必须持有 actor-issued exact current ONLINE lease 与 issuing composition 的 Room exact opaque Session cut；create 则只接受 composition 从当前 materialized Session cut 派生、同 Scope 共享的 opaque `RelayV2ScopeCreateCut`，其 authority 精确绑定 activation namespace、`scopesRevision`、Scope 与 hostEpoch，不接受 UI raw scopeId 作为权威。三条路径都在唯一 product mutation owner 下重读并重验 materialized authority后提交；reply 固定使用 `send_agent_message`（`pane=0`、`submit=true`），kill 固定使用 `RelayV2OutboxArguments.KillSession`，create 固定构造 canonical `RelayV2OutboxArguments.CreateWorktree` 并进入 `sessionId=null` 的 exact create lane。Room commit 后才调用既有 fresh producer/dispatcher；create 成功只表示 queued，不等于 created，不本地插入或导航 Session，必须等待权威 `sessions.changed` upsert；kill 成功同样只表示 queued，不等于 terminated，也不能乐观删除 Session。空 Scope 当前不支持 create，未来需要独立 durable Scope cut，不能伪造。Session projection 的 Room read/构造与所有 recovery materialized-state mutation共用唯一 product mutation owner，发布还需在 actor lifecycle lock 内 final recheck原 ONLINE cut，旧 ONLINE→RESYNCING/disconnect callback不能复活。UI stable identity为不可碰撞的本地编码，v2 phase只投影为明确的 transport health/host status，不宣告 ready/capability。client terminal、enrollment 与 self-revoke 已进入 admitted explicit-v2 default-off root；server/host production capability、其他 command producer 与真实设备互操作仍未交付，该路径不表示完整 UI command runtime 已交付。
