...
- Node 的 default-off `HostConnectorController` 与 canonical `HostCarrier` 之间已有独立 attempt adapter foundation：adapter 仍只把 exact actor、transport 与 drain proof 绑定为 controller 的 `startAttempt`/`disposeAndDrain` port；新建的 closed managed connector/runtime composition 则在内部为每个 controller generation 构造该 exact actor 与 adapter，并让 injected factory 只提供 transport、`bindConnection` 和 drain evidence。所有 actor 复用现有唯一 route sink、同一 host lineage、固定空 advertised capabilities 与 `tw-relay.v2` dialect；controller 继续独占 controller generation/start/stop/fence，actor 继续独占 carrier generation/status/queue/route，Host runtime 继续独占 route/readiness。公开 handle 不暴露 actor、transport、factory、status subscriber、sender 或 route owner；close 先永久 fence start，再经 controller `stopAndDrain`，最后 dispose runtime。该 composition 与 adapter 均不创建 WSS、process、credential、retry timer、capability advertisement 或 fallback，也未接 `relay-host` 或其他 production composition；失败不允许 fallback v1。
