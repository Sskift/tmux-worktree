...
- Android production `RelayV2BaseRuntimeComposition` 另以现有 `SessionSelectionController` / `SelectedSessionReadController` / `SelectedSessionPresentationController` 装配 capability-fenced selected-Session 只读 seam：它只接受同一 composition 签发且仍 current 的 opaque Session cut，并复用 actor current repository read authority、materialized Session exact read和同一 Agent durable repository。`V2ViewModel.timeline(sessionId)` 对每个 current cut只读取一个有界 structured page并仅映射 transcript，run/turn scope保留在本地稳定 identity中，lifecycle evidence绝不提升为 Session-wide `AgentState`。production optional capabilities仍为空，所以当前只在actor read cut处返回`Unavailable`并投影空页，零Agent Room read、零网络、零通知；foreign/stale cut、disconnect和profile fence同样清页。这不表示真实 Agent reply/state/capability、通知或完整 extension runtime已交付。
