...
- Node 另有 default-off、未接 CLI/Tauri 的 Dashboard management protocol-v2 composition session owner：它只从 canonical dist owner entry 接受同 lineage 的真实 credential authority/exchange coordinator、Host connector controller 与 HostCarrier actor，并通过 composition owner 的 fresh exclusive claim 在同一闭包内构造 management composition，把其 request handle 绑定到现有串行 stdio v2 owner；任何既存 composition activation 都不能升级为 session，session claim 也不能被普通 composition factory 重取。调用方 AbortSignal 在 owner activation 前以捕获的原生 descriptor 完成 brand/override 校验，只作为 exact session identity 和外部 abort source；session 将 abort 转发给内部 AbortController，composition/credential/connector 只接收 session-owned signal，后续不通过调用方属性查找监听或清理。公开 handle 只有 one-shot `run()` 与 `closeAndDrain()`；EOF、坏帧、普通失败、abort 或显式 close 都先 fence 新请求，再 exactly-once 等待 composition/controller drain。隐藏 Dashboard child 现固定使用 exact protocol v2，并新增 default-off 的 selection owner（`relayV2DashboardManagementChildRuntime.ts`）：production wrapper 经注入的窄 factory 绑定 canonical Host shipping root 的新 default-off `dashboardManagement` seam；只有 qualified injected Host owner 才能运行真实 session，factory 发布 handle 前失败或合法 handle 明确无 management 且 clean close 时收敛到唯一 typed `UNAVAILABLE` session，session/close 不确定或非法 handle 一律 ordinary exit 1，绝不启动第二 session。production CLI 不注入 trusted deployment inputs，且 .app 只携带 cli.cjs（child entry 为 standalone external），因此 child 对所有 operation 仍 fail closed 返回 `UNAVAILABLE`；它不启动 connector/WSS/process，不生成 credential/enrollment/capability，也没有 v1 fallback。
