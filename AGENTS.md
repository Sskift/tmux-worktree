...
- Dashboard、`relay-server` 和 `relay-host` 的默认 CLI/shipping 路径仍只运行 Relay v1；Android 没有显式、已通过 cold-start admission 的 v2 profile 时也继续使用 Relay v1。Relay v1 shared secret、wire 与升级兼容层仍是生产事实，不能作为“旧代码”删除。
- `docs/relay-v2-contract.md` 冻结了未来 v2 的六项基础互操作能力。Node/Android codec、explicit default-off Broker/Host/Dashboard composition，以及 Android 受限 explicit-v2 base runtime 已形成实现基础，但任何 v2 失败都必须 fail closed，不能静默回退或重试到 v1。
- Relay v2 整体仍为 **NO-GO**：production `qualifiedRecords=[]`，真实 E0/native durability、public TLS/device interoperability、签名和渠道 release/shipping 证据均缺失。loopback local-development 与 single-node self-hosted 只用于开发或非生产部署；self-hosted Host只提供冻结的六项基础 capability，不构成 production qualification。
- 可选 `agent.transcript-lifecycle.v1` 只保留隔离的 contract、Node authority/codec/store/runtime foundations 与 Android dormant foundations。canonical production/self-hosted Host不再构造 Agent store、Codex process 或 extension attachment，也不advertise该capability；不得宣告Agent reply、Waiting/Failed/Completed或通知能力已交付。
- Android explicit-v2 lane已有基础state-sync、Outbox、terminal、credential rollover与self-revoke foundations，但连接状态不等于ready/capability，且没有真实server/host production capability。Room/Keystore/profile-switch barrier等Android边界仍以 `docs/android-v2-architecture.md` 为准。
- 构建、unsigned artifact、真实设备验证、TLS、签名、上传和渠道发布是不同证据；任一构建成功都不代表其他发布面已生产可用。
