...
class FakeLark {
  ...
  async replyCotResult(messageId, chatId, text, idempotencyKey, replyMode) {
    const sent = { messageId, chatId, text, idempotencyKey, replyMode, cot: true };
    this.replies.push(sent);
    await this.beforeReply?.(sent);
    if (this.failReply) throw new Error("reply acknowledgement lost");
    return { messageId: `reply-${this.replies.length}`, raw: {} };
  }

  async sendCotResult(chatId, text, idempotencyKey) {
    const sent = { chatId, text, idempotencyKey, cot: true };
    ...
    return { messageId: `group-cot-${this.groupCards.length}`, raw: {} };
  }
...
test("Lark CLI bridge delivers COT progress plus a separate final answer", async () => {
  const calls = [];
  const responses = [
    { data: { cot_id: "cot-topic", message_id: "om_cot_topic" } },
    { data: {} },
    { data: { message_id: "om-topic-reply" } },
    { data: {} },
    { data: { cot_id: "cot-direct", message_id: "om_cot_direct" } },
    { data: {} },
    { data: { message_id: "om-direct-reply" } },
    { data: {} },
    { data: { cot_id: "cot-group", message_id: "om_cot_group" } },
    { data: {} },
    { data: { message_id: "om-group-reply" } },
    { data: {} },
  ...
  const answer = "answer <at id=\"ou-surprise\"></at>";

  assert.equal(
    (await adapter.replyCotResult("om-root", "oc-one", answer, "tw-cot-topic", "topic")).messageId,
    "om-topic-reply",
  );
  assert.equal(
    (await adapter.replyCotResult("om-root", "oc-one", answer, "tw-cot-direct", "direct")).messageId,
    "om-direct-reply",
  );
  assert.equal(
    (await adapter.sendCotResult("oc-one", answer, "tw-cot-group")).messageId,
    "om-group-reply",
  );
  ...
  assert.deepEqual(calls[0], [
    "--profile", "bot", "api", "POST", "/open-apis/im/v1/message_cot",
  ...
  const topicStart = JSON.parse(calls[1][calls[1].indexOf("--data") + 1]);
  assert.equal(topicStart.cot_id, "cot-topic");
  assert.equal(topicStart.message_id, "om_cot_topic");
  assert.deepEqual(topicStart.events.map(({ event_type }) => event_type), [
    "RUN_STARTED",
    "REASONING_START",
  ...
  assert.ok(topicStart.events.every(({ content }) => typeof JSON.parse(content) === "object"));
  assert.equal(topicStart.events[0].timestamp, 1713200000000);
  assert.deepEqual(calls[2], [
    "--profile", "bot", "im", "+messages-reply",
    "--message-id", "om-root",
    "--markdown", "answer <\u200bat id=\"ou-surprise\"><\u200b/at>",
    "--reply-in-thread",
    "--idempotency-key", "tw-cot-topic",
    "--as", "bot",
    "--json",
  ]);
  const topicFinish = JSON.parse(calls[3][calls[3].indexOf("--data") + 1]);
  assert.deepEqual(topicFinish.events.map(({ event_type }) => event_type), ["RUN_FINISHED"]);

  const directCreate = JSON.parse(calls[4][calls[4].indexOf("--data") + 1]);
  assert.deepEqual(directCreate, { receive_id: "oc-one", origin_message_id: "om-root" });
  assert.equal(calls[6].includes("--reply-in-thread"), false);
  const groupCreate = JSON.parse(calls[8][calls[8].indexOf("--data") + 1]);
  assert.deepEqual(groupCreate, { receive_id: "oc-one" });
  assert.deepEqual(calls[10].slice(0, 6), [
    "--profile", "bot", "im", "+messages-send", "--chat-id", "oc-one",
  ]);
  ...
  await assert.rejects(
    adapter.replyCotResult("om-root", "oc-one", answer, "tw-cot-invalid", "future"),
    /invalid Feishu reply mode/,
  );
  assert.equal(calls.length, 15, "an unknown mode must fail before invoking lark-cli");
  assert.deepEqual(parseFeishuCotHandle({ data: { cotId: "cot-camel", messageId: "om_camel" } }), {
    cotId: "cot-camel",
    messageId: "om_camel",
  });
...
test("a task already running at bind time posts its structured final answer after it stops", async () => {
  ...
  try {
    ...
    assert.equal(h.lark.groupCards.length, 1);
    assert.equal(h.lark.groupCards[0].chatId, "oc-one");
    assert.equal(h.lark.groupCards[0].cot, true);
    assert.match(h.lark.groupCards[0].text, /The exact structured final answer\./);
    assert.doesNotMatch(h.lark.groupCards[0].text, /已结束|完整结果请在 TW 中查看/);
    assert.match(h.lark.groupCards[0].idempotencyKey, /^tw-[0-9a-f]{40}$/);
...
test("a group message converts an inherited local task into one marker-correlated steering turn", async () => {
  ...
  try {
    ...
    assert.equal(h.lark.groupCards.length, 0, "a steered inherited task has no duplicate completion result");
  } finally {
    await h.bridge.close();
    rmSync(h.root, { recursive: true, force: true });
  }
...
test("an uncertain local-task COT result is never sent twice", async () => {
...
test("a delayed local-task COT result never blocks terminal lease renewal", async () => {
...
test("completion authority mismatch fails closed before a group COT result is sent", async () => {
...
test("one authorized mentioned message owns the target, writes once, and posts only marked output", async () => {
  ...
  try {
    ...
    assert.equal(h.lark.replies[0].cot, true);
...
test("restart never repeats non-idempotent COT creation after delivery began", async () => {
  const h = harness();
  try {
    const binding = await h.bridge.createBinding({
      chatId: "oc-one", chatName: "bridge group", sessionName: "managed-one", createdBy: "ou-owner",
    ...
    const state = h.store.read();
    const turn = state.turns.at(-1);
    const text = "must not be duplicated after restart";
    turn.status = "replying";
    state.replies.push({
      id: turn.outboundAttemptId,
      turnId: turn.id,
      sourceMessageId: turn.messageId,
      idempotencyKey: `tw-${createHash("sha256").update(turn.outboundAttemptId).digest("hex").slice(0, 40)}`,
      status: "sending",
      textDigest: createHash("sha256").update(text).digest("hex"),
      createdAt: new Date().toISOString(),
      deliveryKind: "turn-reply",
    ...
    const restarted = new FeishuBridge({
      ...
      lark: h.lark,
      store: h.store,
      instanceId: "daemon-after-cot-create",
      botOpenId: "ou-bot",
    });
    restarted.initializeAfterRestart();
    await restarted.pollTurns();
    ...
    const recovered = h.store.read();
    assert.equal(recovered.replies.find((reply) => reply.id === turn.outboundAttemptId)?.status, "uncertain");
    assert.equal(recovered.turns.find((candidate) => candidate.id === turn.id)?.status, "recovery-required");
    assert.equal(h.lark.replies.length, 0, "a COT create without an idempotency key must never be replayed");
    await restarted.close();
  } finally {
    await h.bridge.close();
    rmSync(h.root, { recursive: true, force: true });
