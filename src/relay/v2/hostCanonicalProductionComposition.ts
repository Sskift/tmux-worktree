...
export async function openRelayV2HostCanonicalProductionComposition(
  ...
  try {
    ...
    // The materialized bootstrap is consulted first because it is structurally
    // inert around any recoverable authority: it returns null before touching
    // reconcile or the cut source whenever the spool holds any recovered or
    // published cut, reservation, or in-flight build. Durable tombstones remain
    // request replay fences only and cannot authorize or block a fresh exact
    // materialized cut. Recovered issuance therefore proceeds unchanged
    // whenever it can succeed at all.
