...
export const RELAY_AGENT_CHAT_CAPABILITY = "agent.chat.v1" as const;
...
export const RELAY_AGENT_CHAT_CODEC_ERROR_DOMAIN =
  "relay-agent-chat-codec-v1" as const;
...
function validateTurn(value: RelayV2JsonValue): void {
  ...
  exact(
    turn,
    [
      "turnId", "session", "userMessage", "status", "sentAt",
    ],
    ["reply", "error", "completedAt", "steeredMessages"],
  );
  ...
  const reply = nullable(field(turn, "reply"), (item) => (
    text(item, RELAY_AGENT_CHAT_MAX_TURN_UTF8_BYTES)
  ));
  ...
  if (status === "working") {
    if (reply !== null || completedAt !== null) reject("schema-mismatch");
  } else if (status === "replied") {
    if (reply === null || error !== null || completedAt === null) reject("schema-mismatch");
