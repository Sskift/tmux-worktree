...
export class FeishuBridge {
  ...
  initializeAfterRestart(): void {
    ...
    for (const binding of this.state.bindings) {
      ...
      if (binding.activityWatch?.status === "sending") {
        ...
        binding.activityWatch.error = "bridge restarted while the COT result disposition was unknown";
        changed = true;
      }
    }
    for (const turn of this.state.turns) {
    ...
    for (const reply of this.state.replies) {
      if (reply.status === "sending") {
        ...
        reply.error = "bridge restarted after non-idempotent COT delivery began";
        changed = true;
        continue;
      }
  ...
  pauseBinding(bindingId: string, force = false): Promise<FeishuBinding> {
    return this.serial(async () => {
      ...
      if (turn?.status === "replying") {
        throw new Error("the Feishu COT reply is being delivered; wait for its disposition before pausing");
      }
      if (watch?.status === "sending") {
        throw new Error("the local Agent COT result is being delivered; wait for its disposition before pausing");
      }
      if ((turn || watch) && !force) {
  ...
  removeBinding(
    ...
    return this.serial(async () => {
      ...
      const watch = this.activeActivityWatch(binding);
      if (turn?.status === "replying") {
        throw new Error("the Feishu COT reply is being delivered; wait for its disposition before unlinking");
      }
      if (watch?.status === "sending") {
        throw new Error("the local Agent COT result is being delivered; wait for its disposition before unlinking");
      }
      if ((turn || watch) && !force) {
  ...
  takeoverBinding(
    ...
    return this.serial(async () => {
      ...
      const watch = this.activeActivityWatch(binding);
      if (turn?.status === "replying") {
        throw new Error("the Feishu COT reply is being delivered; wait for its disposition before takeover");
      }
      if (watch?.status === "sending") {
        throw new Error("the local Agent COT result is being delivered; wait for its disposition before takeover");
      }
      if (turn && !force) {
  ...
  private queueActivityCompletion(
    ...
    // Binding lifecycle cards are ordered before COT results, but their
  ...
  private async deliverActivityCompletion(
    ...
    try {
      const text = activityResult.truncated
        ? `${activityResult.text}\n\n---\n最终回答过长，仅展示前半部分；完整内容请在 TW 中查看。`
        : activityResult.text;
      const sendResult = await this.lark.sendCotResult(
        binding.chatId,
        text,
        idempotencyKey,
      );
      ...
      await this.serial(async () => {
        ...
        this.markBindingStale(currentBinding, "local Agent COT result disposition is uncertain");
        this.persist();
      });
    }
  }
  ...
  private ensureRecoveryNoticeAttempts(): void {
    ...
    for (const binding of this.state.bindings) {
      ...
      const text = watch.status === "uncertain"
        ? "本地 Agent 已停止，但 CoT 与最终答复的投递结果无法确认。为避免重复发送 Agent 内容，请先在本地检查终端和群消息，再恢复绑定。"
  ...
  private async deliverOutboundAttempt(attemptId: string): Promise<void> {
    const prepared = await this.serial(async (): Promise<{
      attempt: FeishuOutboundReply;
      turnChatId?: string;
    } | undefined> => {
      ...
      if (attempt.deliveryKind === "turn-reply") {
        attempt.status = "sending";
        this.persist();
      }
      return {
        attempt: structuredClone(attempt),
        ...(turn ? { turnChatId: turn.chatId } : {}),
      };
    });
    if (!prepared) return;

    try {
      const result = prepared.attempt.deliveryKind === "turn-reply"
        ? await this.lark.replyCotResult(
          prepared.attempt.sourceMessageId,
          prepared.turnChatId!,
          prepared.attempt.text!,
          prepared.attempt.idempotencyKey,
          prepared.attempt.replyMode!,
        )
        : await this.lark.sendCard(
          prepared.attempt.chatId!,
          buildFeishuReplyCard(
            prepared.attempt.text!,
            prepared.attempt.sessionName!,
            prepared.attempt.tone!,
          ),
          prepared.attempt.idempotencyKey,
        );
      await this.serial(async () => {
        const attempt = this.state.replies.find((candidate) => candidate.id === attemptId);
        const expectedStatus = attempt?.deliveryKind === "turn-reply" ? "sending" : "prepared";
        if (!attempt || attempt.status !== expectedStatus) return;
        const turn = this.state.turns.find((candidate) => candidate.id === attempt.turnId);
        attempt.status = "sent";
        attempt.completedAt = nowIso(this.now);
