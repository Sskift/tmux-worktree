...
import { neutralizeFeishuMentions, type FeishuReplyCard } from "./feishuReplyCard.js";
...
export interface FeishuCotHandle {
  cotId: string;
  messageId: string;
}

interface FeishuCotEvent {
  event_type: string;
  content: string;
  timestamp: number;
}
...
export interface FeishuLarkAdapter {
  ...
  sendCotResult(chatId: string, text: string, idempotencyKey: string): Promise<FeishuReplyResult>;
...
export function parseFeishuCotHandle(value: unknown): FeishuCotHandle {
  const visit = (candidate: unknown): FeishuCotHandle | undefined => {
    ...
    const cotId = pickString(candidate.cot_id, candidate.cotId);
    const messageId = pickString(candidate.message_id, candidate.messageId);
    if (cotId && messageId?.startsWith("om_")) return { cotId, messageId };
  ...
  const handle = visit(value);
  if (!handle) throw new Error("create COT response omitted cot_id or message_id");
  return handle;
}
...
function cotStartEvents(idempotencyKey: string, timestamp: number): FeishuCotEvent[] {
  const runId = `run-${idempotencyKey}`;
  const reasoningMessageId = `reasoning-${idempotencyKey}`;
  return [
    cotEvent("RUN_STARTED", { threadId: runId, runId }, timestamp),
    cotEvent("REASONING_START", { messageId: reasoningMessageId }, timestamp + 1),
    cotEvent(
      "REASONING_MESSAGE_START",
      { messageId: reasoningMessageId, role: "reasoning" },
      timestamp + 2,
    ),
    cotEvent(
      "REASONING_MESSAGE_CONTENT",
      { messageId: reasoningMessageId, delta: "TW Agent 已完成处理，正在发送最终答复。" },
      timestamp + 3,
    ),
    cotEvent("REASONING_MESSAGE_END", { messageId: reasoningMessageId }, timestamp + 4),
    cotEvent("REASONING_END", { messageId: reasoningMessageId }, timestamp + 5),
  ];
}

function cotFinishedEvent(idempotencyKey: string, timestamp: number): FeishuCotEvent {
  const runId = `run-${idempotencyKey}`;
  return cotEvent("RUN_FINISHED", { threadId: runId, runId, status: "done" }, timestamp);
}

function cotReplyUncertainEvent(timestamp: number): FeishuCotEvent {
  return cotEvent("RUN_ERROR", {
    message: "最终答复投递结果无法确认",
    code: "REPLY_DELIVERY_UNCERTAIN",
  }, timestamp);
}
...
export class LarkCliBridgeAdapter implements FeishuLarkAdapter {
  ...
  async sendCotResult(
    ...
    return this.deliverCotResult({ chatId, text, idempotencyKey });
  }

  private async deliverCotResult(input: {
    chatId: string;
    ...
    const createRaw = await this.runner(this.commandArgs([
      "api", "POST", "/open-apis/im/v1/message_cot",
      "--params", JSON.stringify({ receive_id_type: "chat_id" }),
      "--data", JSON.stringify({
        receive_id: input.chatId,
        ...(input.sourceMessageId ? { origin_message_id: input.sourceMessageId } : {}),
        ...(input.sourceMessageId && input.replyMode === "topic" ? { reply_in_thread: true } : {}),
      }),
      "--as", "bot",
      "--json",
    ]));
    const cot = parseFeishuCotHandle(createRaw);
    await this.updateCot(cot, cotStartEvents(input.idempotencyKey, this.now()));

    let replyRaw: unknown;
    try {
      replyRaw = input.sourceMessageId
        ? await this.runner(this.commandArgs([
          "im", "+messages-reply",
          "--message-id", input.sourceMessageId,
          "--markdown", neutralizeFeishuMentions(input.text),
          ...(input.replyMode === "topic" ? ["--reply-in-thread"] : []),
      ...
      try {
        await this.updateCot(cot, [cotReplyUncertainEvent(this.now())]);
      } catch {}
      throw error;
    }

    await this.updateCot(cot, [cotFinishedEvent(input.idempotencyKey, this.now())]);
    return {
      messageId: findReplyMessageId(replyRaw),
      raw: { cot: createRaw, reply: replyRaw },
    };
  }

  private async updateCot(cot: FeishuCotHandle, events: FeishuCotEvent[]): Promise<void> {
    await this.runner(this.commandArgs([
      "api", "PUT", "/open-apis/im/v1/message_cot",
      "--data", JSON.stringify({
        events,
        message_id: cot.messageId,
        cot_id: cot.cotId,
      }),
      "--as", "bot",
      "--json",
